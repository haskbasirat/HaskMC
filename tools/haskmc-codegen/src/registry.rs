            ...
            .map(|(reg_name, entries)| {
                let entry_tokens: Vec<TokenStream> = entries
                    ...
                    .map(|(entry_name, entry_data)| {
                        fn json_to_nbt_tag(v: &Value) -> haskmc_nbt::tag::NbtTag {
                            match v {
                                Value::Null => haskmc_nbt::tag::NbtTag::End,
                                Value::Bool(b) => {
                                    haskmc_nbt::tag::NbtTag::Byte(if *b { 1 } else { 0 })
                                }
                                Value::Number(num) => {
                                    if let Some(i) = num.as_i64() {
                                        if i >= i32::MIN as i64 && i <= i32::MAX as i64 {
                                            haskmc_nbt::tag::NbtTag::Int(i as i32)
                                        } else {
                                            haskmc_nbt::tag::NbtTag::Long(i)
                                        }
                                    } else if let Some(f) = num.as_f64() {
                                        haskmc_nbt::tag::NbtTag::Double(f)
                                    } else {
                                        haskmc_nbt::tag::NbtTag::Int(0)
                                    }
                                }
                                Value::String(s) => {
                                    haskmc_nbt::tag::NbtTag::String(s.clone().into())
                                }
                                Value::Array(arr) => haskmc_nbt::tag::NbtTag::List(
                                    arr.iter().map(json_to_nbt_tag).collect(),
                                ),
                                Value::Object(obj) => {
                                    let mut compound = haskmc_nbt::compound::NbtCompound::new();
                                    for (k, val) in obj {
                        ...
                        let bytes = if let haskmc_nbt::tag::NbtTag::Compound(compound) = nbt_tag {
                            haskmc_nbt::Nbt::from(compound).write_unnamed()
                        } else {
                            Vec::new().into()
                        };
