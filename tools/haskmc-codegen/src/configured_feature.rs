...
pub fn build() -> TokenStream {
    ...
    let entries: Vec<TokenStream> = json
        ...
        .map(|(name, value)| {
            ...
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! {
                map.insert(haskmc_data::configured_feature::ConfiguredFeature::#variant_name, #cf);
            }
        })
        .collect();
    ...
    quote! {
        ...
        fn build_configured_features() -> std::collections::HashMap<haskmc_data::configured_feature::ConfiguredFeature, ConfiguredFeature> {
    ...
    match type_str {
        ...
        "minecraft:vegetation_patch" | "minecraft:waterlogged_vegetation_patch" => {
            ...
            let surface = match config["surface"].as_str().unwrap_or("floor") {
                "ceiling" => {
                    quote! { haskmc_util::math::vertical_surface_type::VerticalSurfaceType::Ceiling }
                }
                _ => {
                    quote! { haskmc_util::math::vertical_surface_type::VerticalSurfaceType::Floor }
                }
            };
...
fn value_to_rule_test(v: &Value) -> TokenStream {
    ...
    match type_str {
        ...
        "minecraft:block_match" => {
            ...
            quote! { RuleTest::BlockMatch(BlockMatchRuleTest { block: haskmc_data::BlockId::#block_ident }) }
        }
        ...
        "minecraft:tag_match" => {
            ...
            quote! { RuleTest::TagMatch(TagMatchRuleTest { tag: haskmc_data::tag::Block::#tag_ident }) }
        }
        "minecraft:random_block_match" => {
            ...
            quote! { RuleTest::RandomBlockMatch(RandomBlockMatchRuleTest { block: haskmc_data::BlockId::#block_ident, probability: #prob }) }
        }
...
fn value_to_block_list(v: &Value) -> TokenStream {
    if let Some(tag) = v.as_str().and_then(|s| s.strip_prefix('#')) {
        ...
        return quote! { &haskmc_data::tag::Block::#ident.1 };
    }
    ...
    if let Some(arr) = v.as_array() {
        for b in arr {
            if let Some(s) = b.as_str() {
                ...
                let ident = syn::Ident::new(&name, proc_macro2::Span::call_site());
                blocks.push(quote! { haskmc_data::BlockId::#ident.as_u16() });
            }
        }
    }
    quote! { &[#(#blocks),*] }
...
///
/// # Returns
/// `PlacedFeatureWrapper::Named` for a string value or `PlacedFeatureWrapper::Direct` for an inline object; defaults to `PlacedFeatureWrapper::Named(haskmc_data::placed_feature::PlacedFeature::Acacia)` for other types.
fn value_to_placed_feature_wrapper(v: &Value) -> TokenStream {
    match v {
        Value::String(s) => {
            let name = s.strip_prefix("minecraft:").unwrap_or(s);
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! { PlacedFeatureWrapper::Named(haskmc_data::placed_feature::PlacedFeature::#variant_name) }
        }
        Value::Object(_) => {
        ...
        _ => {
            quote! { PlacedFeatureWrapper::Named(haskmc_data::placed_feature::PlacedFeature::Acacia) }
        }
    }
}
...
///
/// # Returns
/// `Feature::Named` for a string value or `Feature::Inlined` for an object; defaults to `Feature::Named(haskmc_data::configured_feature::ConfiguredFeature::NoOp)` for other types.
fn value_to_feature_ref(v: &Value) -> TokenStream {
    match v {
        Value::String(s) => {
            let name = s.strip_prefix("minecraft:").unwrap_or(s);
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! { Feature::Named(haskmc_data::configured_feature::ConfiguredFeature::#variant_name) }
        }
        Value::Object(_) => {
            let cf = value_to_configured_feature(v);
            quote! { Feature::Inlined(Box::new(#cf)) }
        }
        _ => quote! { Feature::Named(haskmc_data::configured_feature::ConfiguredFeature::NoOp) },
    }
}
