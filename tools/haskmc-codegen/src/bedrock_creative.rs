...
pub fn build() -> TokenStream {
    ...
    if !std::path::Path::new(nbt_path).exists() {
        let generated_path = "../../crates/haskmc-data/src/generated/bedrock_creative.rs";
    ...
    let mut reader = haskmc_nbt::deserializer::NbtReadHelperBedrock::new(
        haskmc_nbt::deserializer::NbtStreamReader(&mut cursor),
    );
    let nbt = haskmc_nbt::Nbt::read(&mut reader).expect("Failed to read creative_items.nbt");

    let mut group_tokens = Vec::new();
    if let Some(haskmc_nbt::tag::NbtTag::List(groups_list)) = nbt.get("groups") {
        for g_tag in groups_list {
            if let haskmc_nbt::tag::NbtTag::Compound(g) = g_tag {
                ...
                if let Some(haskmc_nbt::tag::NbtTag::Compound(icon)) = g.get("icon") {
    ...
    if let Some(haskmc_nbt::tag::NbtTag::List(items_list)) = nbt.get("items") {
        for item_tag in items_list {
            if let haskmc_nbt::tag::NbtTag::Compound(item) = item_tag {
