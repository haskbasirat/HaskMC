...
pub fn build() -> TokenStream {
    ...
    let entries: Vec<TokenStream> = json
        ...
        .map(|(name, value)| {
            ...
            quote! {
                map.insert(haskmc_data::placed_feature::PlacedFeature::#variant_name, #pf);
            }
        })
        .collect();
    ...
    quote! {
        ...
        fn build_placed_features() -> std::collections::HashMap<haskmc_data::placed_feature::PlacedFeature, PlacedFeature> {
        ...
        if let Some(tag) = s.strip_prefix('#') {
            ...
            return quote! {
                BlockPredicate::MatchingBlockTag(MatchingBlockTagPredicate {
                    ...
                    tag: haskmc_data::tag::Block::#tag_ident,
                })
            };
        }
        ...
        quote! {
            {
                ...
                BlockStateCodec {
                    name: &haskmc_data::Block::#block_ident,
...
pub fn value_to_block_state(v: &Value) -> TokenStream {
    ...
    if let Some(props) = v["Properties"].as_object() {
        ...
        quote! {
            haskmc_data::Block::#block_ident.default_state
        }
    }
}
