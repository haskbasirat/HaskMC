use haskmc_nbt::deserializer::{NbtReadHelper, NbtReadHelperBedrock};
use haskmc_util::math::{experience::Experience, vector3::Vector3};
...
pub fn build() -> TokenStream {
    ...
    let geyser_nbt_compound =
        haskmc_nbt::nbt_compress::read_gzip_compound_tag(Cursor::new(geyser_blocks_data)).unwrap();
    ...
    quote! {
        ...
        use haskmc_util::math::int_provider::{UniformIntProvider, IntProvider, NormalIntProvider};
        use haskmc_util::loot_table::*;
        use haskmc_util::math::experience::Experience;
...
fn get_be_data_from_nbt<R: Read + Seek>(
    ...
    let nbt_reader =
        &mut NbtReadHelperBedrock::new(haskmc_nbt::deserializer::NbtStreamReader(&mut *reader));
    ...
    loop {
        ...
        let nbt = haskmc_nbt::Nbt::read(nbt_reader).unwrap();
        ...
        let properties = nbt
            ...
            .map(|(key, val)| {
                let unpacked = match val {
                    ...
                    haskmc_nbt::tag::NbtTag::Int(v) => v.to_string(),
                    haskmc_nbt::tag::NbtTag::String(v) => v.into(),
                    _ => {
...
fn parse_geyser_entry(
    entry: &haskmc_nbt::tag::NbtTag,
    java_block_name: &str,
) -> (String, BTreeMap<String, String>) {
    let compound = match entry {
        haskmc_nbt::tag::NbtTag::Compound(c) => c,
    ...
    if let Some(state_comp) = compound.get_compound("state") {
        for (key, val) in state_comp.clone() {
            let unpacked = match val {
                ...
                haskmc_nbt::tag::NbtTag::Int(v) => v.to_string(),
                haskmc_nbt::tag::NbtTag::String(v) => v.to_string(),
                _ => {
                    panic!(
