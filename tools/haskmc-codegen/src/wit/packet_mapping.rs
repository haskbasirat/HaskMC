...
pub fn build_java_mapping() -> String {
    ...
    output.push_str("// Generated by haskmc-codegen. Do not edit.\n");
    ...
    output.push_str("use haskmc_protocol::codec::var_int::VarInt;\n");
    output.push_str("use haskmc_util::version::JavaMinecraftVersion;\n");
    output.push_str("use bytes::Bytes;\n");
    output.push_str("use std::io::Cursor;\n");
    output.push_str("use std::any::Any;\n");
    output.push_str("use haskmc_protocol::packet::MultiVersionJavaPacket;\n");
    output.push_str("use haskmc_protocol::packet::Packet;\n\n");
    ...
    for state in client_states {
        process_packets(
            ...
            "java_packet",
            "ClientboundPacket",
            &format!("haskmc_protocol::java::client::{}", state),
            false,
    ...
    for state in server_states {
        process_packets(
            ...
            "java_packet",
            "ServerboundPacket",
            &format!("haskmc_protocol::java::server::{}", state),
            false,
...
pub fn build_bedrock_mapping() -> String {
    ...
    process_packets(
        ...
        "packet",
        "BClientboundPacket",
        "haskmc_protocol::bedrock::client",
        true,
        MappingMode::Serialize,
    );
    ...
    process_packets(
        ...
        "packet",
        "BServerboundPacket",
        "haskmc_protocol::bedrock::server",
        true,
        MappingMode::Deserialize,
    );
...
fn convert_value(
    ...
    let expr = match type_ident {
        ...
        "Identifier" => match mode {
            MappingMode::Serialize => {
                format!(
                    "haskmc_util::identifier::Identifier::try_from({}).unwrap()",
                    src
                )
            }
        ...
        "VarUInt" => match mode {
            MappingMode::Serialize => {
                format!(
                    "haskmc_protocol::codec::var_uint::VarUInt({}.try_into().unwrap())",
                    src
                )
            }
            MappingMode::Deserialize | MappingMode::ToWit => {
        ...
        "VarLong" | "VarULong" => {
            ...
            let path = if is_ulong {
                "haskmc_protocol::codec::var_ulong::VarULong"
            } else {
                "haskmc_protocol::codec::var_long::VarLong"
            };
        ...
        "TextComponent" => match mode {
            MappingMode::Serialize => {
                let tmp = dst.unwrap_or("tmp").replace('.', "_");
                prep.push_str(&format!(
                    "{}let component_{} = haskmc_util::text::TextComponent::text({}.clone());\n",
                    prep_prefix, tmp, src
                ));
                if is_ref {
        ...
        "BlockPos" => match mode {
            MappingMode::Serialize => {
                format!(
                    "haskmc_util::math::position::BlockPos::new({}.0, {}.1, {}.2)",
                    src, src, src
                )
            }
            MappingMode::Deserialize | MappingMode::ToWit => {
        ...
        "Vector3" => match mode {
            MappingMode::Serialize => {
                format!(
                    "haskmc_util::math::vector3::Vector3::new({}.0 as _, {}.1 as _, {}.2 as _)",
                    src, src, src
                )
            }
            MappingMode::Deserialize | MappingMode::ToWit => {
        ...
        "LpVector3d" => match mode {
            MappingMode::Serialize => {
                ...
                format!(
                    "haskmc_protocol::codec::lp_vector_3d::LpVector3d(haskmc_util::math::vector3::Vector3::new(parsed_{0}[0], parsed_{0}[1], parsed_{0}[2]))",
                    tmp
                )
            }
            MappingMode::Deserialize | MappingMode::ToWit => {
    ...
    match mode {
        ...
        MappingMode::Deserialize => {
            ...
            if rust_path_prefix.contains("java") {
                ...
                output.push_str("            use haskmc_protocol::ServerPacket;\n");
                output.push_str(&format!(
                    "            let p = <{}::{} as haskmc_protocol::ServerPacket>::read(&mut payload, &version).ok()?;\n",
                    rust_path_prefix, struct_name
                ));
            } else {
                output.push_str(&format!(
                    "        id if id == <{}::{} as haskmc_protocol::Packet>::PACKET_ID as i32 => {{\n",
                    rust_path_prefix, struct_name
                ));
                output.push_str("            use haskmc_protocol::BServerPacket;\n");
                output.push_str(&format!(
                    "            let p = <{}::{} as haskmc_protocol::BServerPacket>::read(&mut Cursor::new(payload)).ok()?;\n",
                    rust_path_prefix, struct_name
                ));
            }
            output.push_str(prep_code);
