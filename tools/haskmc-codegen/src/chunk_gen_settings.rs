    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            Self::Absolute { absolute } => {
                tokens.extend(quote!(YOffset::Absolute(haskmc_util::y_offset::Absolute { absolute: #absolute })));
            }
            Self::AboveBottom { above_bottom } => {
                tokens.extend(quote!(YOffset::AboveBottom(haskmc_util::y_offset::AboveBottom { above_bottom: #above_bottom })));
            }
            Self::BelowTop { below_top } => {
                tokens.extend(quote!(YOffset::BelowTop(haskmc_util::y_offset::BelowTop { below_top: #below_top })));
            }
        }
    }
}
        ...
        match self {
            ...
            Self::StoneDepth {
                ...
                let surface_type_token = match surface_type.as_str() {
                    "ceiling" => quote!(
                        haskmc_util::math::vertical_surface_type::VerticalSurfaceType::Ceiling
                    ),
                    "floor" => {
...
pub fn build() -> TokenStream {
    ...
    quote!(
        ...
        use haskmc_util::random::RandomDeriver;
        use haskmc_util::y_offset::YOffset;
        use crate::biome::{Biome, Parameter, ParameterPoint};
        use haskmc_util::y_offset::Absolute;
        ...
        pub struct StoneDepthMaterialCondition {
            ...
            pub surface_type: haskmc_util::math::vertical_surface_type::VerticalSurfaceType,
        }
