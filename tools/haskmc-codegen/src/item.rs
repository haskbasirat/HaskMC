...
use haskmc_nbt::{Nbt, compound::NbtCompound};
use haskmc_util::registry::TagType;
use haskmc_util::text::TextContent;
use haskmc_util::{registry::RegistryEntryList, text::TextComponent};
...
pub fn build() -> TokenStream {
    ...
    let be_item_components: BTreeMap<String, Option<NbtCompound>> = {
        ...
        let nbt = haskmc_nbt::nbt_compress::read_gzip_compound_tag(&mut cursor)
            .expect("Failed to parse bedrock/item_components.nbt");
        nbt.child_tags
            .into_iter()
            .map(|(k, v)| {
                let comp = match v {
                    haskmc_nbt::tag::NbtTag::Compound(c) => Some(c),
                    _ => None,
                };
                (k.into_string(), comp)
            })
