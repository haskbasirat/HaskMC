...
use haskmc_data::recipes::{CraftingRecipeTypes, RECIPES_CRAFTING};
use haskmc_data::screen::WindowType;
use haskmc_data::statistic::StatisticCategory;
use haskmc_data::tag;
use haskmc_data::tag::Taggable;
use haskmc_protocol::codec::recipe::{DynamicRecipe, OwnedCraftingRecipe};
...
impl ResultSlot {
    ...
    fn refill_output(&self) -> ItemStack {
        let result = if let Some(matched) = self.match_recipe() {
            ...
            let item = haskmc_data::item::Item::from_registry_key(key)
