...
use haskmc_protocol::ser::ReadingError;
use haskmc_world::data::player_data::PlayerDataError;
use std::fmt::Display;
use tracing::Level;

use crate::log_at_level;

pub trait HaskMcError: Send + std::error::Error + Display {
...
impl<ErrorType: HaskMcError + 'static> From<ErrorType> for Box<dyn HaskMcError> {
    fn from(error: ErrorType) -> Self {
        Box::new(error)
    }
}
impl HaskMcError for InventoryError {
    fn is_kick(&self) -> bool {
        use InventoryError::{
...
impl HaskMcError for ReadingError {
    fn is_kick(&self) -> bool {
        true
    }
...
impl HaskMcError for PlayerDataError {
    fn is_kick(&self) -> bool {
        false
    }
