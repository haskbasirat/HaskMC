...
use dashmap::DashMap;
use haskmc_data::chunk::Biome;
use haskmc_data::item::{BedrockItem, BedrockItemVersion};
use haskmc_protocol::bedrock::client::item_registry::{CItemRegistry, ItemData};
use haskmc_protocol::bedrock::client::level_event::{CLevelEvent, LevelEvent};
use haskmc_protocol::bedrock::client::{CBiomeDefinitionList, block_actor_data::CBlockActorData};
use haskmc_protocol::bedrock::network_item::{NetworkItemDescriptor, NetworkItemStackDescriptor};
use haskmc_protocol::codec::data_component::data_to_proto_sound;
use haskmc_world::generation::proto_chunk::GenerationCache;
use rayon::prelude::*;
...
use haskmc_config::BasicConfiguration;
use haskmc_data::block_properties::{blocks_movement, is_air};
use haskmc_data::block_rotation::{Mirror, Rotation};
use haskmc_data::chunk_gen_settings::GenerationSettings;
use haskmc_data::data_component_impl::EquipmentSlot;
use haskmc_data::dimension::Dimension;
use haskmc_data::entity::MobCategory;
use haskmc_data::fluid::FluidState;
use haskmc_data::game_rules::{GameRule, GameRuleValue};
use haskmc_data::{
    Block, BlockStateId,
...
use haskmc_inventory::crafting::recipe_provider::RecipeProvider;
use haskmc_inventory::screen_handler::InventoryPlayer;
use haskmc_nbt::compound::NbtCompound;
use haskmc_protocol::bedrock::client::set_actor_data::{CSetActorData, PropertySyncData};
use haskmc_protocol::bedrock::client::start_game::{CStartGame, ServerTelemetryData};
use haskmc_protocol::java::client::play::{
...
use haskmc_protocol::java::client::play::{CSetEntityMetadata, Metadata};
use haskmc_protocol::{
...
use haskmc_util::resource_location::ResourceLocation;
use haskmc_util::text::{TextComponent, color::NamedColor};
...
use haskmc_world::inventory::Clearable;
use haskmc_world::world::{GetBlockError, WorldPortalExt};
use haskmc_world::{
    CURRENT_BEDROCK_MC_VERSION, biome,
    chunk::{io::Dirtiable, palette::bedrock_water_state},
    inventory::Inventory,
};
use haskmc_world::{chunk::ChunkData, world::BlockAccessor};
use haskmc_world::{level::Level, tick::TickPriority};
pub use haskmc_world::{world::BlockFlags, world_info::LevelData};
...
use haskmc_world::chunk::ChunkHeightmapType::{self, MotionBlocking};
use uuid::Uuid;
use weather::Weather;
...
fn bedrock_chest_block_actor(state_id: BlockStateId, position: BlockPos) -> Option<NbtCompound> {
    ...
    if !block.has_tag(&haskmc_data::tag::Block::C_CHESTS_WOODEN)
        && !block.has_tag(&haskmc_data::tag::Block::MINECRAFT_COPPER_CHESTS)
    {
        return None;
    }
...
impl HaskMcError for GetBlockError {
...
impl World {
    ...
    pub fn load(
        ...
        let haskmc_custom_data_path = level
            .level_folder
            .root_folder
            .join("haskmc_custom_data.nbt");
        let legacy_custom_data_path = level
            .level_folder
            .root_folder
            .join("pumpkin_custom_data.nbt");
        let custom_data_path = if haskmc_custom_data_path.exists() {
            haskmc_custom_data_path
        } else {
            legacy_custom_data_path
        };
        let custom_data = if custom_data_path.exists()
            && let Ok(bytes) = std::fs::read(&custom_data_path)
            && let Ok(nbt) = haskmc_nbt::Nbt::read_unnamed(
                &mut haskmc_nbt::deserializer::NbtReadHelperJava::new(&mut std::io::Cursor::new(
                    bytes,
                )),
            ) {
            nbt.root_tag
    ...
    fn save_block_entities(&self, chunk_pos: Vector2<i32>) {
        ...
        for block_entity in block_entities {
            ...
            {
                nbt.put_compound("HaskMCCustomData", custom_data.clone());
            }
    ...
    pub fn send_remove_mob_effect(&self, entity: &Entity, effect_type: &'static StatusEffect) {
        ...
        let be_packet = haskmc_protocol::bedrock::client::CMobEffect {
            target_runtime_id: VarULong(entity.entity_id as u64),
            event_id: haskmc_protocol::bedrock::client::CMobEffect::EVENT_REMOVE,
    ...
    pub fn send_add_mob_effect(&self, entity: &Entity, effect: &haskmc_data::potion::Effect) {
        let chunk_pos = entity.chunk_pos.load();
        let mut flags: i8 = 0;
        if effect.ambient {
            flags |= 0x01;
        ...
        let be_packet = haskmc_protocol::bedrock::client::CMobEffect {
            target_runtime_id: VarULong(entity.entity_id as u64),
            event_id: haskmc_protocol::bedrock::client::CMobEffect::EVENT_ADD,
    ...
    fn broadcast_java_grouped<P: ClientPacket>(
        ...
        for (version, recipients) in recipients_by_version {
            let packet_data = match JavaClient::serialize_packet_for_version(packet, version) {
                ...
                Err(haskmc_protocol::ser::WritingError::UnsupportedVersion(_)) => {
                    continue;
                }
                Err(err) => {
                    error!(
    ...
    fn component_to_bedrock_text(message: &TextComponent) -> SText<'static> {
        match &*message.0.content {
            haskmc_util::text::TextContent::Translate {
                translate,
                bedrock_translate,
                with,
            } => {
                ...
                let parameters = with
                    .iter()
                    .map(haskmc_util::text::TextComponentBase::to_bedrock_string)
                    .collect();
            ...
            _ => SText::system_message(
                message
                    .0
                    .to_bedrock_legacy(haskmc_util::translation::Locale::EnUs),
            ),
        }
    }
                ...
                if tracked_count > 4096 {
                    recipient.kick(
                        ...
                        &TextComponent::translate_cross(
                            haskmc_data::translation::java::MULTIPLAYER_DISCONNECT_TOO_MANY_PENDING_CHATS,
                            haskmc_data::translation::java::MULTIPLAYER_DISCONNECT_TOO_MANY_PENDING_CHATS,
                            [],
                        ),
                    );
    ...
    fn broadcast_skin_parts<B: BClientPacket>(
        ...
        for (version, recipients) in recipients_by_version {
            ...
            for meta in [
                Metadata::new(
                    haskmc_data::tracked_data::player::PLAYER_MODE_CUSTOMISATION,
                    skin_parts,
                ),
                Metadata::new(
                    haskmc_data::tracked_data::player::PLAYER_MODE_CUSTOMIZATION_ID,
                    skin_parts,
                ),
            ] {
    ...
    pub fn play_sound_event(
        &self,
        sound: &haskmc_data::data_component_impl::IdOr<
            haskmc_data::data_component_impl::SoundEvent,
        >,
        ...
        for (chunk_section, updates) in block_state_updates_by_chunk_section {
            ...
            if updates.len() == 1 {
                ...
                self.broadcast_to_chunk_editioned(
                    ...
                    &haskmc_protocol::bedrock::client::CUpdateBlock::new(
                        block_pos,
                        be_block_id as u32,
                    ),
                );
                ...
                {
                    let bytes = haskmc_nbt::Nbt::from(nbt).write_unnamed();
                ...
                for (block_pos, block_state_id) in &updates {
                    let be_block_id = BlockState::to_be_network_id(*block_state_id);
                    let update_packet = haskmc_protocol::bedrock::client::CUpdateBlock::new(
                        *block_pos,
                        be_block_id as u32,
                    );
            ...
            for (block_pos, block_state_id) in &updates {
                ...
                let packet = haskmc_protocol::bedrock::client::CUpdateBlock::with_layer(
                    *block_pos,
        ...
        tick_data
            ...
            .for_each(|batch| {
                ...
                for scheduled_tick in batch {
                    ...
                    if let Some(haskmc_block) = world.block_registry.get_haskmc_block(block.id) {
                        haskmc_block.on_scheduled_tick(OnScheduledTickArgs {
                            world: &world,
                            block,
                            position: &pos,
                        });
        ...
        tick_data
            ...
            .for_each(|batch| {
                ...
                for scheduled_tick in batch {
                    ...
                    if let Some(haskmc_fluid) = world.block_registry.get_haskmc_fluid(fluid.id) {
                        haskmc_fluid.on_scheduled_tick(&world, fluid, &pos);
                    }
                }
            });
            ...
            .for_each(|batch| {
                ...
                for scheduled_tick in batch {
                    ...
                    if let Some(block) = block
                        && let Some(haskmc_block) = world.block_registry.get_haskmc_block(block.id)
                    {
                        haskmc_block.random_tick(RandomTickArgs {
                            world: &world,
                            block,
                            position: &pos,
                        });
                    ...
                    if let Some(fluid) = fluid
                        && let Some(haskmc_fluid) = world.block_registry.get_haskmc_fluid(fluid.id)
                    {
                        haskmc_fluid.random_tick(fluid, &world, &pos);
                    }
                }
            });
    ...
    pub async fn spawn_bedrock_player(
        ...
        static BEDROCK_CRAFTING_DATA: std::sync::OnceLock<
            Vec<haskmc_protocol::bedrock::client::BedrockRecipe>,
        > = std::sync::OnceLock::new();
        ...
        let start_game = CStartGame {
            ...
            // Bedrock represents the local player at eye height; HaskMC stores feet position.
            ...
            level_settings,
            level_id: String::new(),
            level_name: "HaskMC world".to_string(),
            ...
            server_version: "HaskMC Rust Server".to_string(),
            compound_id: 10,
        ...
        let (groups, entries) = CREATIVE_CONTENT.get_or_init(|| {
            let groups = haskmc_data::bedrock_creative::CREATIVE_GROUPS
                .iter()
                .map(|g| {
                    ...
                    let icon_item = if g.icon_item_id != 0 {
                        NetworkItemDescriptor {
                            ...
                            nbt_data: haskmc_nbt::Nbt::default(),
            ...
            let entries = haskmc_data::bedrock_creative::CREATIVE_ENTRIES
                .iter()
                .enumerate()
        ...
        let bedrock_recipes = BEDROCK_CRAFTING_DATA.get_or_init(|| {
            use haskmc_data::item::{Item, JavaToBedrockItemMapping};
            use haskmc_data::recipes::{CraftingRecipeTypes, RecipeIngredientTypes};
            use haskmc_protocol::bedrock::client::{
                BedrockRecipe, BedrockShapedRecipe, BedrockShapelessRecipe, ItemDescriptorCount,
                RecipeUnlockRequirement,
            };
            use haskmc_protocol::bedrock::network_item::NetworkItemDescriptor;
            use haskmc_protocol::codec::{var_int::VarInt, var_uint::VarUInt};

            let mut mapped_recipes = Vec::new();
            let mut network_id_counter = 1u32;

            for recipe in haskmc_data::recipes::RECIPES_CRAFTING {
                let map_ingredient = |ing: &RecipeIngredientTypes| -> ItemDescriptorCount {
                    let item_key = match ing {
                        ...
                        RecipeIngredientTypes::Tagged(tag) => {
                            ...
                            haskmc_data::tag::get_tag_ids(
                                haskmc_data::tag::RegistryKey::Item,
                                tag_name,
                            )
                            .and_then(|ids| {
                    ...
                    CraftingRecipeTypes::CraftingShaped {
                                ...
                                JavaToBedrockItemMapping::from_java_item_id(item.id)
                        {
                            ...
                            mapped_recipes.push(BedrockRecipe::Shaped(BedrockShapedRecipe {
                                recipe_id: format!("haskmc:recipe_{network_id_counter}"),
                                width: VarInt(width),
        ...
        let crafting_data = haskmc_protocol::bedrock::client::CCraftingData {
        ...
        self.broadcast_packet_except_editioned(
            ...
            &CPlayerInfoUpdate::new(
                (PlayerInfoFlags::ADD_PLAYER
                    ...
                    .bits(),
                &[haskmc_protocol::java::client::play::Player {
                    uuid: gameprofile.id,
                    actions: &[
        ...
        let bedrock_add_player = CAddPlayer {
            ...
            abilities_data: haskmc_protocol::bedrock::client::SerializedAbilitiesData {
                target_player_raw_id: runtime_id as i64,
                player_permissions:
                    haskmc_protocol::bedrock::client::PlayerPermissionLevel::Visitor,
                command_permissions: haskmc_protocol::bedrock::client::CommandPermissionLevel::Any,
                layers: vec![
                    haskmc_protocol::bedrock::client::SerializedAbilitiesDataSerializedLayer {
                        serialized_layer: 0,
                        abilities_set: 0,
                        ability_value: 0,
                        fly_speed: 0.05,
            ...
            .filter(|p| p.gameprofile.id != gameprofile.id)
        {
            ...
            let ex_add_player = CAddPlayer {
                ...
                abilities_data: haskmc_protocol::bedrock::client::SerializedAbilitiesData {
                    ...
                    player_permissions:
                        haskmc_protocol::bedrock::client::PlayerPermissionLevel::Visitor,
                    command_permissions:
                        haskmc_protocol::bedrock::client::CommandPermissionLevel::Any,
                    layers: vec![
                        haskmc_protocol::bedrock::client::SerializedAbilitiesDataSerializedLayer {
                            serialized_layer: 0,
                            abilities_set: 0,
            ...
            let ex_be_mob_equipment = haskmc_protocol::bedrock::client::CMobEquipment {
    ...
    pub async fn spawn_java_player(
            ...
            && client.version.load() >= JavaMinecraftVersion::V_1_13
        {
            ...
            for &key in haskmc_data::tag::RegistryKey::NETWORK_KEYS {
                if haskmc_data::tag::get_registry_key_tags(version, key)
                    .is_some_and(|map| !map.is_empty())
                {
                    tags.push(key);
                }
            }
            let packet = haskmc_protocol::java::client::play::CUpdateTagsPlay::new(&tags);
            if let Ok(packet_data) = JavaClient::serialize_packet_for_version(&packet, version) {
        ...
        let java_player = [haskmc_protocol::java::client::play::Player {
            uuid: gameprofile.id,
            ...
            .filter(|c| c.gameprofile.id != id)
        {
...

            {
                ...
                let be_mob_equipment = haskmc_protocol::bedrock::client::CMobEquipment {
                    target_runtime_id: (existing_player.entity_id() as u64).into(),
                    item: (&held_item).into(),
                    slot: 0,
                    selected_slot: 0,
    ...
    pub async fn respawn_player(self: &Arc<Self>, player: &Arc<Player>, alive: bool) {
        ...
        player
            .send_packet_now_editioned(
                ...
                &haskmc_protocol::bedrock::client::CSetSpawnPosition {
                    spawn_position_type:
                        haskmc_protocol::bedrock::client::SpawnPositionType::WorldRespawn,
    ...
    pub(crate) fn set_block_breaking(
        ...
        let chunk_pos = location.chunk_position(); // HaskMC's BlockPos already has this method
    ...
    pub fn get_fluid(&self, position: &BlockPos) -> &'static haskmc_data::fluid::Fluid {
    ...
    pub fn get_block_and_fluid(
        ...
        &'static haskmc_data::Block,
        &'static haskmc_data::fluid::Fluid,
    ) {
        let id = self.get_block_state_id(position);
        let block = Block::from_state_id(id);
    ...
    pub fn update_neighbors(
        ...
        for direction in BlockDirection::update_order() {
            ...
            if let Some(neighbor_haskmc_block) =
                self.block_registry.get_haskmc_block(neighbor_block.id)
            {
                neighbor_haskmc_block.on_neighbor_update(OnNeighborUpdateArgs {
                    world: self,
            ...
            if let Some(neighbor_haskmc_fluid) =
                self.block_registry.get_haskmc_fluid(neighbor_fluid.id)
            {
                neighbor_haskmc_fluid.on_neighbor_update(
                    self,
                    neighbor_fluid,
                    &neighbor_pos,
                    false,
    ...
    pub fn update_neighbor(self: &Arc<Self>, neighbor_block_pos: &BlockPos, source_block: &Block) {
        ...
        if let Some(neighbor_haskmc_block) = self.block_registry.get_haskmc_block(neighbor_block.id)
        {
            neighbor_haskmc_block.on_neighbor_update(OnNeighborUpdateArgs {
                world: self,
                block: neighbor_block,
    ...
    pub fn add_block_entity(&self, block_entity: Arc<dyn BlockEntity>) {
        ...
        if let Some(nbt) = &block_entity_nbt {
            let bytes = haskmc_nbt::Nbt::from(nbt.clone()).write_unnamed();
            self.broadcast_to_chunk(
                chunk_pos,
                &CBlockEntityData::new(
...

        {
            ...
            if !custom_data.is_empty() {
                let custom_data_path = self
                    ...
                    .root_folder
                    .join("haskmc_custom_data.nbt");
                let nbt = haskmc_nbt::Nbt::from(custom_data.clone());
    ...
    pub fn set_custom_data(&self, namespace: &str, key: &str, value: haskmc_nbt::tag::NbtTag) {
        ...
        let mut namespace_data = custom_data
            ...
            .and_then(|tag| match tag {
                haskmc_nbt::tag::NbtTag::Compound(compound) => Some(compound),
                _ => None,
            })
            .unwrap_or_default();
        ...
        custom_data.child_tags.insert(
            namespace.into(),
            haskmc_nbt::tag::NbtTag::Compound(namespace_data),
        );
    }

    pub fn get_custom_data(&self, namespace: &str, key: &str) -> Option<haskmc_nbt::tag::NbtTag> {
        let custom_data = self
            .custom_data
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn remove_custom_data(&self, namespace: &str, key: &str) {
        ...
        let Some(haskmc_nbt::tag::NbtTag::Compound(mut namespace_data)) =
...
mod tests {
    ...
    fn game_rules_registry() {
        use haskmc_data::game_rules::{GameRule, GameRuleRegistry, GameRuleValue};
