...
use haskmc_data::block_properties::HorizontalAxis;
        ...
        match self {
            ...
            Self::Nether => {
                ...
                match entity_type.id {
                    id if id == haskmc_data::entity::EntityType::PLAYER.id => (current_world
                        .get_player_by_id(entity.get_entity().entity_id))
                    .map_or(80, |player| match player.gamemode.load() {
                        haskmc_util::GameMode::Creative => {
                            ...
                            match player.client.as_ref() {
                                crate::net::ClientPlatform::Java(client) => {
                                    if let Ok(data) = client.serialize_packet(
                                        &haskmc_protocol::java::client::play::CGameEvent::new(
                                            haskmc_protocol::java::client::play::GameEvent::WinGame,
                                            1.0,
                                        ),
                                    ) {
                                ...
                                crate::net::ClientPlatform::Bedrock(client) => {
                                    if let Ok(data) = client.serialize_packet(
                                        &haskmc_protocol::bedrock::client::CShowCredits {
