...
use haskmc_data::tag;
use haskmc_data::{Block, BlockState, item::Item};
use haskmc_util::{
    loot_table::{
...
pub struct LootContextParameters {
    ...
    pub position: Option<haskmc_util::math::vector3::Vector3<f64>>,
    pub world_time: u64,
...
fn apply_bonus(
    ...
    let enchantment_level = params.tool.as_ref().map_or(0, |tool| {
        haskmc_data::Enchantment::from_name(enchantment_name)
        ...
        match &self.content {
            ...
            LootFunctionTypes::EnchantedCountIncrease {
                ...
                let level = params.tool.as_ref().map_or(0.0, |tool| {
                    haskmc_data::Enchantment::from_name(enchantment)
            ...
            LootFunctionTypes::CopyState {
                        ...
                        Block::properties(Block::from_state_id(state.id), state.id)
                {
                    ...
                    if !properties_to_copy.is_empty() {
                        for stack in stacks.iter_mut() {
                            if let Some(block_state_comp) = stack.get_data_component_mut::<haskmc_data::data_component_impl::BlockStateImpl>() {
                                    ...
                                    let properties: Vec<(std::borrow::Cow<'static, str>, std::borrow::Cow<'static, str>)> = properties_to_copy
                                        ...
                                        .collect();
                                    stack.patch.push((
                                        haskmc_data::data_component::DataComponent::BlockState,
                                        Some(Box::new(haskmc_data::data_component_impl::BlockStateImpl {
            ...
            LootFunctionTypes::SetOminousBottleAmplifier => {
                ...
                for stack in stacks.iter_mut() {
                    if let Some(amplifier_comp) = stack.get_data_component_mut::<haskmc_data::data_component_impl::OminousBottleAmplifierImpl>() {
                        amplifier_comp.amplifier = amplifier;
                    } else {
                        stack.patch.push((
                            haskmc_data::data_component::DataComponent::OminousBottleAmplifier,
                            Some(Box::new(haskmc_data::data_component_impl::OminousBottleAmplifierImpl {
                                amplifier,
                            })),
                        ));
                    }
            ...
            LootFunctionTypes::SetPotion { id } => {
                ...
                if let Some(potion) = haskmc_data::potion::Potion::from_name(name) {
                    let potion_id = Some(potion.id as i32);
                    for stack in stacks.iter_mut() {
                        if let Some(potion_contents) = stack.get_data_component_mut::<haskmc_data::data_component_impl::PotionContentsImpl>() {
                            potion_contents.potion_id = potion_id;
                        } else {
                            stack.patch.push((
                                haskmc_data::data_component::DataComponent::PotionContents,
                                Some(Box::new(haskmc_data::data_component_impl::PotionContentsImpl {
                                    potion_id,
                                    custom_color: None,
            ...
            LootFunctionTypes::FurnaceSmelt => {
                for stack in stacks.iter_mut() {
                    for recipe_type in haskmc_data::recipes::RECIPES_COOKING {
                        if let haskmc_data::recipes::CookingRecipeType::Smelting(recipe) =
                            recipe_type
    ...
    fn get_stacks(&self, params: &LootContextParameters) -> Vec<ItemStack> {
        match self {
            ...
            Self::LootTable(entry) => {
                ...
                haskmc_data::chest_loot_table::get_chest_loot_table(&format!("minecraft:{key}"))
            ...
            Self::Tag(tag) => {
                ...
                let items = haskmc_data::tag::get_tag_values(tag::RegistryKey::Item, key)
                    .unwrap_or_default()
                    .iter()
        ...
        match self {
                    ...
                    if let Some(tag_name) = mainhand_enchantment_tag {
                        ...
                        let has_enchant = params.tool.as_ref().is_some_and(|tool| {
                            haskmc_data::tag::get_tag_ids(
                                haskmc_data::tag::RegistryKey::Enchantment,
                                tag,
                            )
                            .is_some_and(|tag_ids| {
                                tag_ids.iter().any(|&ench_id| {
                                    haskmc_data::Enchantment::from_id(ench_id as u8)
            ...
            Self::MatchTool { items } => params.tool.as_ref().is_some_and(|tool| {
                items.as_ref().map_or_else(
                    || {
                        haskmc_data::Enchantment::from_name("minecraft:silk_touch")
            ...
            Self::EnchantmentActiveCheck { active } => {
                params.tool.as_ref().map_or(!*active, |tool| {
                    let has_enchantments = tool
                        .get_data_component::<haskmc_data::data_component_impl::EnchantmentsImpl>()
...
pub fn generate_chest_loot(
    table: &haskmc_util::chest_loot_table::ChestLootTable,
    seed: i64,
) -> Vec<ItemStack> {
    use haskmc_util::random::RandomImpl;
...
pub fn fill_chest_inventory(
    inventory: &std::sync::Arc<dyn haskmc_world::inventory::Inventory>,
    table: &haskmc_util::chest_loot_table::ChestLootTable,
    seed: i64,
) {
