...
use haskmc_protocol::bedrock::server::login::ClientData;
use haskmc_util::version::BedrockMinecraftVersion;
...
impl BedrockClient {
    ...
    pub fn nethernet_public_key(&self) -> Option<&haskmc_util::p384::PublicKey> {
    ...
    pub async fn kick_explicit(
        ...
        if send_packet {
            self.send_packet(&CDisconnect {
                reason: haskmc_protocol::codec::var_int::VarInt(reason as i32),
                skip_message,
                message,
                filtered_message,
            })
    ...
    pub async fn send_chunks(&self, chunks: &[SyncChunk]) {
        ...
        let bedrock_dimension =
            if player.world().dimension == haskmc_data::dimension::Dimension::THE_NETHER {
                1
            } else if player.world().dimension == haskmc_data::dimension::Dimension::THE_END {
                2
            } else {
                0
            };
    ...
    pub fn handle_play_packet(
        ...
        match packet.id {
            ...
            haskmc_protocol::bedrock::server::item_stack_request::SItemStackRequest::PACKET_ID => {
                let packet = haskmc_protocol::bedrock::server::item_stack_request::SItemStackRequest::read(reader)?;
            ...
            haskmc_protocol::bedrock::server::modal_form_response::SModalFormResponse::PACKET_ID => {
                let form_resp = haskmc_protocol::bedrock::server::modal_form_response::SModalFormResponse::read(
                    reader,
                )?;
