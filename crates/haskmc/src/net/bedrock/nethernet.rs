...
use haskmc_util::jwt::Jwks;
use haskmc_util::p384::{
    PublicKey,
    ecdsa::{
...
fn verify_and_strip_identity(
    ...
    if let Some((issuer, keys)) = oidc_verifier {
        ...
        haskmc_util::jwt::verify_oidc_token(token, issuer, keys)
            .map_err(|error| format!("invalid GameServerToken: {error}"))?;
    } else {
        validate_token_expiration(token)?;
    }
    let public_key = haskmc_util::jwt::extract_cpk_from_token(token)
...
fn verify_fingerprint_assertion(
    ...
    let verifying_key = haskmc_util::p384::ecdsa::VerifyingKey::from(public_key);
    verifying_key
