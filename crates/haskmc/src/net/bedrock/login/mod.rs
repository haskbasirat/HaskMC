...
async fn verify_oidc_token_path(
    ...
    token: &str,
    self_signed: bool,
) -> Result<haskmc_util::jwt::PlayerClaims, LoginError> {
    let token = token.to_string();
    let (tx, rx) = tokio::sync::oneshot::channel();
    if self_signed {
        rayon::spawn(move || {
            let res = haskmc_util::jwt::verify_oidc_token_self_signed(&token)
        ...
        rayon::spawn(move || {
            let res = haskmc_util::jwt::verify_oidc_token(&token, &issuer, &jwks)
                .map_err(LoginError::ChainValidationFailed);
            let _ = tx.send(res);
        });
    }
