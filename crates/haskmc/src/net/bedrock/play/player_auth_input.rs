...
use super::*;
use haskmc_data::entity::EntityPose;
...
impl BedrockClient {
    ...
    pub fn handle_player_auth_input(
        ...
        if pos_changed || rot_changed {
            ...
            let delta = haskmc_util::math::vector3::Vector3::new(
                new_pos.x - old_pos.x,
                new_pos.y - old_pos.y,
                new_pos.z - old_pos.z,
            );

            let bedrock_move_packet = haskmc_protocol::bedrock::client::CMovePlayer::new(
                haskmc_protocol::codec::var_ulong::VarULong(player.entity_id() as u64),
                ...
                new_yaw,
                new_yaw, // Head yaw
                haskmc_protocol::bedrock::client::CMovePlayer::MODE_NORMAL,
                on_ground,
                haskmc_protocol::codec::var_ulong::VarULong(0),
                0,
                0,
                haskmc_protocol::codec::var_ulong::VarULong(0),
            );
            ...
            if pos_changed && delta.length_squared() >= 64.0 {
                world.broadcast_packet_except(
                    ...
                    &haskmc_protocol::java::client::play::CEntityPositionSync::new(
                        player.entity_id().into(),
                        new_pos,
                        haskmc_util::math::vector3::Vector3::new(0.0, 0.0, 0.0),
                        je_yaw,
                        je_pitch,
                        on_ground,
                    ),
                ...
                world.broadcast_packet_except_editioned(
                    &[player.gameprofile.id],
                    &haskmc_protocol::java::client::play::CUpdateEntityPosRot::new(
                        player.entity_id().into(),
                        haskmc_util::math::vector3::Vector3::new(
                ...
                world.broadcast_packet_except_editioned(
                    &[player.gameprofile.id],
                    &haskmc_protocol::java::client::play::CUpdateEntityPos::new(
                        player.entity_id().into(),
                        haskmc_util::math::vector3::Vector3::new(
                            new_pos.x.mul_add(4096.0, -(old_pos.x * 4096.0)) as i16,
                            new_pos.y.mul_add(4096.0, -(old_pos.y * 4096.0)) as i16,
                ...
                world.broadcast_packet_except_editioned(
                    &[player.gameprofile.id],
                    &haskmc_protocol::java::client::play::CUpdateEntityRot::new(
                        player.entity_id().into(),
                        je_yaw as u8,   // Use converted Java byte
                        je_pitch as u8, // Use converted Java byte
                        on_ground,
            ...
            if rot_changed {
                world.broadcast_packet_except(
                    ...
                    &haskmc_protocol::java::client::play::CHeadRot::new(
                        player.entity_id().into(),
                        je_yaw as u8,
                    ),
                );
