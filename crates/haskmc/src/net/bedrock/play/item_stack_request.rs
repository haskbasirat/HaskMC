...
impl BedrockClient {
    ...
    pub fn handle_item_stack_request(
        &self,
        player: &Arc<Player>,
        packet: haskmc_protocol::bedrock::server::item_stack_request::SItemStackRequest,
    ) {
        use haskmc_protocol::bedrock::client::item_stack_response::{
            CItemStackResponse, ItemStackResponseContainerInfo, ItemStackResponseInfo,
            ItemStackResponseSlotInfo,
        };
        use haskmc_protocol::bedrock::server::item_stack_request::ItemStackRequestAction;
                ...
                for action in request.actions {
                    match action {
                        ItemStackRequestAction::CraftCreative {
                            ...
                            if index < haskmc_data::bedrock_creative::CREATIVE_ENTRIES.len() {
                                let entry = haskmc_data::bedrock_creative::CREATIVE_ENTRIES[index];
                                if let Some(mapping) =
                                    haskmc_data::item::JavaToBedrockItemMapping::from_bedrock(
                                        entry.item_id,
                                ...
                                tracing::warn!(
                                    ...
                                    index,
                                    haskmc_data::bedrock_creative::CREATIVE_ENTRIES.len()
                                );
                                result = 1;
                                break;
                            }
                        ...
                        | ItemStackRequestAction::Place {
                            ...
                            if count > 0 {
                                ...
                                let merchant_result = screen_handler.window_type()
                                    == Some(haskmc_data::screen::WindowType::Merchant)
                                    && matches!(
                                ...
                                if merchant_result {
                                    ...
                                    .as_any_mut()
                                    .downcast_mut::<haskmc_inventory::merchant::merchant_screen_handler::MerchantScreenHandler>()
                                else {
                                    result = 1;
                                    break;
                                };
    ...
    match container_name {
        ...
        ContainerName::CraftingInput => {
            if is_player_screen {
                ...
                == Some(haskmc_data::screen::WindowType::Crafting)
            {
                if slot_id < 9 {
                    Some(1 + slot_id as usize)
        ...
        ContainerName::CraftingOutputPreview | ContainerName::CreatedOutput => {
            if is_player_screen {
                ...
                match window_type {
                    haskmc_data::screen::WindowType::Crafting => Some(0),
                    haskmc_data::screen::WindowType::Stonecutter => Some(1),
                    haskmc_data::screen::WindowType::Anvil
                    | haskmc_data::screen::WindowType::Furnace
                    | haskmc_data::screen::WindowType::BlastFurnace
                    | haskmc_data::screen::WindowType::Smoker
                    | haskmc_data::screen::WindowType::Grindstone
                    | haskmc_data::screen::WindowType::Merchant => Some(2),
                    haskmc_data::screen::WindowType::Loom
                    | haskmc_data::screen::WindowType::Smithing => Some(3),
                    _ => None,
                }
            } else {
                None
        ...
        ContainerName::AnvilInput => matches!(
            ...
            Some(haskmc_data::screen::WindowType::Anvil)
        )
        .then_some(0),
        ...
        ContainerName::BeaconPayment => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Beacon)
        )
        .then_some(0),
        ContainerName::BrewingStandResult => (matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::BrewingStand)
        ) && slot_id < 3)
        ...
        ContainerName::EnchantingInput => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Enchantment)
        )
        .then_some(0),
        ...
        ContainerName::GrindstoneInput => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Grindstone)
        )
        .then_some(0),
        ...
        ContainerName::LoomInput => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Loom)
        )
        .then_some(0),
        ...
        ContainerName::StonecutterInput => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Stonecutter)
        )
        .then_some(0),
        ...
        ContainerName::CartographyInput => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::CartographyTable)
        )
        .then_some(0),
        ...
        ContainerName::SmithingTableTemplate => matches!(
            screen_handler.window_type(),
            Some(haskmc_data::screen::WindowType::Smithing)
        )
        .then_some(0),
...
fn get_slot_stack(
    screen_handler: &dyn ScreenHandler,
    slot_info: &haskmc_protocol::bedrock::server::item_stack_request::ItemStackRequestSlotInfo,
