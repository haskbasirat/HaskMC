...
impl BedrockClient {
    ...
    pub fn handle_inventory_action(&self, player: &Arc<Player>, packet: SInventoryTransaction) {
        ...
        for action in &packet.actions {
            use haskmc_protocol::bedrock::server::inventory_transaction::InventoryActionSource;
            ...
            TransactionData::UseItem(data) => {
                ...
                if data.action_type.0 == 0 {
                    ...
                    if !stack.is_empty() {
                        ...
                        if let Some(placed_block) = haskmc_data::Block::from_item_id(item_id) {
                            let dummy_use_item_on =
                                haskmc_protocol::java::server::play::SUseItemOn {
                                    hand: VarInt(0),
