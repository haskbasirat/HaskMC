...
use haskmc_config::{AuthenticationConfig, networking::auth::TextureConfig};
...
/// 3. `HaskMC` attempts to authenticate the player against the primary auth server, falling back to secondary auth servers if the primary is down.
///
...
mod tests {
    ...
    fn auth_config_fallbacks_deserialization() {
        ...
        let config: haskmc_config::AuthenticationConfig =
