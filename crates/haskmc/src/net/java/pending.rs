...
use haskmc_config::networking::compression::CompressionInfo;
...
use haskmc_util::{Hand, text::TextComponent, version::JavaMinecraftVersion};
use tokio::{
...
impl PendingConnection {
    ...
    async fn handle_handshake_packet(
        ...
        match packet.id {
            0 => {
                self.handle_handshake(haskmc_protocol::java::server::handshake::SHandShake::read(
                    &mut payload,
                    &self.version.load(),
                )?)
                .await;
    ...
    async fn handle_status_packet(
        ...
        match packet.id {
            id if id == haskmc_protocol::java::server::status::SStatusRequest::to_id(version) => {
                self.handle_status_request(server).await;
                Ok(None)
            }
            id if id
                == haskmc_protocol::java::server::status::SStatusPingRequest::to_id(version) =>
            {
                self.handle_ping_request(
                    haskmc_protocol::java::server::status::SStatusPingRequest::read(
                        &mut payload,
                        &version,
                    )?,
                )
    ...
    async fn handle_login_packet(
        ...
        match packet.id {
            id if id == haskmc_protocol::java::server::login::SLoginStart::to_id(version) => {
                Ok(self
                    .handle_login_start(
                        server,
                        haskmc_protocol::java::server::login::SLoginStart::read(
                            &mut payload,
                            &version,
                        )?,
                    )
            ...
            id if id
                == haskmc_protocol::java::server::login::SEncryptionResponse::to_id(version) =>
            {
                Ok(self
                    .handle_encryption_response(
                        server,
                        haskmc_protocol::java::server::login::SEncryptionResponse::read(
                            &mut payload,
                            &version,
                        )?,
                    )
            ...
            id if id
                == haskmc_protocol::java::server::login::SLoginPluginResponse::to_id(version) =>
            {
                Ok(self
                    .handle_plugin_response(
                        server,
                        haskmc_protocol::java::server::login::SLoginPluginResponse::read(
                            &mut payload,
                            &version,
                        )?,
                    )
            ...
            id if id
                == haskmc_protocol::java::server::login::SLoginCookieResponse::to_id(version) =>
            {
                self.handle_login_cookie_response(
                    &haskmc_protocol::java::server::login::SLoginCookieResponse::read(
                        &mut payload,
                        &version,
                    )?,
                );
            ...
            id if id
                == haskmc_protocol::java::server::login::SLoginAcknowledged::to_id(version) =>
            {
    ...
    pub async fn handle_resource_pack_response(
        ...
        if resource_config.enabled {
            use haskmc_protocol::java::server::config::ResourcePackResponseResult;
