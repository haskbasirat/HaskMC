...
use haskmc_world::level::SyncChunk;
...
use haskmc_protocol::packet::MultiVersionJavaPacket;
use haskmc_protocol::{
...
use crate::{error::HaskMcError, server::Server};
...
impl JavaClient {
        ...
        loop {
            tokio::select! {
                ...
                _ = keep_alive_interval.tick() => {
                    ...
                    if has_timed_out {
                        self.kick(haskmc_macros::translate_cross!(translation::java::DISCONNECT_TIMEOUT, translation::bedrock::DISCONNECT_TIMEOUT)).await;
                        break;
                    }
                    ...
                    let packet = haskmc_protocol::java::client::play::CKeepAlive::new(keep_alive_id);
    ...
    pub fn write_packet_for_version<P: ClientPacket>(
        ...
        haskmc_protocol::java::packet_encoder::write_packet(packet, &version, write)
    }
    ...
    pub fn serialize_packet_for_version<P: ClientPacket>(
        ...
        haskmc_protocol::java::packet_encoder::serialize_packet(packet, &version)
    }
    ...
    pub fn handle_play_packet(
        ...
        match event.packet_id {
            ...
            id if id == haskmc_protocol::java::server::play::SKeepAlive::to_id(version) => {
                self.handle_keep_alive(
                    player,
                    &haskmc_protocol::java::server::play::SKeepAlive::read(&mut payload, &version)?,
                );
            }
            ...
            id if id
                == haskmc_protocol::java::server::play::SPickItemFromEntity::to_id(version) =>
            {
                self.handle_pick_item_from_entity(
                    player,
                    &haskmc_protocol::java::server::play::SPickItemFromEntity::read(
                        &mut payload,
                        &version,
                    )?,
                );
            ...
            id if id == haskmc_protocol::java::server::play::SCustomClickAction::to_id(version) => {
                let packet = haskmc_protocol::java::server::play::SCustomClickAction::read(
                    &mut payload,
                    &version,
                )?;
