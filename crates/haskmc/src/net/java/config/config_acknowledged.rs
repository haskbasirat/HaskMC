...
pub(crate) fn build_dimension_nbt(dim: &haskmc_data::dimension::Dimension) -> Vec<u8> {
    ...
    let mut monster_spawn = haskmc_nbt::compound::NbtCompound::new();
    monster_spawn.put_string("type", "minecraft:uniform".to_string());
    let mut value = haskmc_nbt::compound::NbtCompound::new();
    ...
    haskmc_nbt::Nbt::from(compound).write().to_vec()
}
