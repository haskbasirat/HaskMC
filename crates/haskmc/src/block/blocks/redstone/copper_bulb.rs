...
type CopperBulbLikeProperties = haskmc_data::block_properties::CopperBulbLikeProperties;

pub struct CopperBulbBlock;

impl ChangeOverTimeBlock<WeatherState> for CopperBulbBlock {
    fn get_age(&self, block: &haskmc_data::Block) -> Option<WeatherState> {
    ...
    fn get_next(&self, block: &haskmc_data::Block) -> Option<&'static haskmc_data::Block> {
        get_next(block)
    }

    fn get_previous(&self, block: &haskmc_data::Block) -> Option<&'static haskmc_data::Block> {
        get_previous(block)
    }

    fn get_first(&self, block: &haskmc_data::Block) -> Option<&'static haskmc_data::Block> {
        get_first(block)
    }
}
