...
use haskmc_util::math::{boundingbox::BoundingBox, position::BlockPos};
use haskmc_world::{tick::TickPriority, world::BlockFlags};
...
#[haskmc_block("minecraft:comparator")]
...
impl RedstoneGateBlock<ComparatorLikeProperties> for ComparatorBlock {
    ...
    fn get_power(&self, world: &World, pos: BlockPos, state: &BlockState, block: &Block) -> u8 {
        ...
        if let Some(haskmc_block) = world.block_registry.get_haskmc_block(source_block.id)
            && let Some(level) = haskmc_block.get_comparator_output(GetComparatorOutputArgs {
                world,
                block: source_block,
                state: source_state,
                position: &source_pos,
        ...
        if redstone_level < 15 && source_state.is_solid_block() {
            ...
            let block_level = world
                .block_registry
                .get_haskmc_block(deeper_block.id)
                .and_then(|haskmc_block| {
                    haskmc_block.get_comparator_output(GetComparatorOutputArgs {
                        world,
                        block: deeper_block,
                        state: deeper_state,
