use haskmc_data::data_component_impl::IDSetContent;
        ...
        for level in 1..=4 {
            ...
            for dx in -level..=level {
                for dz in -level..=level {
                    ...
                    if !block.has_tag(&haskmc_data::tag::Block::MINECRAFT_BEACON_BASE_BLOCKS) {
                        layer_valid = false;
                        break;
                    }
    ...
    pub fn apply_effects(&self, world: &Arc<World>, levels: i32) {
        ...
        for player in players.iter() {
            ...
            if let Some(effect) = primary_effect {
                player.add_effect(haskmc_data::potion::Effect {
                    effect_type: effect,
