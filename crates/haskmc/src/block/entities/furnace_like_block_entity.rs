use haskmc_data::{item_stack::ItemStack, recipes::CookingRecipe};

use crate::block::entities::{BlockEntity, PropertyDelegate};
pub use haskmc_world::block::entities::ExperienceContainer;
...
macro_rules! impl_cooking_block_entity_base {
    ($struct_name:ty) => {
        impl CookingBlockEntityBase for $struct_name {
            ...
            fn add_recipe_used(&self, recipe: &haskmc_data::recipes::CookingRecipe) {
            ...
            fn extract_experience_from_recipes(&self) -> i32 {
                ...
                for (recipe_id, count) in recipes.iter() {
                    ...
                    if let Some(xp) = haskmc_data::recipes::get_recipe_experience(recipe_id) {
            ...
            fn can_accept_recipe_output(
                &self,
                recipe: Option<&haskmc_data::recipes::CookingRecipe>,
                max_count: u8,
            ) -> bool {
                ...
                if let Some(recipe_output_item) = haskmc_data::item::Item::from_registry_key(
                    recipe
                        .result
                        .id
            ...
            fn craft_recipe(&self, recipe: Option<&haskmc_data::recipes::CookingRecipe>) -> bool {
                ...
                if let Some(recipe) = recipe {
                    if can_accept_output {
                        ...
                        let Some(output_item) = haskmc_data::item::Item::from_registry_key(
                            recipe
                                .result
                                .id
                                .strip_prefix("minecraft:")
                        ...
                        if items[0].item.id == haskmc_data::item::Item::WET_SPONGE.id
                            && !items[1].is_empty()
                            && items[1].item.id == haskmc_data::item::Item::BUCKET.id
                        {
                            items[1] = ItemStack::new(1, &haskmc_data::item::Item::WATER_BUCKET);
                        }
...
macro_rules! impl_clearable_for_cooking {
    ($struct_name:ty) => {
        impl haskmc_world::inventory::Clearable for $struct_name {
            fn clear(&self) {
                let mut items = self
                    .items
                    .write()
...
macro_rules! impl_inventory_for_cooking {
    ($struct_name:ty) => {
        impl haskmc_world::inventory::Inventory for $struct_name {
            fn size(&self) -> usize {
                Self::INVENTORY_SIZE
            }
            ...
            fn set_stack(&self, slot: usize, stack: ItemStack) {
                ...
                if slot == 0 && !is_same_item {
                    if let Some(recipe) = haskmc_data::recipes::get_cooking_recipe_with_ingredient(
                        stack.item,
    ...
    ($struct_name:ty,$recipe_kind:expr) => {
        impl $crate::block::entities::BlockEntity for $struct_name {
            ...
            fn tick(
                ...
                let furnace_recipe = haskmc_data::recipes::get_cooking_recipe_with_ingredient(
                    top_item.item,
                    $recipe_kind,
                );
                ...
                if self.is_burning() || !bottom_items_is_empty && !is_top_items_empty {
                    if !self.is_burning() && can_accept_output {
                        let base_fuel_ticks =
                            haskmc_data::fuels::get_item_burn_ticks(bottom_item.item.id)
                                .unwrap_or(0);
                            ...
                            if let Ok(mut items_guard) = self.items.try_write() {
                                if !items_guard[1].is_empty() {
                                    ...
                                    if let Some(remainder_id) =
                                        haskmc_data::recipe_remainder::get_recipe_remainder_id(
                                            items_guard[1].item.id,
                                        )
                                        && items_guard[1].is_empty()
                                        && let Some(remainder_item) =
                                            haskmc_data::item::Item::from_id(remainder_id)
                                    {
                ...
                if is_burning != self.is_burning() {
                    ...
                    let mut props =
                        haskmc_data::block_properties::FurnaceLikeProperties::from_state_id(
            ...
            fn from_nbt(nbt: &haskmc_nbt::compound::NbtCompound, position: BlockPos) -> Self
            where
                Self: Sized,
            {
                ...
                if let Some(recipes_compound) = nbt.get_compound("RecipesUsed") {
                    for (recipe_id, tag) in &recipes_compound.child_tags {
                        if let haskmc_nbt::tag::NbtTag::Int(count) = tag {
                ...
                haskmc_world::inventory::sync_read_items_from_nbt(nbt, furnace.items.get_mut().unwrap_or_else(std::sync::PoisonError::into_inner));

                furnace
            }

            fn write_nbt(&self, nbt: &mut haskmc_nbt::compound::NbtCompound) {
                ...
                // Save RecipesUsed in vanilla format (map of recipe ID -> craft count)
                {
                    let recipes = self
                        ...
                        .unwrap_or_else(std::sync::PoisonError::into_inner);
                    if !recipes.is_empty() {
                        let mut recipes_compound = haskmc_nbt::compound::NbtCompound::new();
                        for (recipe_id, count) in recipes.iter() {
                            recipes_compound.put(
                                recipe_id.as_str(),
                                haskmc_nbt::tag::NbtTag::Int(*count as i32),
                            );
                        }
                        nbt.put(
                            "RecipesUsed",
                            haskmc_nbt::tag::NbtTag::Compound(recipes_compound),
                        );
                    }
                }
            ...
            fn get_inventory(
                self: Arc<Self>,
            ) -> Option<Arc<dyn haskmc_world::inventory::Inventory>> {
                Some(self)
            }
            ...
            fn chunk_data_nbt(&self) -> Option<haskmc_nbt::compound::NbtCompound> {
                let mut nbt = haskmc_nbt::compound::NbtCompound::new();
                nbt.put_short("cooking_total_time", self.get_cooking_total_time() as i16);
                nbt.put_short("cooking_time_spent", self.get_cooking_time_spent() as i16);
                nbt.put_short("lit_total_time", self.get_lit_total_time() as i16);
                nbt.put_short("lit_time_remaining", self.get_lit_time_remaining() as i16);
                ...
                if let Ok(guard) = self.items.try_read() {
                    haskmc_world::inventory::sync_write_items_to_nbt(&*guard, &mut nbt);
                }
                Some(nbt)
            }
