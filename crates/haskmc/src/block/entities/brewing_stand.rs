...
use haskmc_data::potion_brewing::{ITEM_RECIPES, POTION_RECIPES};
...
use haskmc_world::inventory::{Inventory, sync_read_items_from_nbt, sync_write_items_to_nbt};
...
pub struct BrewingStandBlockEntity {
    ...
    pub ingredient_item: StdMutex<Option<&'static haskmc_data::item::Item>>,
}
...
impl BrewingStandBlockEntity {
    ...
    fn is_brewable(&self, ingredient: &ItemStack) -> bool {
        ...
        for slot_idx in 0..3usize {
            ...
            if let Some(pc) =
                slot.get_data_component::<haskmc_data::data_component_impl::PotionContentsImpl>()
            ...
            for slot_idx in 0..3usize {
                ...
                for recipe in &ITEM_RECIPES {
                        ...
                        && recipe.ingredient().iter().any(|i| i.id == ingredient_id)
                    {
                        ...
                        let pc = slot.get_data_component::<haskmc_data::data_component_impl::PotionContentsImpl>().cloned();
...
impl haskmc_world::inventory::Inventory for BrewingStandBlockEntity {
    fn size(&self) -> usize {
        Self::INVENTORY_SIZE
    }
...
impl haskmc_world::inventory::Clearable for BrewingStandBlockEntity {
    fn clear(&self) {
        let mut items = self
            .items
            .write()
...
impl crate::block::entities::BlockEntity for BrewingStandBlockEntity {
        ...
        for (i, slot) in items.iter().take(3).enumerate() {
            current[i] = !slot.is_empty()
                && (slot
                    .get_data_component::<haskmc_data::data_component_impl::PotionContentsImpl>()
                    .is_some()
                    || slot.get_item().id == haskmc_data::item::Item::GLASS_BOTTLE.id);
        }
    ...
    fn tick(&self, world: &Arc<crate::world::World>) {
            ...
            for (i, slot) in items_guard.iter().take(3).enumerate() {
                ...
                current[i] = !slot.is_empty()
                    && (slot
                        ...
                        .is_some()
                        || slot.get_item().id == Item::GLASS_BOTTLE.id);
            }
        }
        ...
        if needs_update {
            ...
            let mut props =
                haskmc_data::block_properties::BrewingStandLikeProperties::from_state_id(
                    state.id, block,
                );
