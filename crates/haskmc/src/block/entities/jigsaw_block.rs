...
use haskmc_data::block_properties::{BlockProperties, JigsawLikeProperties, Orientation};
use haskmc_nbt::compound::NbtCompound;
use haskmc_util::{
    math::position::BlockPos,
    random::{RandomGenerator, xoroshiro128::Xoroshiro},
};
use haskmc_world::generation::structure::structures::{
...
impl JigsawBlockEntity {
    ...
    pub fn generate(&self, world: &Arc<World>, levels: i32, keep_jigsaws: bool) {
        ...
        let props =
            JigsawLikeProperties::from_state_id(block_state.id, &haskmc_data::Block::JIGSAW);
    ...
    fn place_structure(world: &Arc<World>, structure: &StructurePosition, keep_jigsaws: bool) {
        ...
        for piece in &mut pieces {
            if let Some(pool_piece) = piece.as_any().downcast_ref::<PoolElementStructurePiece>() {
                haskmc_world::generation::structure::structures::jigsaw::place_pool_element_templates(
                    pool_piece,
                    &mut placer,
                    None,
                    keep_jigsaws,
