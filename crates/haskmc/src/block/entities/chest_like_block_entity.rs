        ...
        impl $crate::block::entities::BlockEntity for $struct_name {
                ...
                Self: Sized,
            {
                ...
                let has_loot_table = chest
                    ...
                    .is_some();
                if !has_loot_table {
                    haskmc_world::inventory::sync_read_items_from_nbt(
                        nbt,
                        chest
                            .items
                            .get_mut()
            ...
            fn chunk_data_nbt(&self) -> Option<haskmc_nbt::compound::NbtCompound> {
                ...
                if !has_loot_table {
                    if let Ok(items) = self.items.try_read() {
                        haskmc_world::inventory::sync_write_items_to_nbt(&*items, &mut nbt);
                    }
                }
                Some(nbt)
            }
    ...
    ($struct_name:ty) => {
        impl haskmc_world::inventory::Clearable for $struct_name {
            fn clear(&self) {
                ...
                <$struct_name as haskmc_world::inventory::Inventory>::mark_dirty(self);
            }
        }
    };
}
    ...
    ($struct_name:ty) => {
        impl $crate::block::viewer::ViewerCountListener for $struct_name {
            fn on_container_open(
                ...
                _position: &haskmc_util::math::position::BlockPos,
            ) {
                self.play_sound(world, haskmc_data::sound::Sound::BlockChestOpen);
            }
            ...
            fn on_container_close(
                ...
                self.play_sound(world, haskmc_data::sound::Sound::BlockChestClose);
            }

            fn on_viewer_count_update(
                &self,
                world: &Arc<$crate::world::World>,
                position: &haskmc_util::math::position::BlockPos,
                old: u16,
                new: u16,
            ) {
...
macro_rules! impl_chest_helper_methods {
    ($struct_name:ty) => {
        impl $struct_name {
            ...
            #[must_use]
            pub fn new(position: haskmc_util::math::position::BlockPos) -> Self {
            ...
            fn play_sound(
                &self,
                world: &Arc<$crate::world::World>,
                sound: haskmc_data::sound::Sound,
            ) {
                let mut rng = haskmc_util::random::xoroshiro128::Xoroshiro::from_seed(
                    haskmc_util::random::get_seed(),
                );

                let (block, state) = world.get_block_and_state(&self.position);
                let properties = haskmc_data::block_properties::ChestLikeProperties::from_state_id(
                    state.id, block,
                );
                let position = match properties.r#type {
                    haskmc_data::block_properties::ChestType::Left => return,
                    haskmc_data::block_properties::ChestType::Single => {
                    ...
                    haskmc_data::block_properties::ChestType::Right => {
                        let direction = haskmc_data::HorizontalFacingExt::to_block_direction(
                            &properties.facing,
                        )
                        .to_offset();
                        haskmc_util::math::vector3::Vector3::new(
                ...
                world.play_sound_fine(
                    ...
                    &position,
                    0.5,
                    haskmc_util::random::RandomImpl::next_f32(&mut rng) * 0.1 + 0.9,
                );
                let bedrock_sound = match sound {
                    haskmc_data::sound::Sound::BlockChestOpen => "chest.open",
                    haskmc_data::sound::Sound::BlockChestClose => "chest.closed",
                    _ => return,
                };
