...
use haskmc_data::block_properties::{BlockProperties, FacingHopper, HopperLikeProperties};
...
use haskmc_world::inventory::{Clearable, Inventory, sync_write_items_to_nbt};
use std::any::Any;
...
impl HopperBlockEntity {
    ...
    fn suck_in_items(&self, world: &Arc<World>) -> bool {
        ...
        if !(state.is_solid() && block.has_tag(&tag::Block::MINECRAFT_DOES_NOT_BLOCK_HOPPERS)) {
            ...
            let search_box = haskmc_util::math::boundingbox::BoundingBox::new(
                pos_up_f,
