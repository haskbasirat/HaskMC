...
use haskmc_data::{Block, BlockDirection, BlockId, BlockState};
use haskmc_protocol::java::server::play::SUseItemOn;
use haskmc_util::math::boundingbox::BoundingBox;
use haskmc_util::math::position::BlockPos;
use haskmc_world::world::{BlockAccessor, BlockFlags};
...
impl BlockRegistry {
    pub fn bone_meal(
        ...
        let Some(behaviour) = self.get_haskmc_block(block.id) else {
            return false;
        };
    ...
    fn entity_blocks_block_placement(entity: &dyn EntityBase) -> bool {
        ...
        entity_type == &haskmc_data::entity::EntityType::END_CRYSTAL
            || entity_type == &haskmc_data::entity::EntityType::FALLING_BLOCK
            || entity_type == &haskmc_data::entity::EntityType::TNT
    ...
    pub fn place_block(
        ...
        match player.gamemode.load() {
            haskmc_util::GameMode::Spectator | haskmc_util::GameMode::Adventure => {
        ...
        if location.0.y + face.to_offset().y > world.get_top_y() {
            player.send_system_message_raw(
                &haskmc_util::text::TextComponent::translate_cross(
                    haskmc_data::translation::java::BUILD_TOOHIGH,
                    haskmc_data::translation::bedrock::BUILD_TOOHIGH,
                    vec![haskmc_util::text::TextComponent::text(
                        (world.get_top_y()).to_string(),
                    )],
                )
                .color_named(haskmc_util::text::color::NamedColor::Red),
                true,
            );
            return Err(BlockPlacingError::BlockOutOfWorld);
        }
        ...
        let replace_clicked_block = if clicked_block == placed_block {
            ...
            if clicked_block == &Block::WATER {
                use haskmc_data::block_properties::{BlockProperties, WaterLikeProperties};
                let water_props =
        ...
        let (final_block_pos, final_face, replacing) = if let Some(replacing) =
            replace_clicked_block
        {
    ...
    pub fn on_synced_block_event(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.on_synced_block_event(OnSyncedBlockEventArgs {
                world,
                block,
                position,
                r#type,
    ...
    pub fn on_entity_collision(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.on_entity_collision(OnEntityCollisionArgs {
                server,
                world,
                block,
                state,
    ...
    pub fn on_entity_step(
        ...
        if let Some(haskmc_block) = self.get_haskmc_block(block.id) {
            haskmc_block.on_entity_step(OnEntityStepArgs {
                world,
                block,
                state,
                position,
    ...
    pub fn on_entity_collision_fluid(&self, fluid: &Fluid, entity: &dyn EntityBase) {
        let haskmc_fluid = self.get_haskmc_fluid(fluid.id);
        if let Some(haskmc_fluid) = haskmc_fluid {
            haskmc_fluid.on_entity_collision(entity);
        }
    }
    ...
    pub fn on_use(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.normal_use(NormalUseArgs {
                server,
                world,
                block,
                position,
    ...
    pub fn explode(&self, block: &Block, world: &Arc<World>, position: &BlockPos) {
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.explode(ExplodeArgs {
                world,
                block,
                position,
            });
    ...
    pub fn use_with_item(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.use_with_item(UseWithItemArgs {
                server,
                world,
                block,
                position,
    ...
    pub fn use_with_item_fluid(
        ...
        let haskmc_fluid = self.get_haskmc_fluid(fluid.id);
        if let Some(haskmc_fluid) = haskmc_fluid {
            return haskmc_fluid.use_with_item(fluid, player, position, item, server, world);
        }
        BlockActionResult::Pass
    }
    ...
    pub fn can_place_at(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.can_place_at(CanPlaceAtArgs {
                server,
                world,
                block_accessor,
                block,
    ...
    pub fn can_update_at(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.can_update_at(CanUpdateAtArgs {
                world,
                block,
                state_id,
                position,
    ...
    pub fn on_place(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.on_place(OnPlaceArgs {
                server,
                world,
                block,
                position,
    ...
    pub fn player_placed(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.player_placed(PlayerPlacedArgs {
                world,
                block,
                state_id,
                position,
    ...
    pub fn on_placed(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.placed(PlacedArgs {
                world,
                block,
                state_id,
                old_state_id,
    ...
    pub fn on_placed_fluid(
        ...
        let haskmc_fluid = self.get_haskmc_fluid(fluid.id);
        if let Some(haskmc_fluid) = haskmc_fluid {
            haskmc_fluid.placed(world, fluid, state_id, position, old_state_id, notify);
        }
    }
    ...
    pub fn on_landed_upon(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.on_landed_upon(OnLandedUponArgs {
                world,
                fall_distance,
                entity,
            });
    ...
    pub fn update_entity_movement_after_fall_on(&self, block: &Block, entity: &dyn EntityBase) {
        if let Some(haskmc_block) = self.get_haskmc_block(block.id) {
            haskmc_block.update_entity_movement_after_fall_on(
    ...
    pub fn broken(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.broken(BrokenArgs {
                block,
                player,
                position,
                server,
    ...
    pub fn on_state_replaced(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.on_state_replaced(OnStateReplacedArgs {
                world,
                block,
                old_state_id,
                position,
    ...
    pub fn post_process_state(
        ...
        for direction in BlockDirection::all() {
            ...
            if let Some(haskmc_block) = haskmc_block {
                let new_state =
                    haskmc_block.get_state_for_neighbor_update(GetStateForNeighborUpdateArgs {
                        world,
                        block,
                        state_id,
                        position,
    ...
    pub fn prepare(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.prepare(PrepareArgs {
                world,
                block,
                state_id,
                position,
    ...
    pub fn get_state_for_neighbor_update(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.get_state_for_neighbor_update(GetStateForNeighborUpdateArgs {
                world,
                block,
                state_id,
                position,
    ...
    pub fn on_neighbor_update(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            haskmc_block.on_neighbor_update(OnNeighborUpdateArgs {
                world,
                block,
                position,
                source_block,
    ...
    #[must_use]
    pub fn get_haskmc_block(&self, block: BlockId) -> Option<&Arc<dyn BlockBehaviour>> {
        self.blocks.get(&block)
    }

    #[must_use]
    pub fn get_haskmc_fluid(&self, fluid_id: u16) -> Option<&Arc<dyn FluidBehaviour>> {
    ...
    pub fn emits_redstone_power(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.emits_redstone_power(EmitsRedstonePowerArgs {
                block,
                state,
                direction,
            });
    ...
    pub fn get_weak_redstone_power(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.get_weak_redstone_power(GetRedstonePowerArgs {
                world,
                block,
                state,
                position,
    ...
    pub fn get_strong_redstone_power(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.get_strong_redstone_power(GetRedstonePowerArgs {
                world,
                block,
                state,
                position,
    ...
    pub fn get_inside_collision_shape(
        ...
        let haskmc_block = self.get_haskmc_block(block.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.get_inside_collision_shape(GetInsideCollisionShapeArgs {
                world,
                block,
                state,
                position,
    ...
    pub fn mirror(
        ...
        self.get_haskmc_block(block.id).map_or_else(
            || block.mirror(state_id, mirror),
            |haskmc_block| haskmc_block.mirror(block, state_id, mirror),
        )
    }
    ...
    pub fn rotate(
        ...
        self.get_haskmc_block(block.id).map_or_else(
            ...
            |haskmc_block| haskmc_block.rotate(block, state_id, rotation),
        )
    }
}
