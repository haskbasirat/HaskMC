...
use haskmc_config::{AdvancedConfiguration, BasicConfiguration};
use haskmc_data::dimension::Dimension;
use haskmc_data::entity::EntityType;
use haskmc_util::permission::PermissionManager;
use haskmc_util::text::color::NamedColor;
use haskmc_world::dimension::into_level;
use haskmc_world::generation::generator::GeneratorInit;
use haskmc_world::world::WorldPortalExt;
use key_store::KeyStore;
use tracing::{debug, error, info, warn};

use crate::command::CommandSender;
use haskmc_protocol::java::client::login::CEncryptionRequest;
use haskmc_protocol::java::client::play::{CChangeDifficulty, CTabList};
use haskmc_protocol::{ClientPacket, java::client::config::CPluginMessage};
use haskmc_util::Difficulty;
use haskmc_util::text::TextComponent;
use haskmc_world::world_info::anvil::{
    AnvilLevelInfo, LEVEL_DAT_BACKUP_FILE_NAME, LEVEL_DAT_FILE_NAME,
};
use haskmc_world::world_info::{LevelData, WorldInfoError, WorldInfoReader, WorldInfoWriter};
...
pub struct Server {
    ...
    pub bedrock_oidc_keys: Arc<OnceCell<(String, haskmc_util::jwt::Jwks)>>,
    /// Cached Bedrock server private key (process-lifetime). Generated on first Bedrock login and reused.
    pub bedrock_private_key: OnceCell<Arc<haskmc_util::p384::ecdsa::SigningKey>>,
...
impl Server {
    ...
    pub async fn new(
        ...
        let level_info = match AnvilLevelInfo.read_world_info(&world_path) {
            ...
            Err(WorldInfoError::InfoNotFound) => {
                ...
                let overworld_gen = haskmc_world::generation::generator::VanillaGenerator::new(
                    basic_config.seed,
                    Dimension::OVERWORLD,
                );
                let default_data =
                ...
                .enabled
        {
            ...
            server.spawn_task(async move {
                let auth = server_clone
                    ...
                    .authentication
                    .clone();
                let keys = match haskmc_util::jwt::fetch_oidc_jwks(
                    auth.url.as_deref(),
                    ...
                    Err(error) => {
                        ...
                        (String::new(), haskmc_util::jwt::Jwks { keys: Vec::new() })
                    }
                };
    ...
    pub fn create_world(self: &Arc<Self>, name: String, dimension: Dimension) -> Arc<World> {
        ...
        let level =
            haskmc_world::dimension::into_level(dimension.clone(), &config, world_path, seed);
    ...
    pub fn reload_datapacks(&self, server: &Arc<Self>) {
        ...
        for player in self.get_all_players() {
            if let crate::net::ClientPlatform::Java(java_client) = player.client.as_ref() {
                let add_packet = haskmc_protocol::java::client::play::CRecipeBookAdd::new(
                    true,
                    &dynamic_recipes,
                );
    ...
    pub fn remove_player(&self, player: &Player) {
        player.increment_stat(
            ...
            haskmc_data::statistic::CustomStatistic::LeaveGame as i32,
            1,
        );
    ...
    pub fn update_tick_times(&self, tick_duration_nanos: i64) {
        ...
        let packet = haskmc_protocol::java::client::play::CDebugSample::new(
            &sample_slice,
            haskmc_protocol::codec::var_int::VarInt(0),
        );
        for world in self.worlds.load().iter() {
            let players = world.players.load();
            let recipients = players
                ...
                .filter(|player| {
                    player.subscribed_debug_sample.load(Ordering::Relaxed)
                        && player.permission_lvl.load() >= haskmc_util::PermissionLvl::Two
                })
