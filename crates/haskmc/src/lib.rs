...
#[macro_use]
extern crate haskmc_macros;

use crate::crash::CrashReport;
use crate::data::VanillaData;
use crate::logging::{
    ConsoleWriter, GzipRollingLogger, HaskMcCommandCompleter, ReadlineLogWrapper,
};
...
pub fn init_logger(advanced_config: &AdvancedConfiguration) {
    ...
    let logger = advanced_config.logging.enabled.then(|| {
        ...
        let (logger, rl): (
            ConsoleWriter,
            Option<Editor<HaskMcCommandCompleter, FileHistory>>,
            ...
            let rl_config = Config::builder()
                ...
                .build();
            let helper = HaskMcCommandCompleter::new();
...
fn setup_console(mut rl: Editor<HaskMcCommandCompleter, FileHistory>, server: Arc<Server>) {
    let (tx, mut rx) = tokio::sync::mpsc::channel(1);
