...
impl NautilusEntity {
    pub fn new(entity: Entity) -> Arc<Self> {
        ...
        let nautilus = Self {
            ...
            inventory: Mutex::new(vec![ItemStack::new(0, &haskmc_data::item::Item::AIR); 9]),
        };
    ...
    pub fn set_dashing(&self, dashing: bool) {
        ...
        self.mob_entity.living_entity.entity.send_meta_data(
            &[Metadata::new(
                haskmc_data::tracked_data::nautilus::DASH,
                dashing,
            )],
            None,
        );
...
impl Animal for NautilusEntity {
    fn is_food(&self, item_stack: &ItemStack) -> bool {
        item_stack.item == &haskmc_data::item::Item::NAUTILUS_SHELL
            || item_stack.item == &haskmc_data::item::Item::PUFFERFISH
            || item_stack.item == &haskmc_data::item::Item::COD
            || item_stack.item == &haskmc_data::item::Item::SALMON
            || item_stack.item == &haskmc_data::item::Item::TROPICAL_FISH
    }
}
    ...
    fn mob_interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
        ...
        if self.is_tame() && !player.get_entity().is_sneaking() {
            if !self.is_saddled.load(Ordering::Relaxed)
                && item_stack.item == &haskmc_data::item::Item::SADDLE
            {
                item_stack.decrement_unless_creative(player.gamemode.load(), 1);
