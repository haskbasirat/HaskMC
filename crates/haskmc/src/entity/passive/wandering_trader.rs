...
use haskmc_data::statistic::{CustomStatistic, StatisticCategory};
use haskmc_data::villager::{
...
use haskmc_protocol::java::client::play::CMerchantOffers;
...
fn add_offers_from_trade_set(
    offers: &mut Vec<haskmc_protocol::java::client::play::MerchantOffer>,
        ...
        match trade.modifier {
            ...
            VillagerTradeModifier::RandomPotion => {
                if let Some(potion_name) =
                    haskmc_data::tag::Potion::MINECRAFT_TRADEABLE.0.choose(rng)
                {
...
impl WanderingTraderEntity {
    ...
    fn can_continue_trading(
        ...
        let range = player
            .living_entity
            .get_attribute_value(&haskmc_data::attributes::Attributes::ENTITY_INTERACTION_RANGE)
            + 4.0;
        entity.is_alive()
...
impl Mob for WanderingTraderEntity {
    ...
    fn mob_write_nbt(&self, nbt: &mut NbtCompound) {
        ...
        if let Some(target) = wander_target {
            nbt.put(
                "wander_target",
                haskmc_nbt::tag::NbtTag::IntArray(vec![target.0.x, target.0.y, target.0.z]),
            );
        }
        ...
        if !offers.is_empty() {
            ...
            for offer in offers.iter() {
                ...
                recipes.push(haskmc_nbt::tag::NbtTag::Compound(recipe));
            }
            let mut offers_compound = NbtCompound::new();
            offers_compound.put("Recipes", haskmc_nbt::tag::NbtTag::List(recipes));
