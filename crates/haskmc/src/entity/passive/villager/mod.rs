...
use haskmc_protocol::java::client::play::{CMerchantOffers, Metadata};
use haskmc_util::math::{boundingbox::BoundingBox, position::BlockPos, vector3::Vector3};
use haskmc_util::text::TextComponent;
use haskmc_util::version::JavaMinecraftVersion;
use haskmc_world::inventory::SimpleInventory;
...
fn enchanted_book_offer_items(
    ...
    use haskmc_data::data_component::DataComponent;
    use haskmc_data::data_component_impl::StoredEnchantmentsImpl;
...
fn enchant_trade_item(
    ...
    use haskmc_data::data_component_impl::EnchantableImpl;
    use rand::RngExt;
...
pub(crate) fn apply_random_dye(rng: &mut impl rand::Rng, stack: &mut ItemStack) {
    use haskmc_data::data_component::DataComponent;
    use haskmc_data::data_component_impl::{DataComponentImpl, DyedColorImpl};
    use rand::RngExt;
...
pub(crate) fn apply_potion(stack: &mut ItemStack, potion_name: &str) {
    use haskmc_data::data_component::DataComponent;
    use haskmc_data::data_component_impl::{DataComponentImpl, PotionContentsImpl};

    let Some(potion) = haskmc_data::potion::Potion::from_name(
        potion_name
...
pub struct VillagerEntity {
    ...
    pub offers: std::sync::Mutex<Vec<haskmc_protocol::java::client::play::MerchantOffer>>,
...
impl VillagerEntity {
    ...
    fn create_explorer_map(&self, destination: &str) -> Option<ItemStack> {
        use haskmc_data::data_component::DataComponent;
        use haskmc_data::data_component_impl::{DataComponentImpl, ItemNameImpl, MapIdImpl};
        use haskmc_data::structures::{StructureKeys, StructureSet};
        use haskmc_world::generation::generator::structure_finder::find_nearest_structure_start;
    ...
    pub fn add_trades(&self, profession: VillagerProfession, level: i32) {
        use haskmc_data::villager::VillagerTradeModifier;
            ...
            while added < trade_set.amount && !remaining_trades.is_empty() {
                ...
                match trade.modifier {
                    ...
                    VillagerTradeModifier::RandomPotion => {
                        let Some(potion_name) = haskmc_data::tag::Potion::MINECRAFT_TRADEABLE
                            .0
                            .choose(&mut rng)
                        else {
                            continue;
                ...
                new_offers.push(haskmc_protocol::java::client::play::MerchantOffer {
    ...
    fn work_at_job_site(&self, game_time: i64, day_time: i64, day: i64) {
...

        {
            ...
            if !offers
                .iter()
                .any(haskmc_protocol::java::client::play::MerchantOffer::needs_restock)
            {
                return;
            }
            for offer in offers.iter_mut() {
                ...
                < 2.0f64.powi(2)
        {
            ...
            if let Some(claimed_profession) = profession_for_block(block) {
                ...
                world.send_entity_status(
                    self.get_entity(),
                    haskmc_data::entity::EntityStatus::VillagerHappy,
    ...
    pub fn set_unhappy(&self) {
        ...
        entity.world.load().send_entity_status(
            entity,
            haskmc_data::entity::EntityStatus::VillagerAngry,
            Some(ActorEventID::VillagerAngry),
        );
        entity.play_sound(haskmc_data::sound::Sound::EntityVillagerNo);
    }
    ...
    fn bedrock_trade_data(
        offers: &[haskmc_protocol::java::client::play::MerchantOffer],
        level: i32,
    ) -> NbtCompound {
        use haskmc_nbt::tag::NbtTag;
    ...
    fn send_trade_offers(
        ...
        use haskmc_protocol::{bedrock::client::CUpdateTrade, codec::var_long::VarLong};
...
impl ScreenHandlerFactory for VillagerEntity {
    fn create_screen_handler(
        &self,
        sync_id: u8,
        player_inventory: &Arc<haskmc_inventory::player::player_inventory::PlayerInventory>,
        ...
        handler.on_trade_updated = Some(Box::new(move |has_result| {
                ...
                .is_ok()
            {
                villager.get_entity().play_sound(if has_result {
                    haskmc_data::sound::Sound::EntityVillagerYes
                } else {
                    haskmc_data::sound::Sound::EntityVillagerNo
                });
            }
        }));
...
impl VillagerEntity {
    ...
    pub fn villager_mob_tick(&self) {
        ...
        if let Some(current_home) = self.get_home_pos() {
            ...
            let valid = if block.has_tag(&haskmc_data::tag::Block::MINECRAFT_BEDS) {
            ...
            if !valid {
                *self
                ...
                if is_sleeping {
                    ...
                    self.get_entity().send_meta_data(
                        &[Metadata::new(
                            haskmc_data::tracked_data::villager::SLEEPING_POS_ID,
                            None::<BlockPos>,
                        )],
                        None,
                    );
        ...
        if self.get_home_pos().is_none() {
            ...
            for entity in nearby_entities {
                if entity.get_entity().entity_id != self.get_entity().entity_id
                    && entity.get_entity().entity_type == &haskmc_data::entity::EntityType::VILLAGER
            ...
            for p in BlockPos::iterate(start, end) {
                ...
                if block.has_tag(&haskmc_data::tag::Block::MINECRAFT_BEDS) {
                    let bed_props = BedProperties::from_state_id(state.id, block);
            ...
            if age % 100 == 0 && self.get_home().is_some() {
                ...
                if has_bed && has_worked {
                    ...
                    let nearby_villagers = world
                        ...
                        .iter()
                        .filter(|e| {
                            e.get_entity().entity_type == &haskmc_data::entity::EntityType::VILLAGER
                        })
                        .count();
                        ...
                        let nearby_golems = world
                            ...
                            .filter(|e| {
                                e.get_entity().entity_type
                                    == &haskmc_data::entity::EntityType::IRON_GOLEM
                            })
                            .count();
...
impl Mob for VillagerEntity {
            ...
            for tag in recipes {
                if let Some(recipe) = tag.extract_compound() {
                        ...
                        && buy_b.as_ref().is_none_or(|stack| !stack.is_empty())
                    {
                        ...
                        offers.push(haskmc_protocol::java::client::play::MerchantOffer {
    ...
    fn on_damage(
        &self,
        _damage_type: haskmc_data::damage::DamageType,
        source: Option<&dyn EntityBase>,
    ) {
        let Some(source) = source.filter(|source| {
            source.get_entity().entity_type == &haskmc_data::entity::EntityType::PLAYER
        }) else {
            return;
        };
    ...
    fn mob_interact(&self, player: &Arc<Player>, _item_stack: &mut ItemStack) -> bool {
        ...
        player.increment_stat(
            ...
            haskmc_data::statistic::CustomStatistic::TalkedToVillager as i32,
            1,
        );
...
mod tests {
    use haskmc_data::data_component_impl::{EnchantmentsImpl, StoredEnchantmentsImpl};
    use haskmc_data::villager::VillagerTradeModifier;
    use haskmc_util::version::JavaMinecraftVersion;
