...
impl SnifferEntity {
    ...
    pub fn set_state(&self, state: SnifferState) {
        ...
        entity.send_meta_data(
            &[Metadata::new(
                haskmc_data::tracked_data::sniffer::STATE,
                VarInt(state.id()),
            )],
            None,
        );
    ...
    fn on_digging_start(&self) {
        ...
        entity.send_meta_data(
            &[Metadata::new(
                haskmc_data::tracked_data::sniffer::DROP_SEED_AT_TICK,
                VarInt(drop_tick),
            )],
            None,
        );
    ...
    fn can_dig_at(&self, pos: BlockPos) -> bool {
        ...
        let block = haskmc_data::Block::from_state_id(block_state.id);
...
impl Mob for SnifferEntity {
    ...
    fn mob_init_data_tracker(&self) {
        ...
        if is_baby {
            entity.send_meta_data(
                &[Metadata::new(
                    haskmc_data::tracked_data::sniffer::BABY_ID,
                    true,
                )],
                None,
            );
