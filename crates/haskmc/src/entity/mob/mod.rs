...
use haskmc_protocol::java::client::play::{CHeadRot, CUpdateEntityRot, Metadata};
...
use haskmc_util::random::xoroshiro128::Xoroshiro;
use haskmc_util::random::{RandomGenerator, get_seed};
...
/// `EnvironmentAttributeSystem` is implemented in `haskmc-data`.
    ...
    pub fn mob_interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
            ...
            || item_stack.item.registry_key == "minecraft:lead"
        {
            ...
            if dist_sq <= Entity::LEASH_SNAP_DISTANCE * Entity::LEASH_SNAP_DISTANCE {
                ...
                if player.gamemode.load() != haskmc_util::GameMode::Creative {
...
pub trait Mob: EntityBase + Send + Sync {
    ...
    fn mob_bedrock_spawn_metadata(
        &self,
    ) -> Option<haskmc_protocol::bedrock::client::set_actor_data::SyncedActorDataList> {
        None
    }
        ...
        if mob_entity.love_ticks.load(Relaxed) > 0 {
            ...
            if ticks % 10 == 0 {
                ...
                world.spawn_particle(
                    ...
                    1.0,
                    1,
                    haskmc_data::particle::Particle::Heart,
                );
            }
        }
