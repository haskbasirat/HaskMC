...
use haskmc_protocol::java::client::play::CWorldEvent;
use haskmc_util::math::vector3::Vector3;
use haskmc_util::math::{boundingbox::BoundingBox, vector2::Vector2};
use haskmc_util::math::{position::BlockPos, vector2::to_chunk_pos};
...
impl SplashPotionEntity {
    pub fn new(entity: Entity) -> Self {
        ...
        Self {
            thrown,
            item_stack: RwLock::new(ItemStack::new(1, &haskmc_data::item::Item::SPLASH_POTION)),
        }
    }
...
fn is_water_potion(stack: &ItemStack) -> bool {
    stack
        ...
        == Some(haskmc_data::potion::Potion::WATER.id as i32)
}
...
impl EntityBase for SplashPotionEntity {
    fn init_data_tracker(&self) {
        ...
        entity.send_meta_data(
            &[haskmc_protocol::java::client::play::Metadata::new(
                haskmc_data::tracked_data::splash_potion::ITEM_STACK,
                &haskmc_protocol::codec::item_stack_seralizer::ItemStackSerializer::from(
                    stack.clone(),
                ),
            )],
            None,
    ...
    fn on_hit(&self, hit: crate::entity::projectile::ProjectileHit) {
        ...
        world.send_entity_status(
            self.get_entity(),
            haskmc_data::entity::EntityStatus::Death,
            Some(haskmc_protocol::bedrock::server::actor_event::ActorEventID::Death),
        );
        ...
        let has_instant = effects.iter().any(|(e, _, _, _, _, _)| {
            e.id == haskmc_data::effect::StatusEffect::INSTANT_DAMAGE.id
                || e.id == haskmc_data::effect::StatusEffect::INSTANT_HEALTH.id
        });
