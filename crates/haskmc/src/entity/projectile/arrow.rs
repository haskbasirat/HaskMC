...
use haskmc_protocol::java::client::play::CEntityVelocity;
use haskmc_protocol::java::client::play::CSoundEffect;
...
impl ArrowEntity {
    ...
    fn write_item_stack_nbt(item_stack: &ItemStack, nbt: &mut haskmc_nbt::compound::NbtCompound) {
        let mut item = haskmc_nbt::compound::NbtCompound::new();
        item_stack.copy_with_count(1).write_item_stack(&mut item);
        nbt.put_compound("item", item);
    }

    fn read_item_stack_nbt(nbt: &haskmc_nbt::compound::NbtCompound) -> Option<ItemStack> {
    ...
    const fn spectral_glowing_effect() -> haskmc_data::potion::Effect {
        haskmc_data::potion::Effect {
            effect_type: &haskmc_data::effect::StatusEffect::GLOWING,
            duration: 200,
            amplifier: 0,
            ambient: false,
            show_particles: true,
...
impl EntityBase for ArrowEntity {
    fn write_custom_nbt(&self, nbt: &mut haskmc_nbt::compound::NbtCompound) {
        let item_stack = self
            .item_stack
            .read()
    ...
    fn read_custom_nbt(&self, nbt: &haskmc_nbt::compound::NbtCompound) {
        ...
        match hit {
            ProjectileHit::Block {
                ...
                *self
                ...
                if block == &haskmc_data::Block::TARGET
...
impl ArrowEntity {
    fn should_skip_collision(&self, self_ent: &Entity, other: &Arc<dyn EntityBase>) -> bool {
        ...
        if (other_ent.entity_type == &haskmc_data::entity::EntityType::ARROW
            || other_ent.entity_type == &haskmc_data::entity::EntityType::SPECTRAL_ARROW)
            || other_ent.entity_type == &haskmc_data::entity::EntityType::ITEM
            || other_ent.entity_type == &haskmc_data::entity::EntityType::FALLING_BLOCK
        {
            return true;
        }
...
fn get_hit_face(hit_pos: Vector3<f64>, block_pos: BlockPos) -> haskmc_data::BlockDirection {
...
mod tests {
    ...
    fn spectral_arrow_applies_vanilla_glowing_effect() {
        ...
        assert_eq!(
            effect.effect_type,
            &haskmc_data::effect::StatusEffect::GLOWING
        );
