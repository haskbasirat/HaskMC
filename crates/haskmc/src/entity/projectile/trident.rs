...
use haskmc_protocol::java::client::play::{CEntityVelocity, CSoundEffect};
...
impl TridentEntity {
    ...
    fn should_skip_collision(&self, self_ent: &Entity, other: &Arc<dyn EntityBase>) -> bool {
        ...
        if other_ent.entity_type == &haskmc_data::entity::EntityType::ARROW
            || other_ent.entity_type == &haskmc_data::entity::EntityType::TRIDENT
            ...
            ProjectileHit::Entity {
                    ...
                    .get_data_component::<haskmc_data::data_component_impl::EnchantmentsImpl>()
                {
                    for (enchantment, level) in enchantments.enchantment.iter() {
                        if **enchantment == haskmc_data::Enchantment::IMPALING {
                            let in_water =
...
fn get_hit_face(hit_pos: Vector3<f64>, block_pos: BlockPos) -> haskmc_data::BlockDirection {
    ...
    if local.x <= eps {
        haskmc_data::BlockDirection::West
    } else if local.x >= 1.0 - eps {
        haskmc_data::BlockDirection::East
    } else if local.y <= eps {
        haskmc_data::BlockDirection::Down
    } else if local.y >= 1.0 - eps {
        haskmc_data::BlockDirection::Up
    } else if local.z <= eps {
        haskmc_data::BlockDirection::North
    } else {
        haskmc_data::BlockDirection::South
    }
}
