...
struct ParticleMeta<'a> {
    particle_id: haskmc_protocol::codec::var_int::VarInt,
    data: &'a [u8],
}

impl haskmc_protocol::java::client::play::MetadataSerializer for ParticleMeta<'_> {
    fn write_metadata(
        &self,
        writer: &mut impl std::io::Write,
        _version: &haskmc_util::version::JavaMinecraftVersion,
    ) -> Result<(), haskmc_protocol::ser::WritingError> {
        use haskmc_protocol::ser::NetworkWriteExt;
...
impl AreaEffectCloudEntity {
    ...
    pub fn new(entity: Entity) -> Arc<dyn EntityBase> {
        ...
        let cloud = Self {
            entity,
            item_stack: Mutex::new(ItemStack::new(0, &haskmc_data::item::Item::GLASS_BOTTLE)),
...
impl EntityBase for AreaEffectCloudEntity {
    fn init_data_tracker(&self) {
        ...
        let meta = ParticleMeta {
            particle_id: haskmc_protocol::codec::var_int::VarInt(
                haskmc_data::particle::Particle::EntityEffect as i32,
            ),
            data: &data_bytes,
        };
        ...
        self.entity.send_meta_data(
            &[haskmc_protocol::java::client::play::Metadata::new(
                haskmc_data::tracked_data::area_effect_cloud::PARTICLE,
                &meta,
            )],
            None,
        );
        ...
        self.entity.send_meta_data(
            &[haskmc_protocol::java::client::play::Metadata::new(
                haskmc_data::tracked_data::area_effect_cloud::RADIUS,
                radius,
            )],
            None,
        );
        ...
        self.entity.send_meta_data(
            &[haskmc_protocol::java::client::play::Metadata::new(
                haskmc_data::tracked_data::area_effect_cloud::WAITING,
                is_waiting,
            )],
            None,
        );
    ...
    fn tick(&self, _caller: &dyn EntityBase, _server: &Server) {
        ...
        for cand in candidates {
            ...
            if *cand_clone.get_entity().entity_type
                == haskmc_data::entity::EntityType::AREA_EFFECT_CLOUD
            {
                continue;
            }
            ...
            if let Some(living_ref) = cand_clone.get_living_entity() {
                for (eff, _, _, _, _, _) in &effs_clone {
                    ...
                    let is_instant = eff.id == haskmc_data::effect::StatusEffect::INSTANT_DAMAGE.id
                        || eff.id == haskmc_data::effect::StatusEffect::INSTANT_HEALTH.id;
                    if is_instant {
                        will_apply = true;
                        break;
                    }
