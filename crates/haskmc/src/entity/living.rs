...
use haskmc_data::tracked_data;
use haskmc_inventory::build_equipment_slots;
use haskmc_inventory::player::player_inventory::PlayerInventory;
use haskmc_inventory::screen_handler::InventoryPlayer;
use haskmc_protocol::bedrock::client::take_item_actor::CTakeItemActor;
...
use haskmc_data::damage::DeathMessageType;
use haskmc_data::data_component_impl::Operation;
use haskmc_data::data_component_impl::food::{ConsumableImpl, ConsumeEffect};
use haskmc_data::data_component_impl::{
...
use haskmc_data::item_stack::{DamageResult, ItemStack};
use haskmc_data::sound::SoundCategory;
use haskmc_data::{Block, Enchantment, translation};
use haskmc_data::{damage::DamageType, sound::Sound};
use haskmc_inventory::entity_equipment::EntityEquipment;
...
impl MetadataSerializer for EffectParticles {
    fn write_metadata(
        ...
        _version: &haskmc_util::version::JavaMinecraftVersion,
    ) -> Result<(), WritingError> {
...
impl LivingEntity {
    ...
    pub fn send_equipment_changes(&self, equipment: &[(EquipmentSlot, ItemStack)]) {
        ...
        for (slot, stack) in equipment {
            if *slot == EquipmentSlot::MAIN_HAND || *slot == EquipmentSlot::OFF_HAND {
                ...
                let be_packet = haskmc_protocol::bedrock::client::CMobEquipment {
                    target_runtime_id: (self.entity_id() as u64).into(),
                    item: haskmc_protocol::bedrock::network_item::NetworkItemStackDescriptor::from(
                        stack,
                    ),
                    slot: 0,
                    selected_slot: 0,
    ...
    fn set_living_flag(&self, flag: u8, value: bool) {
        ...
        let bedrock_meta = (flag == Self::USING_ITEM_FLAG).then(|| {
            let index =
                haskmc_protocol::bedrock::client::set_actor_data::entity_data_flag::USING_ITEM;
            let mask = 1i64 << index;
            if value {
            ...
            let mut meta =
                haskmc_protocol::bedrock::client::set_actor_data::SyncedActorDataList::new();
            meta.set(
                haskmc_protocol::bedrock::client::set_actor_data::entity_data_key::FLAGS,
                haskmc_protocol::bedrock::client::set_actor_data::MetadataValue::Int64(
    ...
    pub fn add_effect(&self, effect: Effect) {
        ...
        if effect.effect_type == &StatusEffect::INSTANT_HEALTH {
            ...
            if !effect.effect_type.attribute_modifiers.is_empty() {
                ...
                let mut touched_attrs: Vec<haskmc_data::attributes::Attributes> = Vec::new();
                for m in effect.effect_type.attribute_modifiers {
    ...
    pub fn swing_hand(&self) {
        ...
        let je_packet = haskmc_protocol::java::client::play::CEntityAnimation::new(
            entity_id.into(),
            haskmc_protocol::java::client::play::Animation::SwingMainArm,
        );
    ...
    pub fn fall(
        ...
        if ground {
            ...
            let haskmc_block = world.block_registry.get_haskmc_block(block.id);
            ...
            let tool = if let Some(cause_ent) = cause {
                    ...
                    .downcast_ref::<crate::entity::player::Player>()
                {
                    let hand_stack = player
                        .inventory()
                        .get_stack_in_hand(haskmc_util::Hand::Right);
    ...
    fn damage_armor_items(&self, caller: &dyn EntityBase, damage_amount: f32) {
        ...
        for (slot_index, mut stack, slot) in armor_slots {
            ...
            if takes_damage {
                ...
                if slot_result != haskmc_data::item_stack::DamageResult::Untouched {
                    if slot_result == haskmc_data::item_stack::DamageResult::Broken {
                        let world = self.entity.world.load();
...
impl LivingEntity {
    pub fn write_living_nbt(&self, nbt: &mut NbtCompound) {
        ...
        {
            let effects_vec: Vec<haskmc_data::potion::Effect> = {
                let effects = self
                    .active_effects
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner);
            ...
            if !effects_vec.is_empty() {
                ...
                for effect in effects_vec {
                    let mut effect_nbt = haskmc_nbt::compound::NbtCompound::new();
    ...
    pub fn damage_with_context(
        ...
        if clamped_health <= 0.0 {
            ...
            world.send_entity_status(&self.entity, haskmc_data::entity::EntityStatus::Death, None);
        }
    ...
    fn tick(&self, caller: &dyn EntityBase, server: &Server) {
            ...
            && self.item_use_time.fetch_sub(1, Ordering::Relaxed) <= 1
        {
            ...
            if let Some(item) = item_in_use.as_ref() {
                ...
                if item
                    .get_data_component::<haskmc_data::data_component_impl::PotionContentsImpl>()
                    .is_some()
                {
        ...
        for consume_effect in consumable.effects.iter() {
            match consume_effect {
                ...
                ConsumeEffect::RemoveEffects(idset) => {
                    if let haskmc_data::data_component_impl::IDSet::IDs(ids) = idset {
...
mod tests {
    ...
    fn regeneration_particle_metadata_uses_vanilla_argb_color() {
        ...
        metadata
            .write(
                &mut bytes,
                &haskmc_util::version::JavaMinecraftVersion::V_26_2,
            )
            .unwrap();
