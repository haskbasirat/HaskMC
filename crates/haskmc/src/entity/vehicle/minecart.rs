...
use haskmc_protocol::java::server::play::SPlayerInput;
use rand::RngExt;
...
use haskmc_data::block_properties::{BlockProperties, PoweredRailLikeProperties};
...
const fn get_exits(
    shape: haskmc_data::block_properties::RailShape,
) -> (Vector3<f64>, Vector3<f64>) {
    use haskmc_data::block_properties::RailShape;
    match shape {
...
impl EntityBase for MinecartEntity {
    ...
    fn tick(&self, caller: &dyn EntityBase, _server: &Server) {
        ...
        if is_on_rails {
            use haskmc_data::block_properties::RailLikeProperties;
            use haskmc_data::block_properties::{RailShape, RailShapeStraight};
