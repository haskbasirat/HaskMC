...
use haskmc_inventory::merchant::merchant_screen_handler::MerchantScreenHandler;
use haskmc_inventory::player::ender_chest_inventory::EnderChestInventory;
use haskmc_protocol::RawPacket;
use haskmc_protocol::bedrock::client::play_status::CPlayStatus;
use haskmc_protocol::bedrock::client::set_time::CSetTime;
use haskmc_protocol::bedrock::client::update_abilities::{Ability, CUpdateAbilities};
...
use haskmc_protocol::bedrock::server::{
    respawn::{RespawnState, SRespawn as SBedrockRespawn},
    text::SText,
};
use haskmc_protocol::codec::item_stack_seralizer::ItemStackSerializer;
use haskmc_util::translation::Locale;
use haskmc_util::version::JavaMinecraftVersion;
use haskmc_world::chunk::{ChunkData, ChunkEntityData};
...
impl JavaPlayer<'_> {
    pub async fn send_packet<C: haskmc_protocol::ClientPacket + Sync>(&self, packet: &C) {
...
impl BedrockPlayer<'_> {
    pub async fn send_packet<P: haskmc_protocol::BClientPacket + Sync>(&self, packet: &P) {
    ...
    #[must_use]
    pub fn client_data(&self) -> Option<Arc<haskmc_protocol::bedrock::server::login::ClientData>> {
        if let ClientPlatform::Bedrock(client) = self.0.client.as_ref() {
...
use haskmc_data::attributes::Attributes;
use haskmc_data::block_properties::{BlockProperties, HorizontalFacing};
use haskmc_data::damage::DamageType;
use haskmc_data::data_component_impl::{AttributeModifiersImpl, EnchantmentsImpl, Operation};
use haskmc_data::data_component_impl::{EquipmentSlot, EquippableImpl, ToolImpl, WeaponImpl};
use haskmc_data::effect::StatusEffect;
use haskmc_data::entity::{EntityPose, EntityStatus, EntityType};
use haskmc_data::item_stack::ItemStack;
use haskmc_data::particle::Particle;
use haskmc_data::sound::{Sound, SoundCategory};
use haskmc_data::statistic::StatisticCategory;
use haskmc_data::tag::Taggable;
use haskmc_data::{Block, BlockState, Enchantment, screen::WindowType, tag, translation};
use haskmc_inventory::player::{
...
use haskmc_inventory::sync_handler::SyncHandler;
use haskmc_macros::send_cancellable;
use haskmc_nbt::compound::NbtCompound;
use haskmc_nbt::tag::NbtTag;
use haskmc_protocol::IdOr;
use haskmc_protocol::SoundEvent;
use haskmc_protocol::bedrock::client::container_open::CContainerOpen;
use haskmc_protocol::bedrock::server::actor_event::{ActorEventID, SActorEvent};
use haskmc_protocol::codec::var_int::VarInt;
use haskmc_protocol::codec::var_long::VarLong;
use haskmc_protocol::codec::var_ulong::VarULong;
...
use haskmc_util::text::hover::HoverEvent;
use haskmc_util::{GameMode, Hand};
use haskmc_world::biome;
use haskmc_world::cylindrical_chunk_iterator::Cylindrical;
use haskmc_world::level::{Level, SyncChunk, SyncEntityChunk};
...
use haskmc_data::potion::Effect;
use haskmc_world::chunk_system::ChunkLoading;
...
pub struct Player {
    ...
    pub compass_target: AtomicCell<Option<haskmc_util::math::position::BlockPos>>,
    pub respawn_location: AtomicCell<Option<haskmc_util::math::position::BlockPos>>,
    ...
    pub bedrock_skin: arc_swap::ArcSwap<haskmc_protocol::bedrock::client::Skin>,
...
impl Player {
    #[must_use]
    pub fn fetch_skin(properties: &[Property]) -> Option<haskmc_protocol::bedrock::client::Skin> {
        ...
        let mut skin = haskmc_protocol::bedrock::client::Skin::steve();
    ...
    pub fn new(
        ...
        let mut bedrock_skin = Self::fetch_skin(&properties)
            .unwrap_or_else(haskmc_protocol::bedrock::client::Skin::steve);
        ...
        if bedrock_skin.skin_id == "Standard_Custom" {
            let skin_id = format!("haskmc:{player_uuid}");
    ...
    pub fn open_ender_chest(self: &Arc<Self>) -> Option<u8> {
        self.increment_stat(
            ...
            haskmc_data::statistic::CustomStatistic::OpenEnderchest as i32,
            1,
        );
    ...
    pub fn attack(&self, victim: &Arc<dyn EntityBase>) {
        ...
        if let Some(strength) = self
            .living_entity
            .get_effect(&haskmc_data::effect::StatusEffect::STRENGTH)
        {
            damage += 3.0 * (f64::from(strength.amplifier) + 1.0);
        }
        if let Some(weakness) = self
            .living_entity
            .get_effect(&haskmc_data::effect::StatusEffect::WEAKNESS)
        {
    ...
    pub fn damage_item_in_slot(&self, slot: &EquipmentSlot, amount: i32) -> bool {
        ...
        let updated = (result != haskmc_data::item_stack::DamageResult::Untouched)
        ...
        if let Some((result, updated_stack)) = updated {
            ...
            if result == haskmc_data::item_stack::DamageResult::Broken {
                if let Some(server) = self.world().server.upgrade()
                    && let Some(player_arc) = self.world().get_player_by_uuid(self.gameprofile.id)
                {
    ...
    pub fn set_respawn_point(
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &haskmc_protocol::bedrock::client::CSetSpawnPosition {
                spawn_position_type:
                    haskmc_protocol::bedrock::client::SpawnPositionType::PlayerRespawn,
    ...
    pub async fn calculate_respawn_point(&self) -> Option<CalculatedRespawnPoint> {
        type BedProperties = haskmc_data::block_properties::WhiteBedLikeProperties;
        type AnchorProperties = haskmc_data::block_properties::RespawnAnchorLikeProperties;
    ...
    pub fn sleep(&self, bed_head_pos: BlockPos) {
        ...
        self.get_entity().send_meta_data(
            &[Metadata::new(
                haskmc_data::tracked_data::player::SLEEPING_POS_ID,
                Some(bed_head_pos),
            )],
            None,
        );
    ...
    pub fn show_title(&self, text: &TextComponent, mode: &TitleMode) {
        match mode {
            TitleMode::Title => {
                self.client.try_enqueue_packet_editioned(
                    &CTitleText::new(text),
                    &haskmc_protocol::bedrock::client::set_title::CSetTitle::new(
                        haskmc_protocol::bedrock::client::TitleType::Title,
                        text.clone().get_text(),
                        0,
                        0,
                        0,
            ...
            TitleMode::SubTitle => {
                self.client.try_enqueue_packet_editioned(
                    &CSubtitle::new(text),
                    &haskmc_protocol::bedrock::client::set_title::CSetTitle::new(
                        haskmc_protocol::bedrock::client::TitleType::Subtitle,
                        text.clone().get_text(),
                        0,
                        0,
                        0,
            ...
            TitleMode::ActionBar => {
                self.client.try_enqueue_packet_editioned(
                    &CActionBar::new(text),
                    &haskmc_protocol::bedrock::client::set_title::CSetTitle::new(
                        haskmc_protocol::bedrock::client::TitleType::Actionbar,
                        text.clone().get_text(),
                        0,
                        0,
                        0,
    ...
    pub fn send_title_animation(&self, fade_in: i32, stay: i32, fade_out: i32) {
        self.try_enqueue_packet_editioned(
            ...
            &haskmc_protocol::bedrock::client::set_title::CSetTitle::new(
                haskmc_protocol::bedrock::client::TitleType::Times,
                String::new(),
                fade_in,
                stay,
                fade_out,
            ...
            if state.is_air() {
                ...
                if finished && matches!(p.client.as_ref(), ClientPlatform::Bedrock(_)) {
                    ...
                    let flags = if can_harvest {
                        haskmc_world::world::BlockFlags::NOTIFY_NEIGHBORS
                    } else {
                        haskmc_world::world::BlockFlags::SKIP_DROPS
                            | haskmc_world::world::BlockFlags::NOTIFY_NEIGHBORS
                    };
    ...
    pub fn get_movement_statistic(&self) -> statistics::CustomStatistic {
        ...
        if block.has_tag(&haskmc_data::tag::Block::MINECRAFT_CLIMBABLE) {
    ...
    pub fn send_difficulty_update(&self) {
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &haskmc_protocol::bedrock::client::CSetDifficulty {
    ...
    pub async fn send_editioned<
        J: haskmc_protocol::ClientPacket + Sync,
        B: haskmc_protocol::BClientPacket + Sync,
    >(
        &self,
        je_packet: &J,
        be_packet: &B,
    ...
    pub fn try_send_client_packet<C: haskmc_protocol::ClientPacket + Sync>(&self, packet: &C) {
        if let ClientPlatform::Java(client) = self.client.as_ref()
            && let Ok(data) = client.serialize_packet(packet)
        {
            client.try_enqueue_packet(data);
    ...
    pub async fn send_client_packet<C: haskmc_protocol::ClientPacket + Sync>(&self, packet: &C) {
        if let ClientPlatform::Java(client) = self.client.as_ref()
            && let Ok(data) = client.serialize_packet(packet)
        {
            client.enqueue_packet(data).await;
    ...
    pub fn set_compass_target(&self, pos: haskmc_util::math::position::BlockPos) {
        use haskmc_protocol::java::client::play::CPlayerSpawnPosition;
        self.compass_target.store(Some(pos));
        self.try_send_client_packet(&CPlayerSpawnPosition::new(pos, 0.0, 0.0, String::new()));
    }

    pub fn get_compass_target(&self) -> Option<haskmc_util::math::position::BlockPos> {
        self.compass_target.load()
    }

    pub fn set_respawn_location(&self, pos: haskmc_util::math::position::BlockPos) {
        self.respawn_location.store(Some(pos));
    }

    pub fn get_respawn_location(&self) -> Option<haskmc_util::math::position::BlockPos> {
    ...
    pub fn get_target_block(
        ...
        world: &Arc<World>,
        max_distance: f64,
    ) -> Option<haskmc_util::math::position::BlockPos> {
        let (yaw, pitch) = (
            self.living_entity.entity.yaw.load(),
            self.living_entity.entity.pitch.load(),
        );
        ...
        let dir = haskmc_util::math::vector3::Vector3::new(
        ...
        send_cancellable! {{
            ...
            'after: {
               ...
               new_world.players.rcu(|current_list| {
                ...
                if new_world.dimension == haskmc_data::dimension::Dimension::THE_NETHER {
                ...
                } else if new_world.dimension == haskmc_data::dimension::Dimension::THE_END {
                    self.trigger_advancement(crate::entity::player::advancement::trigger::AdvancementTrigger::EnterDimension {
                ...
                match self.client.as_ref() {
                    ...
                    ClientPlatform::Bedrock(bedrock) => {
                        ...
                        let change_dim_packet = haskmc_protocol::bedrock::client::CChangeDimension {
    ...
    pub fn damage(
        ...
        amount: f32,
        damage_type: haskmc_data::damage::DamageType,
    ) -> bool {
    ...
    pub fn damage_with_context(
        ...
        if self
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .invulnerable
            && damage_type != haskmc_data::damage::DamageType::GENERIC_KILL
            && damage_type != haskmc_data::damage::DamageType::OUT_OF_WORLD
        {
            return false;
        }
        self.living_entity
    ...
    pub fn ban_explicit(
        ...
        let string_reason = reason.clone().map_or_else(
            ...
            haskmc_util::text::TextComponent::get_text,
        );
    ...
    pub fn set_gamemode(self: &Arc<Self>, gamemode: GameMode) -> bool {
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &haskmc_protocol::bedrock::client::set_player_gamemode::CSetPlayerGameType {
    ...
    pub fn send_client_information(&self) {
        ...
        self.living_entity.entity.send_meta_data(
            &[
                ...
                Metadata::new(
                    haskmc_data::tracked_data::player::PLAYER_MAIN_HAND,
                ...
                Metadata::new(
                    haskmc_data::tracked_data::player::MAIN_ARM_ID,
                    config.main_hand as u8,
                ),
            ],
            None,
    ...
    pub fn tick_maps(&self, server: &Server) {
        use haskmc_data::data_component_impl::MapIdImpl;
                    ...
                    && let Ok(mut map_data) = map_data_arc.try_lock()
                {
                    ...
                    if map_data.dirty || tick_count % 10 == 0 {
                        ...
                        let decoration_type = if is_off_map {
                            &haskmc_data::map_decoration::MapDecorationType::PLAYER_OFF_MAP
                        } else {
                            &haskmc_data::map_decoration::MapDecorationType::PLAYER
                        };
    ...
    pub fn remove_effect(&self, effect_type: &'static StatusEffect) -> bool {
        ...
        self.try_send_client_packet(&haskmc_protocol::java::client::play::CRemoveMobEffect::new(
            self.entity_id().into(),
            effect_id,
        ));
    ...
    pub fn remove_all_effects(&self) -> bool {
        ...
        for effect in effects {
            ...
            self.try_send_client_packet(
                &haskmc_protocol::java::client::play::CRemoveMobEffect::new(
                    self.entity_id().into(),
                    effect_id,
                ),
            );
    ...
    pub fn close_handled_screen(&self) {
        ...
        self.try_enqueue_packet_editioned(
            ...
            &haskmc_protocol::bedrock::server::container_close::SContainerClose {
    ...
    pub fn on_rename_item(self: &Arc<Self>, packet: &SRenameItem<'_>) {
        ...
        if let Some(anvil_handler) = screen_handler
            .as_any_mut()
            .downcast_mut::<haskmc_inventory::anvil::AnvilScreenHandler>()
        {
            ...
            && !stack.is_empty()
        {
            let mut extract_event =
                crate::plugin::api::events::inventory::furnace_extract::FurnaceExtractEvent::new(
                    self.clone(),
                    haskmc_util::math::position::BlockPos::new(0, 0, 0),
                    stack.item.registry_key.to_string(),
    ...
    pub fn swing_hand(&self, hand: Hand, all: bool) {
        ...
        let je_packet = haskmc_protocol::java::client::play::CEntityAnimation::new(
            VarInt(entity_id),
            animation,
        );

        let be_packet = haskmc_protocol::bedrock::server::animate::SAnimate {
            action: haskmc_protocol::bedrock::server::animate::AnimateAction::SwingArm,
            target_actor_runtime_id: haskmc_protocol::codec::var_ulong::VarULong(entity_id as u64),
            data: 0.0,
            swing_source: None,
        };
    ...
    pub fn has_advancement(
        &self,
        advancement: &'static haskmc_data::advancement::Advancement,
    ) -> bool {
    ...
    pub fn has_item_in_inventory(&self, item: &haskmc_data::item::Item) -> bool {
        let main_inv = self
            .inventory
            .main_inventory
            .read()
...
impl InventoryPlayer for Player {
    ...
    fn enqueue_set_held_item_packet(&self, packet: &CSetSelectedSlot) {
        self.try_enqueue_packet_editioned(
            packet,
            &haskmc_protocol::bedrock::client::CPlayerHotbar {
                selected_slot: haskmc_protocol::codec::var_uint::VarUInt(packet.slot as u32),
                container_id: 0,
    ...
    fn fire_enchant_item_event(
        ...
        option: i32,
        exp_level_cost: i32,
        enchantments_to_add: &mut Vec<(&'static haskmc_data::Enchantment, i32)>,
    ) -> bool {
        let Some(player_arc) = self.world().get_player_by_uuid(self.gameprofile.id) else {
            return false;
        };
...
mod tests {
    ...
    use haskmc_nbt::{compound::NbtCompound, tag::NbtTag};
    use uuid::Uuid;
