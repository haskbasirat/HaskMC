...
impl BreathManager {
    ...
    pub fn send_air_supply(&self, player: &Player) {
        ...
        bedrock_meta.set(
            haskmc_protocol::bedrock::client::set_actor_data::entity_data_key::AIR_SUPPLY,
            haskmc_protocol::bedrock::client::set_actor_data::MetadataValue::Short(air as i16),
        );

        player.get_entity().send_meta_data(
            &[Metadata::new(
                haskmc_data::tracked_data::entity::DATA_AIR_SUPPLY_ID,
                VarInt(air),
            )],
            Some(&bedrock_meta),
        );
