...
use haskmc_data::BlockState;
use haskmc_data::biome::Biome;
use haskmc_data::block_properties::blocks_movement;
use haskmc_data::data_component_impl::EquipmentSlot;
use haskmc_data::dimension::Dimension;
use haskmc_data::entity::EntityStatus;
...
use haskmc_protocol::bedrock::client::{CAddActor, CSetActorMotion};
use haskmc_protocol::codec::var_long::VarLong;
use haskmc_protocol::java::client::play::{CUpdateEntityPos, CUpdateEntityPosRot};
use haskmc_protocol::{
    PositionFlag,
    bedrock::client::{
        move_actor_delta::{
...
use haskmc_util::math::vector3::Axis;
...
pub trait EntityBase: Send + Sync + std::any::Any {
    ...
    fn get_job_site_pos(&self) -> Option<haskmc_util::math::position::BlockPos> {
        None
    }

    fn get_home_pos(&self) -> Option<haskmc_util::math::position::BlockPos> {
        None
    }
...
impl Entity {
    ...
    pub fn try_use_portal(&self, portal_world: Arc<World>, pos: BlockPos) {
        ...
        if manager.is_none() {
            ...
            if let Some(portal) = NetherPortal::get_on_axis(
                &world,
                &pos,
                haskmc_data::block_properties::HorizontalAxis::X,
            ) && portal.was_already_valid()
            {
                ...
                &world,
                &pos,
                haskmc_data::block_properties::HorizontalAxis::Z,
            ) && portal.was_already_valid()
            {
                new_manager.set_source_portal(SourcePortalInfo {
                    lower_corner: portal.lower_corner(),
    ...
    pub fn leash_to(&self, holder: Arc<dyn EntityBase>) {
        ...
        *self
        ...
        let je_packet = haskmc_protocol::java::client::play::CSetEntityLink::new(
            self.entity_id,
            holder_entity_id,
            true,
        );
        let be_packet = haskmc_protocol::bedrock::client::CSetActorLink {
            link: haskmc_protocol::bedrock::client::common::ActorLink {
                ridden_unique_id: haskmc_protocol::codec::var_long::VarLong(self.entity_id as i64),
                rider_unique_id: haskmc_protocol::codec::var_long::VarLong(holder_entity_id as i64),
    ...
    pub fn unleash(&self) {
        ...
        let je_packet =
            haskmc_protocol::java::client::play::CSetEntityLink::new(self.entity_id, -1, true);
        let be_packet = haskmc_protocol::bedrock::client::CSetActorLink {
            link: haskmc_protocol::bedrock::client::common::ActorLink {
                ridden_unique_id: haskmc_protocol::codec::var_long::VarLong(self.entity_id as i64),
                rider_unique_id: haskmc_protocol::codec::var_long::VarLong(-1),
                link_type: 0, // Unlink
                immediate: true,
                rider_initiated: false,
                vehicle_angular_velocity: 0.0,
    ...
    pub fn tick_leash(&self) {
        ...
        if let Some(holder) = holder {
            ...
            if distance > Self::LEASH_SNAP_DISTANCE {
                ...
                self.unleash();
                let lead_item =
                    haskmc_data::item_stack::ItemStack::new(1, &haskmc_data::item::Item::LEAD);
                self.world
                    .load()
...
impl Entity {
    pub fn write_nbt(&self, nbt: &mut NbtCompound) {
        ...
        if let Some(custom_name) = &**self.custom_name.load()
            && let Ok(name_json) = haskmc_util::serde_json::to_string(custom_name)
        {
    ...
    pub fn read_nbt_non_mut(&self, nbt: &NbtCompound) {
        ...
        if let Some(name_json) = nbt.get_string("CustomName")
            && let Ok(component) = haskmc_util::serde_json::from_str(name_json)
        {
