...
use haskmc_world::{CURRENT_BEDROCK_MC_PROTOCOL, CURRENT_BEDROCK_MC_VERSION};
use std::{
...
use haskmc::{HaskMcServer, stop_server};

use haskmc_config::{HaskMcConfig, LoadConfiguration};
use haskmc_util::text::{
    TextComponent,
...
async fn main() {
    ...
    let config = HaskMcConfig::load(&exec_dir);

    let vanilla_data = VanillaData::load();

    haskmc::init_logger(&config.advanced);

    info!(
        "{}",
        TextComponent::text(format!(
            "Starting {} {} Java Minecraft (Protocol {}) | {} Bedrock (Protocol {})",
            TextComponent::text("HaskMC")
    ...
    if cfg!(debug_assertions) {
        warn!(
            "HaskMC is running an unoptimized debug build. Do not use this build for performance testing; run `cargo run --release` or use a release binary."
        );
    }
    ...
    let haskmc_server = HaskMcServer::new(config.basic, config.advanced, vanilla_data).await;
    let plugin_wait_time = haskmc_server.init_plugins().await;
    ...
    let advanced_config = &haskmc_server.server.advanced_config;
    info!(
...
fn print_support_links_and_warning() {
    warn!(
        "{}",
        TextComponent::text("HaskMC is currently under heavy development!")
            .color_named(NamedColor::DarkRed)
            .to_pretty_console(),
    );
    info!(
        "Report issues on {}",
        TextComponent::text("https://github.com/haskbasirat/HaskMC/issues")
