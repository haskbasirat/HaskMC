...
mod test {
    ...
    use haskmc_util::text::TextContent;
    use haskmc_util::text::click::ClickEvent;
    use haskmc_util::text::color::{Color, NamedColor};
    ...
    fn component_plain_text(component: &haskmc_util::text::TextComponentBase) -> Option<&str> {
    ...
    #[test]
    fn dynamic_command() {
        let commands_config = haskmc_config::CommandsConfig::default();
    ...
    #[test]
    fn haskmc_command_aliases() {
        let commands_config = haskmc_config::CommandsConfig::default();
        let manager = PermissionManager::new();
        let dispatcher = default_dispatcher(&manager, &commands_config).fallback_dispatcher;

        let legacy_pumpkin_tree = dispatcher.get_tree("pumpkin").unwrap();
        let version_tree = dispatcher.get_tree("version").unwrap();
        let ver_tree = dispatcher.get_tree("ver").unwrap();

        assert_eq!(legacy_pumpkin_tree.description, version_tree.description);
        assert_eq!(legacy_pumpkin_tree.description, ver_tree.description);
    }
