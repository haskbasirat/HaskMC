...
use haskmc_data::{Advancement, translation};
use haskmc_util::identifier::Identifier;
use haskmc_util::resource::ResourceKey;
...
impl ArgumentType for ResourceKeyArgument {
    ...
    fn list_suggestions(
        ...
        if self.0 == ADVANCEMENT_REGISTRY {
            ...
            let biomes = haskmc_data::biome::Biome::ALL
                .iter()
...
impl ResourceKeyArgument {
    ...
    /// Returns a [`CommandContext`]'s parsed resource key argument as a [`Biome`](haskmc_data::biome::Biome).
    pub fn get_biome(
        context: &CommandContext,
        name: &str,
    ) -> Result<&'static haskmc_data::biome::Biome, CommandSyntaxError> {
        ...
        haskmc_data::biome::Biome::from_name(path).ok_or_else(|| {
            ERROR_INVALID_BIOME
