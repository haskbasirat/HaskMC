...
/// ```
/// use haskmc::command::suggestion::SuggestionText;
///
...
mod test {
    ...
    fn apply_insertion_start() {
        let suggestion = Suggestion::without_tooltip(StringRange::at(0), "HaskMC once said: ");
        assert_eq!(
            suggestion.apply("'Server is now running'"),
            "HaskMC once said: 'Server is now running'".to_owned()
        );
    }
