...
use haskmc_config::CommandsConfig;
use haskmc_util::{
    PermissionLvl,
...
pub fn default_dispatcher(
    ...
    // Zero
    dispatcher.register(haskmc::init_command_tree(), "haskmc:command.haskmc");
    ...
    dispatcher.register(tps::init_command_tree(), "haskmc:command.tps");
    ...
    dispatcher.register(plugin::init_command_tree(), "haskmc:command.plugin");
    dispatcher.register(plugins::init_command_tree(), "haskmc:command.plugins");
...
fn resolve_permission_node(
    ...
    for namespace in ["minecraft", "haskmc"] {
...
fn register_level_0_permissions(registry: &PermissionRegistry) {
    ...
    registry
        .register_permission(Permission::new(
            "haskmc:command.haskmc",
            "Shows information about the HaskMC server",
            PermissionDefault::Allow,
        ))
        .unwrap_or_else(|e| tracing::warn!("{e}"));
    registry
...
#[cfg(test)]
mod override_tests {
    use haskmc_config::{CommandOverride, CommandsConfig};
    use haskmc_util::PermissionLvl;
    use haskmc_util::permission::{PermissionDefault, PermissionManager};
