...
use haskmc_data::placed_feature::PlacedFeature as PlacedFeatureKey;
use haskmc_data::structures::{Structure, StructureKeys, StructureType};
use haskmc_data::translation;
use haskmc_data::{Block, BlockStateId, Mirror, Rotation};
use haskmc_util::identifier::Identifier;
use haskmc_util::math::block_box::BlockBox;
use haskmc_util::math::position::BlockPos;
use haskmc_util::math::vector3::Vector3;
use haskmc_util::random::hash_block_pos;
use haskmc_util::text::TextComponent;
use haskmc_world::generation::proto_chunk::ProtoChunk;
use haskmc_world::world::{BlockAccessor, WorldPortalExt};
...
use haskmc_world::generation::feature::configured_features::CONFIGURED_FEATURES;
use haskmc_world::generation::feature::placed_features::{Feature, PLACED_FEATURES};
use haskmc_world::generation::structure::structures::StructureGeneratorContext;
use haskmc_world::generation::structure::structures::jigsaw::{
    PoolElementStructurePiece, place_pool_element_templates,
};
use haskmc_world::generation::structure::structures::jigsaw_placement::{
    DimensionPadding, JigsawPlacement, LiquidSettings, MaxDistance, PoolAliasLookup,
};
use haskmc_world::generation::structure::template::BlockPlacer;

use haskmc_util::PermissionLvl;
use haskmc_util::permission::{Permission, PermissionDefault, PermissionRegistry};
use haskmc_util::random::RandomGenerator;
use haskmc_util::random::legacy_rand::LegacyRand;
...
impl WorldPortalExt for CommandBlockRegistry {
    ...
    fn spawn_mobs_for_chunk_generation(
        ...
        _biome: &'static haskmc_data::biome::Biome,
        _chunk_x: i32,
        _chunk_z: i32,
    ) {
    }
...
impl CommandExecutor for PlaceTemplateExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        let template_name = template_name
            ...
            .to_string();
        let Some(template) =
            haskmc_world::generation::structure::template::get_template(&template_name)
        else {
            return Err(
        ...
        haskmc_world::generation::structure::template::place_template(
            &mut placer,
            &template,
            block_pos.0,
            (0, 0),
    ...
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        let (_piece_count, placer) = {
            ...
            if structure.structure_type == StructureType::Jigsaw {
                ...
                let position = haskmc_world::generation::structure::generate_structure_position(
                    &key,
                    structure,
