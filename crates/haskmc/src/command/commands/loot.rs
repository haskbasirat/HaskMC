...
fn insert_into_inventory(
    inventory: &dyn haskmc_world::inventory::Inventory,
    mut stack: haskmc_data::item_stack::ItemStack,
    ...
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        match self.source {
            Source::Loot => {
                ...
                let chest_table =
                    haskmc_data::chest_loot_table::get_chest_loot_table(&formatted_key);
            ...
            Source::Mine { has_tool } => {
                ...
                if let Some(loot_table) = &block.loot_table {
                    let tool_item = if has_tool {
                        ...
                        haskmc_data::item::Item::from_registry_key(key)
                    } else {
                        None
                    };

                    let tool_stack =
                        tool_item.map(|item| haskmc_data::item_stack::ItemStack::new(1, item));
