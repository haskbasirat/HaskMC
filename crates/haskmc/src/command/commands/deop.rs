    ...
    fn execute(
        ...
        for profile in targets {
            if let Some(op_index) = config.ops.iter().position(|o| o.uuid == profile.id) {
                ...
                let msg = TextComponent::translate_cross(
                    haskmc_data::translation::java::COMMANDS_DEOP_SUCCESS,
                    haskmc_data::translation::bedrock::COMMANDS_DEOP_SUCCESS,
        ...
        if succeeded_deops == 0 {
            Err(CommandError::CommandFailed(TextComponent::translate_cross(
                haskmc_data::translation::java::COMMANDS_DEOP_FAILED,
                haskmc_data::translation::bedrock::COMMANDS_DEOP_FAILED,
                [],
            )))
        } else {
            Ok(succeeded_deops)
