...
impl ItemRegistry {
    ...
    pub fn on_use(&self, stack: &ItemStack, player: &Player) {
        ...
        let haskmc_item = self.get_haskmc_item(item.id);
        if let Some(haskmc_item) = haskmc_item {
            haskmc_item.normal_use(item, player);
        }
    ...
    pub fn on_stopped_using(&self, stack: &ItemStack, player: &Player) {
        if let Some(behaviour) = self.get_haskmc_item(stack.item.id) {
    ...
    pub fn use_on_block(
        ...
        let haskmc_item = self.get_haskmc_item(stack.item.id);
        if let Some(haskmc_item) = haskmc_item {
            haskmc_item.use_on_block(stack, player, location, face, cursor_pos, block, server);
        }
    ...
    pub fn use_on_entity(
        ...
        let haskmc_item = self.get_haskmc_item(stack.item.id);
        if let Some(haskmc_item) = haskmc_item {
            haskmc_item.use_on_entity(stack, player, entity);
        }
    ...
    pub fn can_mine(&self, item: &Item, player: &Player) -> bool {
        let haskmc_block = self.get_haskmc_item(item.id);
        if let Some(haskmc_block) = haskmc_block {
            return haskmc_block.can_mine(player);
        }
        true
    }

    #[must_use]
    pub fn get_haskmc_item(&self, item: u16) -> Option<&Arc<dyn ItemBehaviour>> {
        self.items.get(&item)
    }
}
