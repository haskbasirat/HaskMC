...
impl ItemBehaviour for BowItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        player
            .living_entity
            .set_active_hand(haskmc_util::Hand::Right, stack, Self::USE_DURATION);
    }
...
impl BowItem {
    ...
    pub fn release_bow(player: &Player) {
        ...
        if let Some(enchantments) =
            held.get_data_component::<haskmc_data::data_component_impl::EnchantmentsImpl>()
        {
            has_infinity = enchantments
                .enchantment
                .iter()
                .any(|(e, _)| **e == haskmc_data::Enchantment::INFINITY);
        }
    ...
    pub fn fire_arrow(player: &Player, power: f32, projectile: &ItemStack) {
        ...
        if let Some(enchantments) =
            stack.get_data_component::<haskmc_data::data_component_impl::EnchantmentsImpl>()
        {
            for (enchantment, level) in enchantments.enchantment.iter() {
                if **enchantment == haskmc_data::Enchantment::POWER {
                    arrow.base_damage *= 1.0 + 0.25 * (f64::from(*level) + 1.0);
                } else if **enchantment == haskmc_data::Enchantment::PUNCH {
                    arrow.punch_level.store(*level as u8, Ordering::Relaxed);
                } else if **enchantment == haskmc_data::Enchantment::FLAME {
