...
impl ItemBehaviour for SplashPotionItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        let mut stack = (!main_s.is_empty()
            && main_s.item.id == haskmc_data::item::Item::SPLASH_POTION.id)
            .then_some(main_s);
        if stack.is_none() {
            let off_s = player.inventory.off_hand_item();
            if !off_s.is_empty() && off_s.item.id == haskmc_data::item::Item::SPLASH_POTION.id {
                stack = Some(off_s);
                used_main = false;
            }
        }
        ...
        if used_main {
            ...
            player
                .inventory
                .set_stack_in_hand(haskmc_util::Hand::Left, s);
        }
    }
...
impl ItemBehaviour for LingeringPotionItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        let mut used_main = true;
        let mut stack = (!main_s.is_empty()
            && main_s.item.id == haskmc_data::item::Item::LINGERING_POTION.id)
            .then_some(main_s);
        if stack.is_none() {
            let off_s = player.inventory.off_hand_item();
            if !off_s.is_empty() && off_s.item.id == haskmc_data::item::Item::LINGERING_POTION.id {
                stack = Some(off_s);
                used_main = false;
            }
        }
