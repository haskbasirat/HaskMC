...
impl ItemBehaviour for TridentItem {
    ...
    fn on_stopped_using(&self, _stack: &ItemStack, player: &Player) {
        ...
        if let Some(enchantments) =
            stack_guard.get_data_component::<haskmc_data::data_component_impl::EnchantmentsImpl>()
        {
            for (enchantment, level) in enchantments.enchantment.iter() {
                if **enchantment == haskmc_data::Enchantment::RIPTIDE {
        ...
        if player.gamemode.load() != GameMode::Creative {
            ...
            if main_hand_item.item.id == Item::TRIDENT.id {
                ...
                let off_hand_slot =
                    haskmc_inventory::player::player_inventory::PlayerInventory::OFF_HAND_SLOT;
