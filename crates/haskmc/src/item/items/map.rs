...
use haskmc_data::data_component_impl::DataComponentImpl;
    ...
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        let (found, mut hand_stack, hand) =
            if !held_stack.is_empty() && held_stack.item.id == Item::MAP.id {
                (true, held_stack, haskmc_util::Hand::Right)
            } else {
                let off_hand = inventory.off_hand_item();
                if !off_hand.is_empty() && off_hand.item.id == Item::MAP.id {
                    (true, off_hand, haskmc_util::Hand::Left)
                } else {
                    (false, held_stack, haskmc_util::Hand::Right)
                }
            };
