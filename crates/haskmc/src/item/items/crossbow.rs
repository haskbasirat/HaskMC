...
use haskmc_data::data_component_impl::{ChargedProjectilesImpl, EnchantmentsImpl};
...
impl ItemBehaviour for CrossbowItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        player
            .living_entity
            .set_active_hand(haskmc_util::Hand::Right, stack, 72000);
    }
    ...
    fn on_stopped_using(&self, _stack: &ItemStack, player: &Player) {
        ...
        if let Some(enchantments) = stack.get_data_component::<EnchantmentsImpl>() {
            for (enchantment, level) in enchantments.enchantment.iter() {
                if **enchantment == haskmc_data::Enchantment::QUICK_CHARGE {
                    charge_time -= 5 * level;
                }
            }
        }
            ...
            let (arrow_nbt_wrapper, slot) = arrow_slot.map_or_else(
                || {
                    if player.gamemode.load() == GameMode::Creative {
                        let mut arrow_nbt = haskmc_nbt::compound::NbtCompound::new();
            ...
            held.get_data_component::<EnchantmentsImpl>()
                .is_some_and(|enchantments| {
                    enchantments
                        .enchantment
                        .iter()
                        .any(|(e, _)| **e == haskmc_data::Enchantment::MULTISHOT)
                });
