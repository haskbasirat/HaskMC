...
impl PluginLoader for NativePluginLoader {
    fn load<'a>(&'a self, path: &'a Path) -> PluginLoadFuture<'a> {
        Box::pin(async {
            ...
            // Ensure this plugin was built against a compatible HaskMC/Pumpkin plugin API version.
            // SAFETY: The API version is an exported `u32` constant created by `#[plugin_impl]`.
            let plugin_api_version = unsafe {
                match library
                    .get::<*const u32>(b"HASKMC_API_VERSION")
                    .or_else(|_| library.get::<*const u32>(b"PUMPKIN_API_VERSION"))
                {
                    Ok(symbol) => **symbol,
