...
impl pumpkin::plugin::server::HostServer for PluginHostState {
    ...
    async fn get_difficulty(&mut self, res: Resource<Server>) -> wasmtime::Result<Difficulty> {
        ...
        Ok(match resource.provider.get_difficulty() {
            haskmc_util::Difficulty::Peaceful => Difficulty::Peaceful,
            haskmc_util::Difficulty::Easy => Difficulty::Easy,
            haskmc_util::Difficulty::Normal => Difficulty::Normal,
            haskmc_util::Difficulty::Hard => Difficulty::Hard,
        })
    }
    ...
    async fn create_world(
        ...
        let internal_dim = match dimension {
            Dimension::Overworld => haskmc_data::dimension::Dimension::OVERWORLD,
            Dimension::Nether => haskmc_data::dimension::Dimension::THE_NETHER,
            Dimension::End => haskmc_data::dimension::Dimension::THE_END,
        };
    ...
    async fn get_all_advancement_ids(
        ...
        let ids = haskmc_data::Advancement::get_identifier_list()
            .iter()
    ...
    async fn get_all_enchantment_ids(
        ...
        for enc in haskmc_data::enchantment::Enchantment::ALL {
    ...
    async fn op_player(
...

        {
            ...
            if let Some(existing) = config.ops.iter_mut().find(|o| o.uuid == uuid) {
                ...
                let op_entry =
                    haskmc_config::op::Op::new(uuid, name, internal_level, bypasses_player_limit);
        ...
        if removed {
            if let Some(player) = server.get_player_by_uuid(uuid) {
                ...
                player.set_permission_lvl(
                    server,
                    haskmc_util::PermissionLvl::Zero,
                    &command_dispatcher,
                );
            }
    ...
    async fn ban_player(
        ...
        let reason_text = options
            ...
            .map_or_else(
                ...
                haskmc_util::text::TextComponent::to_pretty_console,
            );
            ...
            && let Some(player) = server.get_player_by_uuid(uuid)
        {
            player.kick(
                ...
                &haskmc_util::text::TextComponent::text(reason_text.clone()),
            );
        }
...
impl pumpkin::plugin::server::HostWhitelistManager for PluginHostState {
            ...
            for player in to_kick {
                player.kick(
                    ...
                    &haskmc_macros::translate_cross!(
                        haskmc_data::translation::java::MULTIPLAYER_DISCONNECT_NOT_WHITELISTED,
                        haskmc_data::translation::bedrock::DISCONNECT_KICKED
                    ),
                );
            }
        }
    ...
    async fn add_player(
        ...
        if config.whitelist.iter().any(|e| e.uuid == uuid) {
            ...
            config
                .whitelist
                .push(haskmc_config::whitelist::WhitelistEntry::new(uuid, name));
            config.save();
            Ok(true)
        }
    }
