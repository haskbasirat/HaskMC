    ...
    fn from_wasm_event(event: Event, state: &mut PluginHostState) -> Self {
        match event {
            Event::ChunkLoadEvent(data) => {
                ...
                let chunk_data = haskmc_world::chunk::ChunkData {
                    section: haskmc_world::chunk::ChunkSections::new(24, -64),
                    heightmap: std::sync::Mutex::new(
                        haskmc_world::chunk::ChunkHeightmaps::default(),
                    ),
                    x: data.chunk_x,
                    z: data.chunk_z,
                    block_ticks: haskmc_world::tick::scheduler::ChunkTickScheduler::default(),
                    fluid_ticks: haskmc_world::tick::scheduler::ChunkTickScheduler::default(),
                    pending_block_entities: std::sync::Mutex::new(
                        std::collections::HashMap::default(),
                    ),
                    light_engine: std::sync::Mutex::new(haskmc_world::chunk::ChunkLight::default()),
                    light_populated: std::sync::atomic::AtomicBool::new(false),
                    status: haskmc_data::chunk::ChunkStatus::Empty,
                    blending_data: None,
                    dirty: std::sync::atomic::AtomicBool::new(false),
                    inhabited_time: std::sync::atomic::AtomicU64::new(0),
                    custom_data: std::sync::Mutex::new(haskmc_nbt::compound::NbtCompound::new()),
                };
                Self {
                    world,
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::ChunkPopulateEvent(data) => Self {
                chunk_pos: haskmc_util::math::vector2::Vector2::new(data.chunk_x, data.chunk_z),
                cancelled: data.cancelled,
            },
            _ => panic!("unexpected event type"),
        }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::GenericGameEvent(data) => Self {
                ...
                position: haskmc_util::math::vector3::Vector3::new(
