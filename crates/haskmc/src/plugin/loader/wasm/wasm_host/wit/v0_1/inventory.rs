...
const fn from_wasm_hand(hand: WitHand) -> haskmc_util::Hand {
    match hand {
        WitHand::Right => haskmc_util::Hand::Right,
        WitHand::Left => haskmc_util::Hand::Left,
    }
}
        ...
        match provider {
            ...
            InventoryProvider::PlayerMain(player) => {
                for slot in 0..36 {
                    player
                        .inventory()
                        .set_stack(slot, haskmc_data::item_stack::ItemStack::EMPTY.clone());
                    let empty_serializer = ItemStackSerializer::from(
                        haskmc_data::item_stack::ItemStack::EMPTY.clone(),
                    );
                    let packet = CSetContainerSlot::new(0, 0, slot as i16, &empty_serializer);
