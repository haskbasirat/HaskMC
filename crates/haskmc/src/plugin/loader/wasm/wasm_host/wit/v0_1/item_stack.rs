...
use haskmc_data::data_component_impl::{CustomNameImpl, EnchantmentsImpl, LoreImpl};
use haskmc_nbt::tag::NbtTag;
use haskmc_protocol::codec::data_component::{deserialize, serialize};
use std::borrow::Cow;
...
impl PluginHostState {
    ...
    ) -> wasmtime::Result<Arc<Mutex<haskmc_data::item_stack::ItemStack>>> {
        self.resource_table
...
impl HostItemStack for PluginHostState {
    async fn new(
        ...
        let item = haskmc_data::item::Item::from_registry_key(
            registry_key
                .strip_prefix("minecraft:")
                .unwrap_or(&registry_key),
        )
        .unwrap_or(&haskmc_data::item::Item::AIR);
        let stack = haskmc_data::item_stack::ItemStack::new(count, item);
    ...
    async fn get_max_count(&mut self, res: Resource<ItemStackHandle>) -> wasmtime::Result<u8> {
        ...
        if let Some((_, data)) = stack
            ...
            && let Some(max_size) = data
                .as_any()
                .downcast_ref::<haskmc_data::data_component_impl::MaxStackSizeImpl>()
        {
