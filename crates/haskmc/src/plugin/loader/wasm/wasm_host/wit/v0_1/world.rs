use haskmc_data::block_properties::NoteblockInstrument as InternalNoteblockInstrument;
use haskmc_data::block_state::PistonBehavior;
use haskmc_data::{BlockDirection as InternalBlockDirection, BlockId, BlockStateId};
use haskmc_util::math::position::BlockPos;
use haskmc_world::chunk::ChunkHeightmapType;
use haskmc_world::chunk::io::Dirtiable;
...
pub(crate) fn from_wit_game_rule(rule: WitGameRule) -> GameRule {
    // SAFETY: WIT GameRule and haskmc_data::game_rules::GameRule have identical variant order
...
pub(crate) const fn to_wit_bounding_box(
    bb: haskmc_util::math::boundingbox::BoundingBox,
) -> WitBoundingBox {
    WitBoundingBox {
...
pub(crate) fn to_wit_block(block: &haskmc_data::Block) -> WitBlock {
    WitBlock {
        id: block.id.as_u16(),
...
pub(crate) fn to_wit_block_state(
    state: &haskmc_data::BlockState,
    pos: Option<&BlockPos>,
) -> WitBlockState {
    let dummy_pos = BlockPos::new(0, 0, 0);
    let internal_pos = pos.unwrap_or(&dummy_pos);
    let block = haskmc_data::Block::from_state_id(state.id);
...
impl PluginHostState {
    ...
    ) -> wasmtime::Result<haskmc_util::text::TextComponent> {
        Ok(self
            .resource_table
...
impl pumpkin::plugin::world::Host for PluginHostState {
    async fn resolve_block_state(
        ...
        let Some(block) = haskmc_data::Block::from_name(&name) else {
            return Ok(None);
        };
    ...
    async fn block_state_to_info(
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            let block = haskmc_data::Block::from_state_id(bsid);
    ...
    async fn get_block_by_id(&mut self, id: u16) -> wasmtime::Result<Option<WitBlock>> {
        ...
        Ok(block_id.map(|id| to_wit_block(haskmc_data::Block::from_id(id))))
    }

    async fn get_block_by_name(&mut self, name: String) -> wasmtime::Result<Option<WitBlock>> {
        Ok(haskmc_data::Block::from_name(&name).map(to_wit_block))
    }
    ...
    async fn get_all_blocks(&mut self) -> wasmtime::Result<Vec<WitBlock>> {
        ...
        for raw_id in 0..BlockId::COUNT {
            if let Some(id) = BlockId::new(raw_id) {
                blocks.push(to_wit_block(haskmc_data::Block::from_id(id)));
            }
        }
        Ok(blocks)
    }
    ...
    async fn get_all_block_names(&mut self) -> wasmtime::Result<Vec<String>> {
        ...
        for raw_id in 0..BlockId::COUNT {
            if let Some(id) = BlockId::new(raw_id) {
                names.push(haskmc_data::Block::from_id(id).name.to_string());
            }
        }
        Ok(names)
    }
    ...
    async fn get_states_for_block(
        ...
        let block_ref = haskmc_data::Block::from_id(block_id);
        Ok(block_ref
            .states
            .iter()
    ...
    async fn get_states_for_block_id(
        ...
        let block_ref = haskmc_data::Block::from_id(id);
        Ok(block_ref
            .states
            .iter()
            .map(|s| to_wit_block_state(s, None))
    ...
    async fn get_block_from_state_id(
        ...
        Ok(bsid.map(|id| to_wit_block(haskmc_data::Block::from_state_id(id))))
    }
    ...
    async fn get_block_from_state(&mut self, state: WitBlockState) -> wasmtime::Result<WitBlock> {
        ...
        Ok(to_wit_block(haskmc_data::Block::from_state_id(bsid)))
    }
    ...
    async fn get_default_state_from_block_id(
        ...
        Ok(block_id.map(|id| {
            let block = haskmc_data::Block::from_id(id);
    ...
    async fn get_block_state_by_id(
        ...
        let bsid = BlockStateId::new(state_id);
        Ok(bsid.map(|id| {
            let state = haskmc_data::BlockState::from_id(id);
...
impl pumpkin::plugin::world::HostWorld for PluginHostState {
    ...
    async fn get_chunk(
        ...
        let pos = haskmc_util::math::vector2::Vector2::new(x, z);
    ...
    async fn get_block_id(
        ...
        let internal_pos = BlockPos::new(pos.x, pos.y, pos.z);
        let state = world_ref.provider.get_block_state(&internal_pos);
        Ok(haskmc_data::BlockId::from_state_id(state.id).as_u16())
    }
    ...
    async fn set_block(
        ...
        let block_id = BlockId::new_or_air(block.id);
        let default_state_id = haskmc_data::Block::from_id(block_id)
            .default_state
            .id
            .as_u16();
    ...
    async fn set_block_by_id(
        ...
        let default_state_id = haskmc_data::Block::from_id(id).default_state.id.as_u16();
        self.set_block_state(world, pos, default_state_id, update_flags)
            .await
    }
    ...
    async fn set_block_by_name(
        ...
        if let Some(block) = haskmc_data::Block::from_name(&name) {
    ...
    async fn get_top_block_y(
        ...
        Ok(self
            .get_world_res(&world)?
            .provider
            .get_top_block(haskmc_util::math::vector2::Vector2::new(x, z)))
    }
    ...
    async fn play_sound(
        ...
        let sound_data = haskmc_data::sound::Sound::from_name(&sound_name)
        ...
        world_ref.provider.play_sound_raw(
            sound_data as u16,
            internal_category,
            &haskmc_util::math::vector3::Vector3::new(pos.0, pos.1, pos.2),
            volume,
            pitch,
        );
        Ok(())
    ...
    async fn spawn_particle(
        ...
        let world_ref = self.get_world_res(&world)?;
        let particle_data =
            haskmc_data::particle::Particle::from_id(particle as u16).ok_or_else(|| {
                wasmtime::Error::msg(format!("Unknown particle ID: {}", particle as u16))
            })?;

        world_ref.provider.spawn_particle(
            haskmc_util::math::vector3::Vector3::new(pos.0, pos.1, pos.2),
            haskmc_util::math::vector3::Vector3::new(
                offset.0 as f32,
                offset.1 as f32,
                offset.2 as f32,
            ),
    ...
    async fn spawn_entity(
        ...
        let internal_type = haskmc_data::entity::EntityType::from_name(type_name)
            .ok_or_else(|| wasmtime::Error::msg(format!("Invalid entity type: {type_name}")))?;

        let internal_pos = haskmc_util::math::vector3::Vector3::new(pos.0, pos.1, pos.2);
    ...
    async fn ray_trace_blocks(
        ...
        Ok(res.map(|(p, _)| {
            super::events::to_wasm_position(haskmc_util::math::vector3::Vector3::new(
                f64::from(p.0.x),
                f64::from(p.0.y),
                f64::from(p.0.z),
            ))
    ...
    async fn get_block_entity_nbt(
        ...
        let mut nbt = haskmc_nbt::NbtCompound::new();
    ...
    async fn set_block_entity_nbt(
        ...
        let mut reader = haskmc_nbt::deserializer::NbtReadHelperJava::new(
            haskmc_nbt::deserializer::NbtStreamReader(&mut cursor),
        );
        let mut nbt: haskmc_nbt::NbtCompound = haskmc_nbt::Nbt::read_unnamed(&mut reader)
    ...
    async fn set_chunk_generator(
        ...
        world_ref.level.set_world_gen(Arc::new(
            haskmc_world::generation::generator::WorldGenerator::Custom(wasm_gen),
        ));
        Ok(())
    }
...
impl pumpkin::plugin::world::HostChunk for PluginHostState {
    ...
    async fn get_block(
        ...
        let id = chunk_data
            ...
            .get_block_absolute_y(pos.x as usize, pos.y, pos.z as usize)
            .unwrap_or(BlockStateId::AIR);
        let block = haskmc_data::Block::from_state_id(id);
        Ok(to_wit_block(block))
    }
    ...
    async fn get_biome(
        ...
        let id = chunk_data
            ...
            .unwrap_or(0);
        let biome =
            haskmc_data::biome::Biome::from_id(id).unwrap_or(&haskmc_data::biome::Biome::THE_VOID);
...
impl pumpkin::plugin::world::HostWorldBorder for PluginHostState {
    ...
    async fn get_center(
        ...
        Ok(super::events::to_wasm_position(
            haskmc_util::math::vector3::Vector3::new(guard.center_x, 0.0, guard.center_z),
        ))
    }
...
impl pumpkin::plugin::world::HostChunkBuffer for PluginHostState {
    ...
    async fn set_block_state_id(
        ...
        if x < 16 && z < 16 {
            let world_x =
                haskmc_world::generation::positions::chunk_pos::start_block_x(res.provider.x)
                    + x as i32;
            let world_z =
                haskmc_world::generation::positions::chunk_pos::start_block_z(res.provider.z)
                    + z as i32;
            ...
            let block_state = haskmc_data::BlockState::from_id(
                haskmc_data::BlockStateId::new(state_id).unwrap_or(haskmc_data::BlockStateId::AIR),
            );
    ...
    async fn fill_layer(
        ...
        let res = self.get_chunk_buffer_res(&this)?;
        let start_x = haskmc_world::generation::positions::chunk_pos::start_block_x(res.provider.x);
        let start_z = haskmc_world::generation::positions::chunk_pos::start_block_z(res.provider.z);
        let block_state = haskmc_data::BlockState::from_id(
            haskmc_data::BlockStateId::new(state_id).unwrap_or(haskmc_data::BlockStateId::AIR),
        );
        // SAFETY: `proto_chunk` points to a valid proto chunk allocated for world generation and is not aliased across threads.
...
pub struct WasmChunkGenerator {
    ...
    pub dimension: haskmc_data::dimension::Dimension,
    pub seed: u64,
}
...
impl WasmChunkGenerator {
    fn invoke_phase(
        ...
        proto_chunk: &mut haskmc_world::ProtoChunk,
    ) {
...
impl haskmc_world::generation::generator::CustomChunkGenerator for WasmChunkGenerator {
    fn dimension(&self) -> &haskmc_data::dimension::Dimension {
        &self.dimension
    }
    ...
    fn step_to_biomes(&self, chunk: &mut haskmc_world::ProtoChunk) {
        self.invoke_phase(pumpkin::plugin::world::GenerationPhase::Biomes, chunk);
        chunk.stage = haskmc_world::chunk_system::StagedChunkEnum::Biomes;
    }

    fn step_to_noise(&self, chunk: &mut haskmc_world::ProtoChunk) {
        self.invoke_phase(pumpkin::plugin::world::GenerationPhase::Noise, chunk);
        chunk.stage = haskmc_world::chunk_system::StagedChunkEnum::Noise;
    }

    fn step_to_surface(&self, chunk: &mut haskmc_world::ProtoChunk) {
        self.invoke_phase(pumpkin::plugin::world::GenerationPhase::Surface, chunk);
        chunk.stage = haskmc_world::chunk_system::StagedChunkEnum::Surface;
    }

    fn step_to_carvers(&self, chunk: &mut haskmc_world::ProtoChunk) {
        chunk.stage = haskmc_world::chunk_system::StagedChunkEnum::Carvers;
    }

    fn step_to_features(
        &self,
        cache: &mut haskmc_world::chunk_system::generation_cache::Cache,
        _block_registry: &dyn haskmc_world::world::WorldPortalExt,
    ) {
        ...
        chunk.stage = haskmc_world::chunk_system::StagedChunkEnum::Features;
    }
}
    ...
    fn wit_particle_ids_match_internal_particle_ids() {
        let cases = [
            (
                ...
                haskmc_data::particle::Particle::AngryVillager,
            ),
            (
                pumpkin::plugin::particles::Particle::HappyVillager,
                haskmc_data::particle::Particle::HappyVillager,
            ),
            (
                pumpkin::plugin::particles::Particle::SulfurCubeGoo,
                haskmc_data::particle::Particle::SulfurCubeGoo,
            ),
        ];

        for (wit, internal) in cases {
            assert_eq!(
                haskmc_data::particle::Particle::from_id(wit as u16),
                Some(internal)
            );
        }
    }
