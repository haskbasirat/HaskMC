...
#[must_use]
pub const fn from_wit_attribute(attr: Attribute) -> &'static haskmc_data::attributes::Attributes {
...
) -> haskmc_data::data_component_impl::EquipmentSlot {
...
#[must_use]
pub const fn to_wit_damage_type(damage_type: &haskmc_data::damage::DamageType) -> WitDamageType {
    // SAFETY: WIT enum is generated in the same order as the internal enum / id
    unsafe { std::mem::transmute(damage_type.id) }
}

#[must_use]
pub fn from_wit_damage_type(wit: WitDamageType) -> haskmc_data::damage::DamageType {
    haskmc_data::damage::DamageType::from_id(wit as u8)
        .unwrap_or(haskmc_data::damage::DamageType::GENERIC)
}
    ...
    async fn clear_equipment(&mut self, this: Resource<WitLivingEntity>) -> wasmtime::Result<()> {
        ...
        if let Some(living) = entity.get_living_entity() {
            ...
            let slots_to_clear: Vec<(
                haskmc_data::data_component_impl::EquipmentSlot,
                haskmc_data::item_stack::ItemStack,
            )> = equipment
                .equipment
                .drain()
                .map(|(slot, _)| (slot, haskmc_data::item_stack::ItemStack::EMPTY.clone()))
                .collect();
            drop(equipment);
