//! # `HaskMC` Plugin Utilities (`haskmc-plugin-utils`)
//!
//! Utilities for `HaskMC` plugins that use the inherited Pumpkin Marketplace compatibility layer:
...
//!
//! ```rust,ignore
//! use haskmc_plugin_api::{Plugin, Context, register_plugin};
//! use haskmc_plugin_utils::{init, check_license_online, check_for_updates};
//!
//! struct MyPlugin;
//!
...
//!         let metadata = haskmc_plugin_utils::init(context)
//!             .map_err(|e| format!("Plugin initialization failed: {e}"))?;
//!
//!         // 2. Check license online against marketplace
//!         let license_check = haskmc_plugin_utils::check_license_online(None)
...
//!         if let Ok(update) = haskmc_plugin_utils::check_for_updates() {
...
/// Initializes `haskmc-plugin-utils` using the plugin's runtime `Context`.
///
...
pub fn init(
    context: &haskmc_plugin_api::Context,
...
/// Initializes `haskmc-plugin-utils` with explicit metadata (useful for tests or custom initialization).
///
/// # Errors
///
...
pub fn evaluate_license(grace_period_days: u32) -> LicenseStatus {
    let Some(folder) = get_data_folder() else {
        return LicenseStatus::Invalid("haskmc_plugin_utils has not been initialized".to_string());
    };
