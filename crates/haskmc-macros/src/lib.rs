...
use haskmc_data::tag::{RegistryKey, get_tag_ids};
...
pub fn java_packet(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        impl #impl_generics crate::packet::MultiVersionJavaPacket for #name #ty_generics #where_clause {
            #[must_use]
            #[inline]
            fn to_id(version: haskmc_util::version::JavaMinecraftVersion) -> i32 {
...
#[proc_macro_attribute]
pub fn haskmc_block(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let generated = quote! {
        impl #impl_generics crate::block::BlockMetadata for #name #ty_generics #where_clause {
            fn ids() -> Box<[haskmc_data::BlockId]> {
                [haskmc_data::BlockId::#const_ident].into()
            }
        }
    };
...
/// - `item` – The input `TokenStream` representing the struct to implement `BlockMetadata` for.
#[proc_macro_attribute]
pub fn haskmc_block_from_tag(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics crate::block::BlockMetadata for #name #ty_generics #where_clause {
            fn ids() -> Box<[haskmc_data::BlockId]> {
                Box::new([ #(haskmc_data::BlockId::#const_values),* ])
            }
        }
    };
...
//     let code = quote! {
//         #item
//         impl #impl_generics haskmc_world::block::properties::BlockPropertyMetadata for #name #ty_generics {
    ...
    match expr {
        syn::Expr::Path(expr_path) => {
            ...
            let (is_java, const_ident) = match seg_refs.as_slice() {
                ...
                | ["haskmc_data" | "crate", "translation", "java", ident] => (true, *ident),
                ["translation", "bedrock", ident]
                | ["haskmc_data" | "crate", "translation", "bedrock", ident] => (false, *ident),
                _ => return None,
            };

            let key = if is_java {
                haskmc_data::translation::java::get(const_ident)
                    .and_then(haskmc_data::translation::java::get_value)
            } else {
                haskmc_data::translation::bedrock::get(const_ident)
                    .and_then(haskmc_data::translation::bedrock::get_value)
            };
...
pub fn translate_cross(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        {
            #[allow(deprecated)]
            haskmc_util::text::TextComponent::translate_cross(#java_expr, #bedrock_expr, vec![#(#args),*])
        }
    };
...
#[deprecated(
    since = "0.1.0",
    note = "Use `haskmc_macros::translate_cross!` macro instead for cross-platform (Java & Bedrock) translation support."
)]
#[proc_macro]
pub fn translate_java(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        {
            #[allow(deprecated)]
            haskmc_util::text::TextComponent::translate(#java_expr, vec![#(#args),*])
        }
    };
