...
//!
//! This crate provides everything needed to write a `HaskMC` server plugin compiled
//! to WebAssembly. It intentionally retains Pumpkin's `pumpkin:plugin@0.1.0` WIT
//! namespace so existing plugins remain ABI-compatible. A plugin consists of a type
//! that implements [`Plugin`], registered with the [`register_plugin!`] macro.
//!
//! # Quick start
//!
//! ```rust,ignore
//! use haskmc_plugin_api::{Plugin, PluginMetadata, Context, register_plugin, permissions::permissions};
//!
//! struct MyPlugin;
//!
...
/// The trait that every `HaskMC` plugin must implement.
///
...
/// Registers the provided type as a `HaskMC` plugin.
///
