...
static HASKMC_EN_US_JSON: &str = include_str!("../../../assets/translations/en_us.json");
static HASKMC_BRB_JSON: &str = include_str!("../../../assets/translations/brb.json");
static HASKMC_DE_DE_JSON: &str = include_str!("../../../assets/translations/de_de.json");
static HASKMC_ES_ES_JSON: &str = include_str!("../../../assets/translations/es_es.json");
static HASKMC_FR_FR_JSON: &str = include_str!("../../../assets/translations/fr_fr.json");
static HASKMC_HR_HR_JSON: &str = include_str!("../../../assets/translations/hr_hr.json");
static HASKMC_IT_IT_JSON: &str = include_str!("../../../assets/translations/it_it.json");
static HASKMC_JA_JP_JSON: &str = include_str!("../../../assets/translations/ja_jp.json");
static HASKMC_KA_GE_JSON: &str = include_str!("../../../assets/translations/ka_ge.json");
static HASKMC_KO_KR_JSON: &str = include_str!("../../../assets/translations/ko_kr.json");
static HASKMC_NDS_DE_JSON: &str = include_str!("../../../assets/translations/nds_de.json");
static HASKMC_NL_BE_JSON: &str = include_str!("../../../assets/translations/nl_be.json");
static HASKMC_NL_NL_JSON: &str = include_str!("../../../assets/translations/nl_nl.json");
static HASKMC_RO_RO_JSON: &str = include_str!("../../../assets/translations/ro_ro.json");
static HASKMC_RU_RU_JSON: &str = include_str!("../../../assets/translations/ru_ru.json");
static HASKMC_SQ_AL_JSON: &str = include_str!("../../../assets/translations/sq_al.json");
static HASKMC_ZH_CN_JSON: &str = include_str!("../../../assets/translations/zh_cn.json");
static HASKMC_ZH_HK_JSON: &str = include_str!("../../../assets/translations/zh_hk.json");
static HASKMC_ZH_TW_JSON: &str = include_str!("../../../assets/translations/zh_tw.json");
static HASKMC_LZH_JSON: &str = include_str!("../../../assets/translations/lzh.json");
static HASKMC_TR_TR_JSON: &str = include_str!("../../../assets/translations/tr_tr.json");
static HASKMC_UK_UA_JSON: &str = include_str!("../../../assets/translations/uk_ua.json");
static HASKMC_VI_VN_JSON: &str = include_str!("../../../assets/translations/vi_vn.json");
static HASKMC_PT_BR_JSON: &str = include_str!("../../../assets/translations/pt_br.json");
static HASKMC_PL_PL_JSON: &str = include_str!("../../../assets/translations/pl_pl.json");
...
pub static TRANSLATIONS: LazyLock<Mutex<[HashMap<String, String>; Locale::COUNT]>> =
    LazyLock::new(|| {
        ...
        let haskmc_en_us = parse_json(HASKMC_EN_US_JSON);
        let haskmc_brb = parse_json(HASKMC_BRB_JSON);
        let haskmc_de_de = parse_json(HASKMC_DE_DE_JSON);
        let haskmc_es_es = parse_json(HASKMC_ES_ES_JSON);
        let haskmc_fr_fr = parse_json(HASKMC_FR_FR_JSON);
        let haskmc_hr_hr = parse_json(HASKMC_HR_HR_JSON);
        let haskmc_it_it = parse_json(HASKMC_IT_IT_JSON);
        let haskmc_ja_jp = parse_json(HASKMC_JA_JP_JSON);
        let haskmc_ka_ge = parse_json(HASKMC_KA_GE_JSON);
        let haskmc_ko_kr = parse_json(HASKMC_KO_KR_JSON);
        let haskmc_nds_de = parse_json(HASKMC_NDS_DE_JSON);
        let haskmc_nl_be = parse_json(HASKMC_NL_BE_JSON);
        let haskmc_nl_nl = parse_json(HASKMC_NL_NL_JSON);
        let haskmc_ro_ro = parse_json(HASKMC_RO_RO_JSON);
        let haskmc_ru_ru = parse_json(HASKMC_RU_RU_JSON);
        let haskmc_sq_al = parse_json(HASKMC_SQ_AL_JSON);
        let haskmc_zh_cn = parse_json(HASKMC_ZH_CN_JSON);
        let haskmc_zh_hk = parse_json(HASKMC_ZH_HK_JSON);
        let haskmc_zh_tw = parse_json(HASKMC_ZH_TW_JSON);
        let haskmc_lzh = parse_json(HASKMC_LZH_JSON);
        let haskmc_tr_tr = parse_json(HASKMC_TR_TR_JSON);
        let haskmc_uk_ua = parse_json(HASKMC_UK_UA_JSON);
        let haskmc_vi_vn = parse_json(HASKMC_VI_VN_JSON);
        let haskmc_pt_br = parse_json(HASKMC_PT_BR_JSON);
        let haskmc_pl_pl = parse_json(HASKMC_PL_PL_JSON);

        for (key, value) in vanilla_en_us {
            array[Locale::EnUs as usize].insert(format!("minecraft:{key}"), value);
        }
        for (key, value) in haskmc_en_us {
            array[Locale::EnUs as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_brb {
            array[Locale::Brb as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_de_de {
            array[Locale::DeDe as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_es_es {
            array[Locale::EsEs as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_fr_fr {
            array[Locale::FrFr as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_hr_hr {
            array[Locale::HrHr as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_it_it {
            array[Locale::ItIt as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_ja_jp {
            array[Locale::JaJp as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_ka_ge {
            array[Locale::KaGe as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_ko_kr {
            array[Locale::KoKr as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_nds_de {
            array[Locale::NdsDe as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_nl_be {
            array[Locale::NlBe as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_nl_nl {
            array[Locale::NlNl as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_ro_ro {
            array[Locale::RoRo as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_ru_ru {
            array[Locale::RuRu as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_sq_al {
            array[Locale::SqAl as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_zh_cn {
            array[Locale::ZhCn as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_zh_hk {
            array[Locale::ZhHk as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_zh_tw {
            array[Locale::ZhTw as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_lzh {
            array[Locale::Lzh as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_tr_tr {
            array[Locale::TrTr as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_uk_ua {
            array[Locale::UkUa as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_vi_vn {
            array[Locale::ViVn as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_pt_br {
            array[Locale::PtBr as usize].insert(format!("haskmc:{key}"), value);
        }
        for (key, value) in haskmc_pl_pl {
            array[Locale::PlPl as usize].insert(format!("haskmc:{key}"), value);
        }
