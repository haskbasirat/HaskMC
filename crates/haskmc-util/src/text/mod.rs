...
impl TextComponentBase {
    ...
    #[must_use]
    pub fn to_nbt_compound(&self) -> haskmc_nbt::NbtCompound {
    ...
    pub fn to_nbt_compound_for_version(
    ...
    ) -> haskmc_nbt::NbtCompound {
        let mut compound = haskmc_nbt::NbtCompound::new();
        match &*self.content {
        ...
        if let Some(ref click) = self.style.click_event {
            let mut click_tag = haskmc_nbt::NbtCompound::new();
            match click {
        ...
        if let Some(ref hover) = self.style.hover_event {
            let mut hover_tag = haskmc_nbt::NbtCompound::new();
            if *version >= JavaMinecraftVersion::V_1_21_5 {
                ...
                match hover {
                    ...
                    HoverEvent::ShowItem { id, count } => {
                        hover_tag.put_string("action", "show_item".to_string());
                        let mut contents = haskmc_nbt::NbtCompound::new();
    ...
    pub fn to_nbt_tag_for_version(
        &self,
        version: &JavaMinecraftVersion,
    ) -> haskmc_nbt::tag::NbtTag {
        if *version >= JavaMinecraftVersion::V_1_20_3
            && self.style.is_empty()
            && self.extra.is_empty()
            && let TextContent::Text { text } = &*self.content
        {
            haskmc_nbt::tag::NbtTag::String(text.to_string().into_boxed_str())
        } else {
            haskmc_nbt::tag::NbtTag::Compound(self.to_nbt_compound_for_version(version))
        }
    }
...
fn nbt_compound_to_json(compound: &haskmc_nbt::NbtCompound) -> serde_json::Value {
    let mut map = serde_json::Map::new();
...
fn nbt_tag_to_json(tag: &haskmc_nbt::tag::NbtTag) -> serde_json::Value {
    match tag {
        haskmc_nbt::tag::NbtTag::End => serde_json::Value::Null,
        haskmc_nbt::tag::NbtTag::Byte(b) => serde_json::json!(*b),
        haskmc_nbt::tag::NbtTag::Short(s) => serde_json::json!(*s),
        haskmc_nbt::tag::NbtTag::Int(i) => serde_json::json!(*i),
        haskmc_nbt::tag::NbtTag::Long(l) => serde_json::json!(*l),
        haskmc_nbt::tag::NbtTag::Float(f) => serde_json::json!(*f),
        haskmc_nbt::tag::NbtTag::Double(d) => serde_json::json!(*d),
        haskmc_nbt::tag::NbtTag::ByteArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
        haskmc_nbt::tag::NbtTag::String(s) => serde_json::Value::String(s.to_string()),
        haskmc_nbt::tag::NbtTag::List(list) => {
            serde_json::Value::Array(list.iter().map(nbt_tag_to_json).collect())
        }
        haskmc_nbt::tag::NbtTag::Compound(c) => nbt_compound_to_json(c),
        haskmc_nbt::tag::NbtTag::IntArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
        haskmc_nbt::tag::NbtTag::LongArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
    }
}
...
impl TextComponent {
    ...
    #[deprecated(
        since = "0.1.0",
        note = "Use the `haskmc_macros::translate_java!` macro instead for compile-time translation key and parameter checking."
    )]
    #[must_use]
    ...
    #[deprecated(
        since = "0.1.0",
        note = "Use the `haskmc_macros::translate_cross!` macro instead for compile-time translation key and parameter checking."
    )]
    #[must_use]
    ...
    ///
    /// # Arguments
    /// - `namespace` – The namespace for the translation (e.g. "exampleplugin").
...
impl TextComponent {
    ...
    #[must_use]
    pub fn player_sprite(profile: haskmc_nbt::NbtCompound, hat: bool) -> Self {
        Self(TextComponentBase {
    ...
    pub fn encode_for_version(&self, version: &JavaMinecraftVersion) -> Box<[u8]> {
        ...
        let mut writer = haskmc_nbt::serializer::NbtWriteHelperJava::new(&mut bytes);
...
pub struct ProfileNbt(pub haskmc_nbt::NbtCompound);
...
pub enum TextContent {
    ...
    #[serde(skip)]
    Custom {
        /// The full translation key with namespace (e.g. "exampleplugin:some.text").
        key: Cow<'static, str>,
...
mod test {
    ...
    fn serialize_text_component() {
        ...
        let decoded = haskmc_nbt::Nbt::read_unnamed(&mut reader).unwrap();
