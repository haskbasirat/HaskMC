...
mod tests {
    ...
    /// `Legacy`. These sequences are pinned from `HaskMC`'s Xoroshiro implementation (whose
    /// generator core is bit-verified against vanilla in `haskmc-util`) to guard the enum dispatch
