...
pub fn plugin_impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        use haskmc::plugin::PluginFuture;
        ...
        #[unsafe(no_mangle)]
        pub static METADATA: std::sync::LazyLock<haskmc::plugin::PluginMetadata> = std::sync::LazyLock::new(|| {
            haskmc::plugin::PluginMetadata {
        ...
        #[unsafe(no_mangle)]
        pub static HASKMC_API_VERSION: u32 = haskmc::plugin::PLUGIN_API_VERSION;

        // Retained so native plugins built for Pumpkin can still be loaded.
        #[unsafe(no_mangle)]
        pub static PUMPKIN_API_VERSION: u32 = haskmc::plugin::PLUGIN_API_VERSION;

        #input_struct

        impl haskmc::plugin::Plugin for #struct_ident {
            #(#methods)*
        }

        #[unsafe(no_mangle)]
        pub fn plugin() -> Box<dyn haskmc::plugin::Plugin> {
