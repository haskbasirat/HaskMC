...
use haskmc_data::sound::Sound;
use haskmc_nbt::{serializer::NbtWriteHelperJava, tag::NbtTag};
...
impl DataComponentCodec<Self> for CustomNameImpl {
    ...
    fn deserialize(seq: &mut impl NetworkReadExt) -> Result<Self, ReadingError> {
        ...
        Ok(Self {
            name: haskmc_util::text::TextComponent::text(String::from(name)),
        })
    }
}
...
impl DataComponentCodec<Self> for LoreImpl {
    fn serialize(&self, seq: &mut impl NetworkWriteExt) -> Result<(), WritingError> {
        ...
        for line in &self.lines {
            seq.write_slice(
                &line.encode_for_version(&haskmc_util::version::JavaMinecraftVersion::V_26_2),
            )?;
        }
        Ok(())
    }
...
impl DataComponentCodec<Self> for ItemNameImpl {
    fn serialize(&self, seq: &mut impl NetworkWriteExt) -> Result<(), WritingError> {
        let mut name = haskmc_nbt::compound::NbtCompound::new();
...
fn deserialize_item_stack_template(
    seq: &mut impl NetworkReadExt,
) -> Result<haskmc_data::item_stack::ItemStack, ReadingError> {
    ...
    Ok(haskmc_data::item_stack::ItemStack::new_with_component(
        count,
        haskmc_data::item::Item::from_id(item_id).unwrap_or(&haskmc_data::item::Item::AIR),
        patch,
    ))
}

fn serialize_item_stack_template(
    stack: &haskmc_data::item_stack::ItemStack,
    seq: &mut impl NetworkWriteExt,
) -> Result<(), WritingError> {
