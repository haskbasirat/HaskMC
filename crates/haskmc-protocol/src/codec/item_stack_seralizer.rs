...
use haskmc_data::data_component_impl::{CustomNameImpl, DataComponentImpl, ItemNameImpl};
use haskmc_data::item::Item;
use haskmc_data::item_id_remap::{remap_item_id_for_version, remap_item_id_from_version};
...
use haskmc_data::data_component_type_id_remap::{
...
fn decode_custom_name(component_data: &[u8]) -> Result<Box<dyn DataComponentImpl>, ReadingError> {
    ...
    let mut nbt_reader = haskmc_nbt::deserializer::NbtReadHelperJava::new(&mut cursor);
