...
use haskmc_data::block_state_remap::remap_block_state_for_version;
use haskmc_data::packet::CURRENT_MC_VERSION;
use haskmc_util::math::position::get_local_cord;
use haskmc_util::version::JavaMinecraftVersion;
use haskmc_world::chunk::ChunkData;
use haskmc_world::chunk::format::LightContainer;
use haskmc_world::chunk::palette::NetworkPalette;
use std::io::Write;
...
pub fn write_chunk_data(
    ...
    if version >= &JavaMinecraftVersion::V_1_21_5 {
        ...
        let mut comp = haskmc_nbt::compound::NbtCompound::new();
        if let Some(ref ws) = heightmaps.world_surface {
            comp.put(
                "WORLD_SURFACE",
                haskmc_nbt::tag::NbtTag::LongArray(ws.to_vec()),
            );
        }
        if let Some(ref mb) = heightmaps.motion_blocking {
            comp.put(
                "MOTION_BLOCKING",
                haskmc_nbt::tag::NbtTag::LongArray(mb.to_vec()),
            );
        }
        if let Some(ref mbnl) = heightmaps.motion_blocking_no_leaves {
            comp.put(
                "MOTION_BLOCKING_NO_LEAVES",
                haskmc_nbt::tag::NbtTag::LongArray(mbnl.to_vec()),
            );
        }
    ...
    for (pos, nbt) in block_entities.iter() {
        ...
        let id = nbt.get_string("id").map_or(0, |id_str| {
            ...
            haskmc_data::block_properties::BLOCK_ENTITY_TYPES
                .iter()
                .position(|&n| n == name)
                .unwrap_or(0)
        });
        let remapped_id =
            haskmc_data::block_entity_type_id_remap::remap_block_entity_type_id_for_version(
                id as u32, *version,
            );
        ...
        client_nbt.child_tags.remove("HaskMCCustomData");
