use haskmc_data::packet::clientbound::play::MERCHANT_OFFERS;
    ...
    fn read(
        ...
        for _ in 0..offers_count {
            let (base_cost_a, output, cost_b) = if *version >= JavaMinecraftVersion::V_1_20_5 {
                ...
                let item = haskmc_data::item::Item::from_id(item_id)
                    .unwrap_or(&haskmc_data::item::Item::AIR);
                let base_cost_a = ItemStackSerializer(std::borrow::Cow::Owned(
                    haskmc_data::item_stack::ItemStack::new(count, item),
                ));
                ...
                let cost_b = if has_cost_b {
                    ...
                    let item_b = haskmc_data::item::Item::from_id(item_id_b)
                        .unwrap_or(&haskmc_data::item::Item::AIR);
                    Some(ItemStackSerializer(std::borrow::Cow::Owned(
                        haskmc_data::item_stack::ItemStack::new(count_b, item_b),
                    )))
                } else {
                    None
                };
