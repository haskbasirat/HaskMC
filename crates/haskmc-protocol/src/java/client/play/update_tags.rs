...
pub struct CUpdateTagsPlay<'a> {
    pub tags: &'a [haskmc_data::tag::RegistryKey],
}
...
fn remap_tag_entry_id(key: RegistryKey, id: u16, version: JavaMinecraftVersion) -> u16 {
    match key {
        RegistryKey::Item => haskmc_data::item_id_remap::remap_item_id_for_version(id, version),
        RegistryKey::EntityType => {
            haskmc_data::entity_id_remap::remap_entity_id_for_version(id, version)
        }
        _ => id,
    }
}
