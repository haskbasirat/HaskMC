use haskmc_data::packet::clientbound::play::LOGIN;
use haskmc_util::{resource_location::ResourceLocation, version::JavaMinecraftVersion};
...
) -> haskmc_nbt::compound::NbtCompound {
    use haskmc_nbt::compound::NbtCompound;
    use haskmc_nbt::deserializer::NbtReadHelperJava;
    use haskmc_nbt::tag::NbtTag;
    use std::io::Cursor;

    let mut root = NbtCompound::new();
    let synced = haskmc_data::registry::Registry::get_synced(version);
            ...
            if reg_name == "dimension_type" {
                ...
                for entry in &reg.registry_entries {
                    if let Some(ref data) = entry.data {
                        ...
                        if let Ok(element_nbt) = haskmc_nbt::Nbt::read_unnamed(&mut reader) {
    ...
    fn write_packet_data(
        ...
        if v1_16 {
            ...
            if !v1_20_2 {
                ...
                let nbt_bytes = haskmc_nbt::Nbt::new(String::new(), registry_codec).write();
                ...
                if v1_16_2 && *version < JavaMinecraftVersion::V_1_19 {
                    ...
                    let dim_bytes = haskmc_nbt::Nbt::new(String::new(), dim_type_compound).write();
...
mod tests {
    ...
    fn v1_20_login_packet() {
        ...
        let codec = haskmc_nbt::Nbt::read(&mut reader).expect("NBT codec");
