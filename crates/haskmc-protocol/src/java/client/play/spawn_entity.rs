...
use haskmc_data::entity_id_remap::remap_entity_id_for_version;
use haskmc_data::packet::clientbound::play::ADD_ENTITY;
...
mod tests {
    ...
    fn legacy_tail(velocity: haskmc_util::math::vector3::Vector3<f64>) -> [u8; 6] {
    ...
    fn encode_spawn(version: JavaMinecraftVersion) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        let velocity = haskmc_util::math::vector3::Vector3::new(0.5, -0.5, 0.25);
        let packet = CSpawnEntity::new(
            VarInt(1),
            uuid::Uuid::nil(),
            VarInt(1),
            haskmc_util::math::vector3::Vector3::new(1.0, 2.0, 3.0),
            0.0,
            90.0,
            90.0,
            VarInt(42),
