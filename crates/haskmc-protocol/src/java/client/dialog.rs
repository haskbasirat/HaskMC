...
impl<'a> DialogNBT<'a> {
    ...
    #[must_use]
    pub const fn from_nbt(compound: &'a haskmc_nbt::compound::NbtCompound) -> Self {
    ...
    pub fn write_packet_data(
        ...
        match &self.0 {
            ...
            DialogNBTSource::Nbt(nbt) => {
                let tag = haskmc_nbt::tag::NbtTag::Compound((*nbt).clone());
                write.write_nbt(tag)?;
                Ok(())
            }
        }
...
pub enum DialogNBTSource<'a> {
    Struct(&'a Dialog),
    Nbt(&'a haskmc_nbt::compound::NbtCompound),
}
