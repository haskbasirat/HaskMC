//! Configuration management and serialization for the `HaskMC` server.
#![deny(missing_docs)]
...
use haskmc_util::{Difficulty, GameMode, PermissionLvl, random};
...
/// Root configuration container for `HaskMC` server settings.
#[derive(Deserialize, Serialize, Default)]
#[serde(default)]
pub struct HaskMcConfig {
...
impl LoadConfiguration for HaskMcConfig {
    fn get_path() -> &'static Path {
        Path::new("haskmc.toml")
    }
...
fn write_migrated_configuration<T: Serialize>(path: &Path, config: &T) {
    match toml::to_string(config) {
        Ok(toml_str) => {
            if let Err(err) = fs::write(path, toml_str) {
                warn!(
                    "Couldn't migrate configuration to {}. Reason: {}",
                    path.display(),
                    err
                );
            }
        }
        Err(err) => warn!("Couldn't serialize migrated configuration. Reason: {err}"),
    }
}
...
pub trait LoadConfiguration {
        ...
        Self: Sized + Default + Serialize + DeserializeOwned,
    {
        ...
        let primary_path = config_dir.join(Self::get_path());
        let legacy_path = Self::get_legacy_path().map(|path| config_dir.join(path));
        let path = if primary_path.exists() {
            primary_path.clone()
        } else if let Some(path) = legacy_path.filter(|path| path.exists()) {
            println!(
                "Loading legacy configuration from {} and migrating it to {}.",
                path.display(),
        ...
        let should_migrate = path != primary_path && path.exists();
        ...
        if should_migrate {
            write_migrated_configuration(&primary_path, &config);
        }
    ...
    /// Returns an optional legacy configuration path used for one-time migration.
    #[must_use]
    fn get_legacy_path() -> Option<&'static Path> {
        None
    }
