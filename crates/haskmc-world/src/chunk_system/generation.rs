...
mod tests {
    ...
    fn dimensions_taller_than_their_noise_settings_generate_all_sections() {
        for (dimension, terrain_state) in [
            (
                ...
                haskmc_data::Block::NETHERRACK.default_state.id,
            ),
            (
                Dimension::THE_END,
                haskmc_data::Block::END_STONE.default_state.id,
            ),
        ] {
            let seed = Seed(42);
    ...
    fn configured_seed_generates_vanilla_ancient_city_chunk() {
        ...
        for x in 496..512 {
            for z in -192..-176 {
                for y in -64..320 {
                    let block = chunk
                        .get_block_state(&haskmc_util::math::vector3::Vector3::new(x, y, z))
                        .to_block_id();
                    if [
                        haskmc_data::Block::DEEPSLATE_BRICKS.id,
                        haskmc_data::Block::POLISHED_DEEPSLATE.id,
                        haskmc_data::Block::REINFORCED_DEEPSLATE.id,
                        haskmc_data::Block::SCULK.id,
                    ]
                    .contains(&block)
                    {
                        city_blocks += 1;
                    }
                    if block == haskmc_data::Block::JIGSAW.id {
                        jigsaw_blocks += 1;
                    }
                }
            }
            ...
            for z in -1328..-1312 {
                for y in -64..320 {
                    let block = chunk
                        ...
                        .to_block_id();
                    if [
                        haskmc_data::Block::DARK_OAK_LOG.id,
                        haskmc_data::Block::DARK_OAK_PLANKS.id,
                        haskmc_data::Block::DARK_OAK_FENCE.id,
                    ]
                    .contains(&block)
                    {
                        outpost_blocks += 1;
    ...
    fn fixed_seed_generates_vanilla_end_ship_chunk() {
        ...
        for y in 123..=146 {
            for x in -4896..=-4881 {
                for z in -4400..=-4393 {
                    let state =
                        chunk.get_block_state(&haskmc_util::math::vector3::Vector3::new(x, y, z));
    ...
    fn pillager_outpost_features_shape_ground_at_vanilla_height() {
        ...
        for (x, y, z) in [(1173, 70, -1311), (1173, 70, -1305)] {
            let state = chunk.get_block_state(&haskmc_util::math::vector3::Vector3::new(x, y, z));
            assert_eq!(state.to_block_id(), haskmc_data::Block::GRASS_BLOCK.id);
        }
        ...
        let state =
            cage_chunk.get_block_state(&haskmc_util::math::vector3::Vector3::new(1183, 68, -1330));
        assert_eq!(state.to_block_id(), haskmc_data::Block::GRASS_BLOCK.id);
    }
}
