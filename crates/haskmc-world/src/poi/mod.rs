...
impl PoiRegion {
    ...
    fn compress_chunk_data(chunk_data: &PoiChunkData) -> std::io::Result<Vec<u8>> {
        let mut root = haskmc_nbt::compound::NbtCompound::new();
        root.put_int("DataVersion", chunk_data.data_version);

        let mut sections_comp = haskmc_nbt::compound::NbtCompound::new();
        for (sec_key, sec_data) in &chunk_data.sections {
            let mut sec_comp = haskmc_nbt::compound::NbtCompound::new();
            sec_comp.put_byte("Valid", sec_data.valid);
            let mut rec_list = Vec::new();
            for rec in &sec_data.records {
                let mut rec_comp = haskmc_nbt::compound::NbtCompound::new();
                ...
                rec_list.push(haskmc_nbt::tag::NbtTag::Compound(rec_comp));
            }
        ...
        let uncompressed = haskmc_nbt::Nbt::from(root).write();
    ...
    fn decompress_chunk_data(compressed: &[u8]) -> std::io::Result<PoiChunkData> {
        ...
        let nbt = haskmc_nbt::Nbt::read(&mut reader)
        ...
        if let Some(sec_tag) = nbt.get_compound("Sections") {
            for (sec_key, tag) in &sec_tag.child_tags {
                if let haskmc_nbt::tag::NbtTag::Compound(sec_comp) = tag {
                    let valid = sec_comp.get_byte("Valid").unwrap_or(1);
                    let mut records = Vec::new();
                    if let Some(haskmc_nbt::tag::NbtTag::List(rec_list)) = sec_comp.get("Records") {
                        for rec_t in rec_list {
                            if let haskmc_nbt::tag::NbtTag::Compound(rc) = rec_t {
...
mod tests {
    ...
    fn poi_find_closest_matching() {
        let mut storage = PoiStorage::new(std::env::temp_dir().join("haskmc_poi_closest_test"));
    ...
    #[test]
    fn poi_storage_mca() {
        let dir = std::env::temp_dir().join("haskmc_poi_mca_test");
