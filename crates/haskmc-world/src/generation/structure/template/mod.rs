...
//!
//! ```ignore
//! use haskmc_world::generation::structure::template::{TemplateCache, TemplatePiece};
//! use haskmc_data::Rotation;
//!
...
use haskmc_data::Mirror;
use haskmc_data::Rotation;
use haskmc_nbt::{compound::NbtCompound, tag::NbtTag};
use haskmc_util::math::vector3::Vector3;
use haskmc_util::random::{RandomImpl, hash_block_pos, legacy_rand::LegacyRand};
...
pub use haskmc_data::{BlockState, Mirror as BlockMirror, Rotation as BlockRotation};
...
pub fn place_template(
    ...
    chunk_box: Option<&haskmc_util::math::block_box::BlockBox>,
) {
...
pub fn place_template_with_options(
    ...
    for block in &template.blocks {
        ...
        if apply_waterlogging
            && placer.get_block_state(&world_pos).to_block_id() == haskmc_data::Block::WATER.id
        ...
        if skip_air && state.id.to_block_id() == haskmc_data::Block::AIR.id {
            continue;
        }
        ...
        let final_block = haskmc_data::Block::from_id(state.id.to_block_id());
