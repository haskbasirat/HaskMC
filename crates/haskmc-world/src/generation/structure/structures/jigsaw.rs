...
use haskmc_util::math::block_box::BlockBox;
...
pub enum PoolElementKind {
    ...
    List(Vec<Self>),
    Feature(haskmc_data::placed_feature::PlacedFeature),
}
    ...
    fn into_element(self) -> Option<(PoolElementKind, JigsawProjection)> {
        match self {
            ...
            Self::Feature {
                ...
                haskmc_data::placed_feature::PlacedFeature::from_name(feature)
...
impl PoolElementKind {
    ...
    #[must_use]
    pub fn get_bounding_box(&self, offset: BlockPos, rotation: haskmc_data::Rotation) -> BlockBox {
        match self {
            Self::Single { template, .. } => {
    ...
    pub fn get_shuffled_jigsaw_blocks(
        ...
        random: &mut haskmc_util::random::RandomGenerator,
    ) -> Vec<JigsawBlock> {
        match self {
            Self::Single { template, .. } => {
            ...
            Self::Feature(_) => vec![JigsawBlock {
                ...
                facing: haskmc_util::BlockDirection::Down,
                up: haskmc_util::BlockDirection::South,
...
impl PoolElement {
    #[must_use]
    pub const fn feature(&self) -> Option<haskmc_data::placed_feature::PlacedFeature> {
        match self.kind {
...
pub struct JigsawBlock {
    ...
    pub facing: haskmc_util::BlockDirection,
    pub up: haskmc_util::BlockDirection,
...
impl JigsawBlock {
    ...
    pub fn from_template_block(
        ...
        let facing = match facing_part {
            "south" => haskmc_util::BlockDirection::South,
            "east" => haskmc_util::BlockDirection::East,
            "west" => haskmc_util::BlockDirection::West,
            "up" => haskmc_util::BlockDirection::Up,
            "down" => haskmc_util::BlockDirection::Down,
            _ => haskmc_util::BlockDirection::North,
        };

        let up = match up_part {
            "north" => haskmc_util::BlockDirection::North,
            "south" => haskmc_util::BlockDirection::South,
            "east" => haskmc_util::BlockDirection::East,
            "west" => haskmc_util::BlockDirection::West,
            "down" => haskmc_util::BlockDirection::Down,
            _ => haskmc_util::BlockDirection::Up,
        };
    ...
    pub fn can_attach(
        source: &Self,
        target_facing: haskmc_util::BlockDirection,
        target_name: &str,
    ) -> bool {
...
impl StructurePieceBase for PoolElementStructurePiece {
    ...
    fn place(
        ...
        random: &mut haskmc_util::random::RandomGenerator,
        _seed: i64,
        chunk_box: &haskmc_util::math::block_box::BlockBox,
    ) {
        let origin =
            haskmc_util::math::vector3::Vector3::new(self.pos.0.x, self.pos.0.y, self.pos.0.z);
        ...
        self.element
            .for_each_template(|_name, processor_list, legacy, template| {
                ...
                let placement_origin = haskmc_util::math::vector3::Vector3::new(
...
pub struct JigsawGenerator {
    ...
    pub pool_aliases: &'static [haskmc_data::structures::PoolAliasBinding],
}
...
impl JigsawGenerator {
    ...
    pub const fn with_pool_aliases(
        mut self,
        aliases: &'static [haskmc_data::structures::PoolAliasBinding],
    ) -> Self {
...
impl StructureGenerator for JigsawGenerator {
    fn get_structure_position(
        ...
        let structure = context
            .structure_key
            .map(|key| haskmc_data::structures::Structure::get(&key));
...
mod tests {
    ...
    fn ancient_city_builds_a_multi_piece_graph() {
        ...
        let context = StructureGeneratorContext {
            ...
            min_y: -64,
            height_sampler: None,
            structure_key: Some(haskmc_data::structures::StructureKeys::AncientCity),
        };
    ...
    fn pillager_outpost_builds_the_vanilla_jigsaw_graph() {
        ...
        let world_gen = crate::generation::get_world_gen(
            haskmc_util::world_seed::Seed(SEED as u64),
            haskmc_data::dimension::Dimension::OVERWORLD,
            false,
            Vec::new(),
            String::new(),
        );
        ...
        let context = StructureGeneratorContext {
            ...
            structure_key: Some(haskmc_data::structures::StructureKeys::PillagerOutpost),
        };
