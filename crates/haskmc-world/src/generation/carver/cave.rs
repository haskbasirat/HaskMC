...
use haskmc_data::carver::{CarverAdditionalConfig, CarverConfig, HeightProvider};
...
impl CaveCarver {
    ...
    fn create_room(
        ...
        let horizontal_radius =
            1.5 + f64::from(haskmc_util::math::sin(std::f32::consts::FRAC_PI_2) * thickness);
    ...
    fn create_tunnel(
        ...
        for current_step in step..dist {
            ...
            let horizontal_radius =
                1.5 + f64::from(haskmc_util::math::sin(progress_arg) * thickness);
            let vertical_radius = horizontal_radius * y_scale;
            let cos_x = haskmc_util::math::cos(vertical_rotation);
            x += f64::from(haskmc_util::math::cos(horizontal_rotation) * cos_x);
            y += f64::from(haskmc_util::math::sin(vertical_rotation));
            z += f64::from(haskmc_util::math::sin(horizontal_rotation) * cos_x);
    ...
    fn carve_block(
        ...
        if block.id == haskmc_data::Block::GRASS_BLOCK.id
            || block.id == haskmc_data::Block::MYCELIUM.id
        {
            *has_grass = true;
        }
...
mod tests {
    use super::*;
    use haskmc_data::carver::CAVE;
    use haskmc_data::{Block, BlockStateId, dimension::Dimension};
    ...
    fn carve_at(
        ...
        y: i32,
        z: i32,
        initial_state: &'static haskmc_data::BlockState,
    ...
    fn find_aquifer_carve_state(
        run: &mut Run,
        predicate: impl Fn(&'static haskmc_data::BlockState, bool) -> bool,
    ) -> Option<(i32, i32, i32, &'static haskmc_data::BlockState)> {
