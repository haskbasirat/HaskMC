...
impl CanyonCarver {
    ...
    fn do_carve(
        ...
        for current_step in step..distance {
            ...
            let mut horizontal_radius =
                1.5 + f64::from(haskmc_util::math::sin(progress) * thickness);
            ...
            let xc = haskmc_util::math::cos(vertical_rotation);
            let xs = haskmc_util::math::sin(vertical_rotation);
            x += f64::from(haskmc_util::math::cos(horizontal_rotation) * xc);
            y += xs as f64;
            z += f64::from(haskmc_util::math::sin(horizontal_rotation) * xc);
    ...
    fn carve_block(
        ...
        let block = haskmc_data::Block::from_state_id(state_id);
        ...
        if block.id.has_tag(config.replaceable) {
            ...
            place_carved_block(
                run,
                haskmc_util::math::vector3::Vector3::new(x, y, z),
                state,
