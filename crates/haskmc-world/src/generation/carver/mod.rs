...
use haskmc_data::block_state::BlockState;
use haskmc_data::carver::{CANYON, CAVE, CAVE_EXTRA_UNDERGROUND, NETHER_CAVE};
use haskmc_data::carver::{CarverAdditionalConfig, CarverConfig};
use haskmc_data::chunk_gen_settings::MaterialRule;
...
use haskmc_util::random::{RandomGenerator, RandomImpl};
...
impl CarverBlockIds {
    ...
    pub fn new() -> Self {
        Self {
            air: haskmc_data::Block::AIR.default_state,
            cave_air: haskmc_data::Block::CAVE_AIR.default_state,
            lava: haskmc_data::Block::LAVA.default_state,
            dirt: haskmc_data::Block::DIRT.default_state,
            grass_block: haskmc_data::Block::GRASS_BLOCK.default_state,
            mycelium: haskmc_data::Block::MYCELIUM.default_state,
        }
    }
}
...
fn get_large_feature_seed(seed: u64, chunk_x: i32, chunk_z: i32) -> u64 {
    let mut random = haskmc_util::random::legacy_rand::LegacyRand::from_seed(seed);
...
const fn new_carver_random(seed: u64, non_vanilla_random: bool) -> RandomGenerator {
    if non_vanilla_random {
        RandomGenerator::Xoroshiro(haskmc_util::random::xoroshiro128::Xoroshiro::from_seed(
            seed,
        ))
    } else {
        RandomGenerator::Legacy(haskmc_util::random::legacy_rand::LegacyRand::from_seed(
            seed,
        ))
    }
}
...
fn schedule_fluid_tick_for_state(
    ...
    if state.id == haskmc_data::Block::WATER.default_state.id {
        chunk.schedule_fluid_tick(x, y, z, &Fluid::WATER);
    } else if state.id == haskmc_data::Block::LAVA.default_state.id {
...
mod tests {
    ...
    use haskmc_data::chunk_gen_settings::{
