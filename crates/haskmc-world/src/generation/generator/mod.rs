...
use haskmc_data::structures::{StructurePlacementCalculator, StructureSet};
...
pub trait CustomChunkGenerator: Send + Sync {
    ...
    fn default_block(&self) -> &'static BlockState {
        haskmc_data::Block::AIR.default_state
    }
...
impl WorldGenerator {
    ...
    #[must_use]
    pub fn find_spawn_position(&self) -> haskmc_util::math::position::BlockPos {
        match self {
            Self::Noise(noise_gen) => noise_gen.find_spawn_position(),
            _ => haskmc_util::math::position::BlockPos::ZERO,
        }
    }
}
...
impl VanillaGenerator {
    ...
    pub fn find_spawn_position(&self) -> haskmc_util::math::position::BlockPos {
        if self.settings.spawn_target.is_empty() {
            return haskmc_util::math::position::BlockPos::ZERO;
        }
