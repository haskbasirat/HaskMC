...
use haskmc_data::block_properties::is_air;
use haskmc_data::chunk::DoublePerlinNoiseParameters;
use haskmc_data::dimension::Dimension;
use haskmc_data::fluid::{Fluid, FluidState};
use haskmc_data::structures::{
    Structure, StructureKeys, StructurePlacementType, StructureSet, WeightedEntry,
};
use haskmc_data::tag::RegistryKey;
use haskmc_data::{Block, BlockState, block_properties::blocks_movement, chunk::Biome};
use haskmc_data::{BlockId, BlockStateId, tag};
use haskmc_util::random::xoroshiro128::XoroshiroSplitter;
use haskmc_util::random::{RandomImpl, get_carver_seed};
use haskmc_util::{
    HeightMap,
...
use haskmc_data::tag::get_tag_ids;
        ...
        for (set_index, set) in StructureSet::ALL.iter().enumerate() {
            ...
            match &set.placement.placement_type {
                StructurePlacementType::RandomSpread(spread) => {
                    let region_x = haskmc_util::math::floor_div(self.x, spread.spacing);
                    let region_z = haskmc_util::math::floor_div(self.z, spread.spacing);
