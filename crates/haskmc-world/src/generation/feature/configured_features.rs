...
use haskmc_util::{math::position::BlockPos, random::RandomGenerator};
...
pub static CONFIGURED_FEATURES: LazyLock<
    HashMap<haskmc_data::configured_feature::ConfiguredFeature, ConfiguredFeature>,
> = LazyLock::new(build_configured_features);

pub static BONE_MEAL_FEATURES: LazyLock<
    HashSet<haskmc_data::configured_feature::ConfiguredFeature>,
> = LazyLock::new(|| {
    haskmc_data::tag::get_tag_values(
        haskmc_data::tag::RegistryKey::WorldgenConfiguredFeature,
        "minecraft:can_spawn_from_bone_meal",
    )
    .into_iter()
    .flatten()
    .filter_map(|name| haskmc_data::configured_feature::ConfiguredFeature::from_name(name))
    .collect()
});
...
include!("../../../../haskmc-data/src/generated/configured_features_generated.rs");
...
mod tests {
    ...
    fn bonemeal_features_use_tag_output() {
        let tag_values = haskmc_data::tag::get_tag_values(
            haskmc_data::tag::RegistryKey::WorldgenConfiguredFeature,
            "minecraft:can_spawn_from_bone_meal",
        );
