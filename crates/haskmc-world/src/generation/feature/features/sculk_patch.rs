...
use haskmc_data::tag::Block::MINECRAFT_SCULK_REPLACEABLE_WORLD_GEN;
...
impl SculkPatchFeature {
    pub fn generate<T: GenerationCache>(
        ...
        for _ in 0..extra_growths {
            ...
            if state.is_air() && below_state.is_side_solid(haskmc_data::BlockDirection::Up) {
        ...
        for _ in 0..self.extra_rare_growths.get(random) {
            ...
            if proto_chunk_state(chunk, candidate).is_some_and(|state| state.to_state().is_air())
                && proto_chunk_state(chunk, below).is_some_and(|state| {
                    state
                        .to_state()
                        .is_side_solid(haskmc_data::BlockDirection::Up)
                })
            {
...
fn sculk_vein_state_with_face(
    ...
    face: haskmc_data::BlockDirection,
    ...
    match face {
        haskmc_data::BlockDirection::Down => properties.r#down = true,
        haskmc_data::BlockDirection::Up => properties.r#up = true,
        haskmc_data::BlockDirection::North => properties.r#north = true,
        haskmc_data::BlockDirection::South => properties.r#south = true,
        haskmc_data::BlockDirection::West => properties.r#west = true,
        haskmc_data::BlockDirection::East => properties.r#east = true,
    }
...
fn set_proto_chunk_state(
    chunk: &mut crate::ProtoChunk,
    pos: BlockPos,
    state: &'static haskmc_data::BlockState,
) {
...
fn grow_sculk_veins<T: GenerationCache>(chunk: &mut T, sculk_pos: BlockPos) {
    for dir in haskmc_data::BlockDirection::all() {
