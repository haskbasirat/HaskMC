...
mod test {
    ...
    fn structure_references_are_rebuilt_when_resuming_generation() {
        ...
        use haskmc_data::structures::StructureKeys;
    ...
    fn heightmap_roundtrip_through_chunk_data_resume() {
        ...
        assert_eq!(chunk_data.status, haskmc_data::chunk::ChunkStatus::Noise);
            ...
            for local_y in 0..dumped_height {
                for z in 0..16usize {
                    ...
                    if mismatches < 10 {
                        ...
                        let act_block = haskmc_data::BlockState::from_id(actual).id.to_block().name;
                        let exp_block = haskmc_data::BlockState::from_id(
                            haskmc_data::BlockStateId::new(expected).unwrap(),
                        )
                        .id
                        .to_block()
                        .name;
        ...
        for x in 0..16usize {
            for local_y in dumped_window_height(expected_data)..chunk.height() as usize {
                for z in 0..16usize {
                    let actual = chunk.get_block_state_raw(x as i32, local_y as i32, z as i32);
                    assert!(
                        haskmc_data::BlockState::from_id(actual).is_air(),
    ...
    fn no_blend_no_beard_0_0() {
        let expected: Vec<u16> = haskmc_util::read_data_from_file!(
