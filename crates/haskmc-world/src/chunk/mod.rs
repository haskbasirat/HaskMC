...
use haskmc_data::block_properties::{blocks_movement, has_random_ticks, is_air};
use haskmc_data::chunk::ChunkStatus;
use haskmc_data::fluid::Fluid;
use haskmc_data::tag::Block::MINECRAFT_LEAVES;
use haskmc_data::{Block, BlockState, BlockStateId};
...
pub enum ChunkParsingError {
    ...
    FailedReadStatus(haskmc_nbt::Error),
...
pub enum ChunkSerializingError {
    ...
    ErrorSerializingChunk(haskmc_nbt::Error),
}
...
mod tests {
    ...
    use haskmc_data::{Block, block_properties::has_random_ticks};
    ...
    fn chunk_custom_data() {
        ...
        chunk.set_custom_data(
            "my_plugin",
            "test_key",
            NbtTag::String("hello_haskmc".into()),
        );
        assert!(chunk.has_custom_data("my_plugin", "test_key"));
        assert_eq!(
            chunk.get_custom_data("my_plugin", "test_key"),
            Some(NbtTag::String("hello_haskmc".into()))
        );
