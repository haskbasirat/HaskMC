...
pub fn bedrock_water_state(id: BlockStateId) -> BlockStateId {
    ...
    if state.is_waterlogged() || (state.is_liquid() && Fluid::from_state_id(id).is_none()) {
        haskmc_data::Block::WATER.default_state.id
    } else {
        haskmc_data::Block::AIR.default_state.id
    }
}
        ...
        match self {
            Self::Homogeneous(registry_id) => BeNetworkSerialization {
                ...
                palette: NetworkPalette::Single(
                    haskmc_data::biome::Biome::from_id(*registry_id).map_or(1, |b| b.be_network_id),
                ),
                packed_data: Box::new([]),
            },
            Self::Heterogeneous(data) => {
                ...
                BeNetworkSerialization {
                    ...
                    palette: NetworkPalette::Indirect(
                        data.palette
                            .iter()
                            .map(|&id| {
                                haskmc_data::biome::Biome::from_id(id)
...
impl BlockPalette {
    ...
    pub fn convert_be_water_network(&self) -> Option<BeNetworkSerialization<u16>> {
        let air = haskmc_data::Block::AIR.default_state.id;
        let water = haskmc_data::Block::WATER.default_state.id;
        match self {
            Self::Homogeneous(id) => {
...
mod tests {
    ...
    use haskmc_data::{Block, BlockStateId};
