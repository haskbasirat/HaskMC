...
use bytes::Bytes;
use haskmc_data::{Block, BlockStateId, chunk::ChunkStatus, fluid::Fluid};
use haskmc_nbt::compound::NbtCompound;
use haskmc_util::resource_location::{FromResourceLocation, ResourceLocation, ToResourceLocation};
...
fn extract_u16_array(tag: &haskmc_nbt::tag::NbtTag) -> Option<Box<[BlockStateId]>> {
    match tag {
        haskmc_nbt::tag::NbtTag::IntArray(arr) => Some(
            arr.iter()
                .map(|&x| BlockStateId::new_or_air(x as u16))
                .collect(),
        ),
        haskmc_nbt::tag::NbtTag::ByteArray(arr) => Some(
            arr.iter()
                .map(|&x| BlockStateId::new_or_air(x as u16))
                .collect(),
        ),
        haskmc_nbt::tag::NbtTag::LongArray(arr) => Some(
            arr.iter()
                .map(|&x| BlockStateId::new_or_air(x as u16))
                .collect(),
        ),
        haskmc_nbt::tag::NbtTag::List(list) => {
            let ids: Box<[BlockStateId]> = list
                .iter()
                .map(|t| match t {
                    haskmc_nbt::tag::NbtTag::Int(x) => BlockStateId::new_or_air(*x as u16),
                    haskmc_nbt::tag::NbtTag::Short(x) => BlockStateId::new_or_air(*x as u16),
                    haskmc_nbt::tag::NbtTag::Byte(x) => BlockStateId::new_or_air(*x as u16),
                    haskmc_nbt::tag::NbtTag::Long(x) => BlockStateId::new_or_air(*x as u16),
                    haskmc_nbt::tag::NbtTag::Compound(compound) => {
                        if let Ok(entry) =
...
fn extract_u8_array(tag: &haskmc_nbt::tag::NbtTag) -> Option<Box<[u8]>> {
    match tag {
        haskmc_nbt::tag::NbtTag::ByteArray(arr) => Some(arr.iter().map(|&x| x as u8).collect()),
        haskmc_nbt::tag::NbtTag::IntArray(arr) => Some(arr.iter().map(|&x| x as u8).collect()),
        haskmc_nbt::tag::NbtTag::List(list) => {
            let bytes: Box<[u8]> = list
                .iter()
                .map(|t| match t {
                    haskmc_nbt::tag::NbtTag::Byte(x) => *x as u8,
                    haskmc_nbt::tag::NbtTag::Int(x) => *x as u8,
                    haskmc_nbt::tag::NbtTag::Short(x) => *x as u8,
                    haskmc_nbt::tag::NbtTag::String(s) => {
                        let name = s.strip_prefix("minecraft:").unwrap_or(s);
                        haskmc_data::biome::Biome::from_name(name).map_or(0, |b| b.id)
                    }
                    _ => 0,
                })
                .collect();
...
fn parse_scheduled_tick<T>(nbt: &haskmc_nbt::compound::NbtCompound) -> Option<ScheduledTick<T>>
where
...
impl ChunkData {
    ...
    pub fn internal_from_bytes(
        ...
        let mut reader = haskmc_nbt::deserializer::NbtReadHelperJava::new(&mut cursor);
        let nbt = if is_named {
            haskmc_nbt::Nbt::read(&mut reader)
        } else {
            haskmc_nbt::Nbt::read_unnamed(&mut reader)
        }
        ...
        if let Some(sections_list) = root_tag.get_list("sections") {
            for section_tag in sections_list {
                if let haskmc_nbt::tag::NbtTag::Compound(section_compound) = section_tag {
        ...
        if let Some(list) = root_tag.get_list("block_ticks") {
            for tag in list {
                if let haskmc_nbt::tag::NbtTag::Compound(compound) = tag
        ...
        if let Some(list) = root_tag.get_list("block_entities") {
            for tag in list {
                if let haskmc_nbt::tag::NbtTag::Compound(nbt) = tag
        ...
        let custom_data = root_tag
            ...
            .or_else(|| root_tag.get_compound("PumpkinCustomData"))
    ...
    fn internal_to_bytes(&self) -> Bytes {
        ...
        for i in 0..self.section.count {
            ...
            let biome_palette_tags: Vec<NbtTag> = biomes_nbt
                ...
                .map(|&val| {
                    let name =
                        haskmc_data::biome::Biome::from_id(val).map_or("plains", |b| b.registry_id);
        ...
        if !custom_data.is_empty() {
            root_compound.put_compound("HaskMCCustomData", custom_data.clone());
        }

        let nbt = haskmc_nbt::Nbt::from(root_compound);
        nbt.write()
    }
...
impl ChunkEntityData {
    fn internal_from_bytes(
        ...
        let pos_array = match (nbt.get_int("Position-X"), nbt.get_int("Position-Z")) {
            (Some(x), Some(z)) => [x, z],
            _ => {
                if let Some(haskmc_nbt::tag::NbtTag::IntArray(pos)) = nbt.get("Position") {
                    if pos.len() >= 2 {
                        [pos[0], pos[1]]
                    } else {
                        [0, 0]
        ...
        let entities = match nbt.get("Entities") {
            Some(haskmc_nbt::tag::NbtTag::List(list)) => list
                .iter()
                .filter_map(|t| match t {
                    haskmc_nbt::tag::NbtTag::Compound(c) => Some(c.clone()),
                    _ => None,
                })
                .collect(),
            _ => Vec::new(),
    ...
    fn internal_to_bytes(&self) -> Bytes {
        ...
        root.put(
            "Position",
            haskmc_nbt::tag::NbtTag::IntArray(vec![self.x, self.z]),
        );
        let entities_tag: Vec<haskmc_nbt::tag::NbtTag> = self
            .data
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .iter()
            .map(|c| haskmc_nbt::tag::NbtTag::Compound(c.clone()))
            .collect();
        root.put_list("Entities", entities_tag);

        let nbt = haskmc_nbt::Nbt::from(root);
        nbt.write()
    }
}
...
mod tests {
    ...
    fn extract_u8_array_from_vanilla_string_palette() {
        ...
        assert_eq!(
            result[0],
            haskmc_data::biome::Biome::from_name("plains").unwrap().id
        );
        assert_eq!(
            result[1],
            haskmc_data::biome::Biome::from_name("the_void").unwrap().id
        );
    }
}
